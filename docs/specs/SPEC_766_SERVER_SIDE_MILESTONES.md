...
| `first_deliverable` | `tribe_deliverables.status='completed'` + `assigned_member_id` | ✅ existe | trigger `AFTER INSERT OR UPDATE ON tribe_deliverables` quando `status='completed'` (espelha `trg_tribe_deliverable_completed_xp`; **chave = status, não `completed_at`** — esta é coluna de auditoria derivada que o próprio trigger de XP seta; PR3 aterrou 1 linha com `status='completed'` e `completed_at` NULL) |
| `promotion` | `members.operational_role='tribe_leader'` (papel leader-track real; **NÃO existe valor `'leader'`** — 6 tribe_leaders ao vivo). **Não há tabela `role_transitions`** (0 relations). A RPC `promote_to_leader_track` **NÃO seta `operational_role`** (só mexe em `selection_applications`); quem seta `tribe_leader` é o trigger `sync_operational_role_cache` a partir de `auth_engagements` | ✅ existe (PR4) | trigger `AFTER UPDATE OF operational_role ON members WHEN (NEW='tribe_leader' AND OLD<>'tribe_leader')` — captura QUALQUER path; **sem invariante** (cache mutável, demoção rotineira) |
...
5. **Promoção recorrente NÃO é suportada na v1.** `UNIQUE(member_id,'promotion')` bloqueia uma 2ª celebração (promovido→despromovido→repromovido). Se o requisito surgir, exige nova chave (`promotion_2`) ou mudança do modelo UNIQUE — registrar como decisão, não bug.
   - **Clarificação PR4 (semântica "1ª gravação", não "1ª promoção"):** o backfill silencioso cobre só quem é `tribe_leader` no momento do deploy. Um membro promovido→despromovido **antes** do deploy NÃO é backfillado; se for repromovido **depois**, o trigger dispara e ele celebra — porque é a 1ª vez que o **marco é gravado**, não a 1ª promoção histórica. Aceitável no v1 (não há tabela `role_transitions` para reconstruir histórico). Documentado no header da mig `…204`.
...
- **PR 4 — `promotion`. ✅ ENTREGUE (mig `20260805000204`).** Trigger `AFTER UPDATE OF operational_role ON members FOR EACH ROW WHEN (NEW.operational_role='tribe_leader' AND OLD.operational_role IS DISTINCT FROM 'tribe_leader')` — o trigger captura QUALQUER path de promoção (inclusive admin direto), preferido ao gancho em `promote_to_leader_track` (esta RPC **não** seta `operational_role` — só mexe em `selection_applications`; quem seta `tribe_leader` é `sync_operational_role_cache` a partir de `auth_engagements`). 🔴 **Correção de grounding:** NÃO existe valor `operational_role='leader'` — o papel de liderança de tribo é **`tribe_leader`** (6 ao vivo); o `'leader'` do rascunho original deste SPEC estava errado. Escopo NARROW (só `tribe_leader`; `manager`/`deputy_manager` são papéis GP-estruturais). `schema-cache-columns.test.mjs` (gate ADR-0012): **não engajado** — trigger não adiciona coluna. **Nenhum invariante novo** (contagem fica 31): `promotion` não tem fonte imutável (`operational_role` é cache mutável; demoção é rotina), então um check direcional geraria falso-positivo permanente em todo líder rotacionado — guard estrutural = WHEN + UNIQUE + `record_milestone` REVOKE. Council: `data-architect` + `security-engineer` (ambos GO-with-changes — todas aplicadas).
