...
by nature (relabel); `get_public_impact_data` (LGPD-public, hardcoded `'2026-03-01'`) → **PR2 sibling** (also present-blind impact-hours — #420 seal precondition);
`detect_and_notify_detractors` → reconciled with seal track in PR11 (verified already `present=true`-safe, no change; the SEPARATE `get_admin_dashboard` detractor/dropout alerts ARE present-blind → #420).
...
10. **PR10 — p175 gate extension:** forward-defense regex (no inline rate re-impl) + visibility hard-gate + grant ladder. **✅ SHIPPED** — contract test `p277-419-m3-pr10-p175-gate-attendance.test.mjs` locks three invariants (no migration): (A) the canonical eligibility primitive `_attendance_eligible_events` is type-based and the engagement/reliability summaries delegate to it — no metric primitive may reintroduce `event_tag_assignments`/`general_meeting`/`tribe_meeting` tag-based eligibility (§3b); (B) reliability-visibility hard-gate — anon-granted public RPCs (`get_public_leaderboard`/`get_public_platform_stats`/`get_public_impact_data`/`get_public_trail_ranking`) never surface reliability (D10); (C) grant ladder — the 5 engagement+reliability primitives are `service_role`-only (no anon/authenticated), `get_dropout_risk_members` is authenticated + internal `manage_event` gate, never anon. DB-gated checks read live bodies via `_audit_list_public_function_bodies()` so the gate auto-tracks the real schema. Deferred polish (not a hard-gate violation): `TribeDashboardIsland.tsx` monthly-trend line label "Taxa de Presença" → "Participação" (the series plots `exec_tribe_dashboard` engagement post-PR3b; "Presença" is used generically across that dashboard).
11. **PR11 — Seal track. ✅ SHIPPED** (migration `20260805000079`): `events.roster_sealed_at` + `seal_event_attendance(p_event_id)` materializes absent rows (present=false, excused=false) for eligible no-shows in the operational-union cohort via canonical `_attendance_eligible_events` (ON CONFLICT (event_id,member_id) DO NOTHING, idempotent/non-destructive; `manage_event` gate fail-closed; REVOKE anon / GRANT authenticated+service_role). **Onboarding sealing-safety**: `auto_complete_first_meeting` (trigger) + `auto_detect_onboarding_completions` (batch twin) guarded with `present=true` so a sealed absence cannot complete `first_meeting`. `sync_attendance_points` + `detect_and_notify_detractors` verified already `present=true`-safe (no body change). **Demonstrated antes→depois** (rolled-back seal of the 2026-05-21 geral, 15 no-shows materialized): reliability_global **0.9911 → 0.9557** (absent_total 5→20), engagement_global **0.7991 → 0.7991 UNCHANGED** — sealing moves only the recorded-denominator reliability, never the engagement headline. Reliability stays a coverage-gated self/admin diagnostic (D10 + PR10 hard-gate). **PRECONDITION before any REAL seal coverage** = repoint 3 present-blind consumers to present-aware semantics (#420): `get_public_impact_data` (anon impact-hours), `get_member_attendance_hours` (hours+streak), `get_admin_dashboard` (dropout/detractor alerts) — each reads row-existence/absence as attendance and would regress on seal.
