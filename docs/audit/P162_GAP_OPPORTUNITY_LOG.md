...
- **Sediment learnings (3 NEW):**
  ...
  - **SEDIMENT-232.B**: `apply_migration` MCP no longer creates shadow rows at NOW() (p227 SEDIMENT-227.A) — verified empirically across 4 apply_migration calls in this session, schema_migrations rowcount unchanged until `supabase migration repair --status applied <ts>` was run locally. Either the bug was patched upstream or the MCP version changed. Watch behavior next session to confirm.
