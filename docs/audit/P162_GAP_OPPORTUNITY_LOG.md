...
- **Trigger:** Após shipar PR #275 (OAuth allowlist subdomains) + PR #276 (execution.taskSupport strip), PM reconectou Perplexity → connector mostra "Connected · vitor@vitormr.dev" MAS tools tab ainda "No tools to display". Server-side tools/list HTTP 200 com 299 tools spec-compliant. Reject is happening INSIDE Perplexity client por motivo desconhecido.
...
- **Live audit row verified** post-dispatch via `SELECT FROM pii_access_log WHERE id = 'ee6a1541-...'`:
  * `accessor_id` = `880f736c-3e76-4df4-9375-33575c190305` → resolves to `Vitor Maia Rodovalho` (vitor@vitormr.dev) in members table (real human actor, FK OK)
