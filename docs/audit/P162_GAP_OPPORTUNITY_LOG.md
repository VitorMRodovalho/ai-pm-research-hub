...
- **Out of scope (carries forward):**
  - **#332 next dispatch** (W3 retroactive notification + Art. 18 §IV deletion offer for 1 affected candidate — [REDACTED-332-NAME] background pillar pre-block).
...
- **What:** Ships audit-log infrastructure to support PM-managed retroactive Art. 18 §IV notification of 1 pre-block transcribed candidate ([REDACTED-332-NAME], application `[REDACTED-332-APPLICATION_ID]`, background pillar video `[REDACTED-332-VIDEO_ID]`, transcription 2428 chars, Drive file `[REDACTED-332-DRIVE_FILE_ID]`). Issue #332 stays OPEN with status `ready-to-dispatch` — actual email dispatch is PM's manual step (would create false audit chain if I logged a "sent" timestamp before the email actually goes out).
...
- **Docs `docs/audit/lgpd-art11-remediation/notification_eduardo_luz_p238_interim.md` (NEW folder)**:
...
- **Live smoke 2/2 PASS** (via direct DO-block PERFORM):
  1. `lgpd_record_retroactive_notification(e780d8a9..., 'interim_v1', 'pt-BR', 'email')` from service-role context → raises "Unauthorized: no member record for caller" (correct — service-role has no member record).
  2. `lgpd_execute_retroactive_deletion(e780d8a9..., 6afb7e26..., 'CI smoke')` from service-role context → same Unauthorized error (gate fires before any data modification — transcription is NOT cleared by the smoke).
...
- **NO video/audio reprocessing**: per PM directive — the deletion path is offered, not pre-executed. Eduardo's transcription remains until he requests deletion or 30-day window passes (tacit retention; right to delete remains exercisable indefinitely).
...
- **Out of scope (carries forward — operator-level)**:
  1. PM dispatches email to [REDACTED-332-NAME] via nucleoia@pmigo.org.br using the interim_v1 template.
  ...
  4. If Eduardo responds requesting deletion: PM deletes Drive file `[REDACTED-332-DRIVE_FILE_ID]` as Workspace admin, captures confirmation URL/screenshot as `p_drive_deletion_ref`, calls `lgpd_execute_retroactive_deletion`, replies to Eduardo with the pii_access_log.id confirming deletion. Sibling C3 (#333 invariant U) ratchets cleanly post-deletion.
  5. If Eduardo does not respond within 30 days: PM may optionally log a `lgpd_art_18_retroactive_notification_tacit_close` follow-up call with template_version='interim_v1.tacit_close'. Right to deletion remains exercisable at any future moment.
...
- **Operator runbook (PM-facing)** for actually closing #332:
  1. PM dispatches the email to [REDACTED-332-NAME] via `nucleoia@pmigo.org.br` using the interim_v1 template from `docs/audit/lgpd-art11-remediation/notification_eduardo_luz_p238_interim.md`. Save the Gmail "sent" confirmation outside the platform for the private chain (Gmail Sent folder + optional PDF screenshot).
  2. From an authenticated MCP-Claude session, invoke the notification tool:
     ```
     lgpd_record_retroactive_notification(
       p_application_id: "[REDACTED-332-APPLICATION_ID]",
...
- **Post-hotfix PM retry succeeded**: `lgpd_record_retroactive_notification(p_application_id='[REDACTED-332-APPLICATION_ID]', p_template_version='interim_v1', p_lang='pt-BR', p_notification_method='email', p_dispatched_at='2026-05-24T00:29:00-03:00')` returned `{success:true, pii_access_log_id:"ee6a1541-bef6-4f7c-be69-c1e7efbc6568", application_id, template_version, lang, notification_method, dispatched_at:"2026-05-24T03:29:00+00:00", target_member_id:null}`. `target_member_id` NULL is correct because Eduardo is applicant-only (no members row yet).
- **Live audit row verified** post-dispatch via `SELECT FROM pii_access_log WHERE id = 'ee6a1541-...'`:
  ...
  * `reason` = `'template=interim_v1; lang=pt-BR; method=email; application_id=[REDACTED-332-APPLICATION_ID]; applicant_email=[REDACTED-332-EMAIL]'`
...
- **Email evidence**: PDF of Gmail "Sent" thread (1866042393540389608) saved by PM to org offline records under `LGPD/Art_18_remediation/eduardo_luz_p238/` — PII (Eduardo's email + UUIDs) deliberately NOT committed to repo. PDF metadata shows sent at "24 de maio de 2026 às 00:29" Brasília time, matching the `p_dispatched_at` passed to the RPC. PM made one minor body edit pre-send: "um vídeo de resposta (pilar Background)" → "os vídeos de resposta (pilar Background e as demais perguntas)" — factual clarification (Eduardo recorded all 5 pillars, only Background was transcribed pre-block); preserves `template_version='interim_v1'` semantics.
- **30-day window opens 2026-05-24**: if Eduardo responds requesting deletion within window, PM removes Drive file `[REDACTED-332-DRIVE_FILE_ID]` (Workspace admin) + invokes `lgpd_execute_retroactive_deletion` MCP tool (preview first → confirm:true) → writes `lgpd_art_18_deletion_executed` audit row with full `deletion_artifacts` jsonb → satisfies sibling C3 #333 invariant U precondition cleanly. If Eduardo does not respond by 2026-06-23, tacit retention applies; right to deletion remains exercisable indefinitely; #333 PM sequencing call defaults to path (b) 1-row allowlist.
...
- **Sibling status post-close**:
  ...
  * #333 — still OPEN, blocked on PM sequencing call (wait for Eduardo response vs immediate allowlist)
...
- **Carries forward**:
  ...
  4. **All p239b carries unchanged**: #333 W4 invariant U still blocked on PM sequencing call (wait Eduardo response 30d vs immediate allowlist); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
...
- **Carries forward (unchanged)**:
  ...
  3. **All p239b carries unchanged**: #333 W4 invariant U still blocked on PM sequencing call (wait Eduardo response 30d vs immediate allowlist); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
...
- **Carries forward**:
  ...
  4. **All p239b carries unchanged**: #333 W4 invariant U blocked on PM sequencing call (Eduardo 30d window opens 2026-05-24); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
