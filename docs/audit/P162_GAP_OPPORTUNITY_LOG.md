...
- **What:** Ships audit-log infrastructure to support PM-managed retroactive Art. 18 §IV notification of 1 pre-block transcribed candidate (Eduardo Luz, application `e780d8a9-55e0-4a6c-9370-4acc24a9619d`, background pillar video `6afb7e26-b806-4028-a8d5-0a22d1a0584b`, transcription 2428 chars, Drive file `14bA9rCezVD0Usko-S28ZtJnXd63MwsO6`). Issue #332 stays OPEN with status `ready-to-dispatch` — actual email dispatch is PM's manual step (would create false audit chain if I logged a "sent" timestamp before the email actually goes out).
...
- **Out of scope (carries forward — operator-level)**:
  1. PM dispatches email to Eduardo Luz via nucleoia@pmigo.org.br using the interim_v1 template.
  ...
  4. If Eduardo responds requesting deletion: PM deletes Drive file `14bA9rCezVD0Usko-S28ZtJnXd63MwsO6` as Workspace admin, captures confirmation URL/screenshot as `p_drive_deletion_ref`, calls `lgpd_execute_retroactive_deletion`, replies to Eduardo with the pii_access_log.id confirming deletion. Sibling C3 (#333 invariant U) ratchets cleanly post-deletion.
...
- **Live audit row verified** post-dispatch via `SELECT FROM pii_access_log WHERE id = 'ee6a1541-...'`:
  ...
  * `reason` = `'template=interim_v1; lang=pt-BR; method=email; application_id=e780d8a9-55e0-4a6c-9370-4acc24a9619d; applicant_email=eduardoluz.pm@gmail.com'`
