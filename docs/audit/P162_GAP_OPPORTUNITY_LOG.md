...
<!-- Items 79-81: landed 2026-06-10 via PR #202 refresh (council snapshot 2026-05-20, verbatim) -->
...
- **Trigger:** Council code-reviewer audit of PR #169. Initial seed had `contact_email='redacted-039@example.com'` in a public-repo migration. Fixed inline by removing the column from the INSERT (nullified live row). Raises broader question: what's the policy for `partner_entities.contact_email` in future seeds?
...
- **Smoke:** PM verified via MCP host (resolve + list + add all green, smoke row `7e96d90a-8b26-4ddd-9875-2aa44bb0cbbb` `redacted-087@example.com` kind=personal kept as legitimate alternate).
...
- **Trigger:** Após shipar PR #275 (OAuth allowlist subdomains) + PR #276 (execution.taskSupport strip), PM reconectou Perplexity → connector mostra "Connected · redacted-085@example.com" MAS tools tab ainda "No tools to display". Server-side tools/list HTTP 200 com 299 tools spec-compliant. Reject is happening INSIDE Perplexity client por motivo desconhecido.
...
- **Trigger:** PM ask mid-session — Welma é atual Diretora de Certificação @ PMI-GO (`redacted-089@example.com`), vai revisar + aprovar junto ao Ivan o TAP CPMAI Ciclo 3 (chain `897aeddf-a895-4f66-822a-b52fdccbf35c`). PM enfatizou: "quando feito via insert no banco corta os fluxos" → não basta INSERT direto, precisa também enviar magic link.
...
- **Out of scope (carries forward):**
  - **#332 next dispatch** (W3 retroactive notification + Art. 18 §IV deletion offer for 1 affected candidate — o candidato afetado (#332) background pillar pre-block).
...
### 206. RESOLVED-#332-INFRA — Wave 3 retroactive Art. 18 §IV notification + deletion audit-log infrastructure shipped (p238b)

- **When:** 2026-05-23 / 2026-05-24 p238b (single PR; dispatched as #2 next-up after p238 #331 close per PM-stated order).
- **What:** Ships audit-log infrastructure to support PM-managed retroactive Art. 18 §IV notification of 1 pre-block transcribed candidate (o candidato afetado (#332), application `[application_id redigido — #332]`, background pillar video `[video_screening_id redigido — #332]`, transcription 2428 chars, Drive file `[drive_file_id redigido — #332]`). Issue #332 stays OPEN with status `ready-to-dispatch` — actual email dispatch is PM's manual step (would create false audit chain if I logged a "sent" timestamp before the email actually goes out).
- **Migration `20260805000023_p238_332_lgpd_art18_retroactive_deletion_log.sql` (live applied + canonical registered)**:
  - `ALTER TABLE public.pii_access_log ADD COLUMN IF NOT EXISTS deletion_artifacts jsonb` (nullable; existing rows untouched; only deletion events populate it).
  - `CREATE OR REPLACE FUNCTION public.lgpd_record_retroactive_notification(p_application_id uuid, p_template_version text, p_lang text, p_notification_method text DEFAULT 'email', p_dispatched_at timestamptz DEFAULT NULL)` — SECDEF, pinned search_path, gates on `can_by_member(caller_member_id, 'manage_member')`. Validates p_template_version non-empty + p_lang non-empty + p_notification_method ∈ (email, whatsapp, in_person, other). Looks up target_member_id by lowering email (handles applicant→member transition cleanly). Inserts pii_access_log row with `context='lgpd_art_18_retroactive_notification'`, `fields_accessed=ARRAY['email']`, `reason=format('template=%s; lang=%s; method=%s; application_id=%s; applicant_email=%s', ...)`, `accessed_at=COALESCE(p_dispatched_at, now())` so PM can backdate to the actual send moment. Returns jsonb with pii_access_log_id (audit chain anchor).
  - `CREATE OR REPLACE FUNCTION public.lgpd_execute_retroactive_deletion(p_application_id uuid, p_video_id uuid, p_deletion_reason text, p_drive_deletion_ref text DEFAULT NULL)` — SECDEF, pinned search_path, gates on `can_by_member('manage_member')`. Validates `p_deletion_reason` length >= 8. Verifies video_id belongs to application_id (cross-app deletion guard). Rejects idempotent no-op (transcription IS NULL → raises to keep audit chain clean). Captures `old_transcription_len = length(v_vs.transcription)` BEFORE clearing. UPDATE pmi_video_screenings SET transcription = NULL. Inserts pii_access_log row with `context='lgpd_art_18_deletion_executed'`, `fields_accessed=ARRAY['transcription', 'drive_file_id', 'drive_file_name']`, and full `deletion_artifacts` jsonb containing: video_id, application_id, pillar, question_index, old_transcription_len, drive_file_id, drive_file_name, drive_deletion_ref (operator-supplied confirmation; e.g., Drive trash URL), deleted_at, deletion_reason, reversible=false, lgpd_basis='Art. 18 IV'. Does NOT touch `selection_evaluation_ai_suggestions` (those rows record `consumed_at=NULL` for non-consumed suggestions → inert audit history). Does NOT call Drive API — Drive file removal is PM's manual operator step (captured in p_drive_deletion_ref).
  - Sanity DO asserts: deletion_artifacts column exists with jsonb type + both RPCs have correct context literals + both have manage_member gate + deletion RPC writes deletion_artifacts + single-overload defense (SEDIMENT-232.A) for both RPCs.
- **Docs `docs/audit/lgpd-art11-remediation/notification_art18_interim_template.md` (NEW folder)**:
  - PM-approved interim notification template (template_version='interim_v1'). Status disclaimer: Angeline async produces legal-grade replacement under sibling #334.
  - Metadata header: destinatário, email, application_id, ciclo, status, affected video screening id + drive file id + transcription length, other pillars status, AI suggestion id (consumed_at=NULL = non-binding), consent state, Wave 1 emergency block reference, Wave 2 forward UI reference.
  - **Texto pt-BR (primary)**: full email subject + body. Explains the Art. 11 §I gap retrospectively, lists the 3 platform corrections (Wave 1 block, Wave 2 portal, this retroactive notice), presents the 2 candidate options (KEEP DATA or REQUEST DELETION via reply with sentinel "SOLICITO EXCLUSÃO LGPD ART 18"), commits to 30 days for deletion execution, states the candidate's application proceeds normally either way, includes tacit-retention rule post-30-days, signs from Vitor Maia Rodovalho with nucleoia@pmigo.org.br as reply-to. Footer references the affected uuids + p238 PR + LGPD article basis.
  - **English fallback (en-US)**: mirror translation, sentinel phrase "REQUEST LGPD ART 18 DELETION".
  - **Operational checklist (7 steps)**: PM review of template, dispatch via official email, call `lgpd_record_retroactive_notification` to anchor audit, save Gmail sent-confirmation outside platform (private chain), set 30-day reminder, if candidate requests deletion → PM deletes Drive file manually + calls `lgpd_execute_retroactive_deletion` with drive_deletion_ref, post-deletion confirms transcription IS NULL for the affected row (satisfies sibling C3 #333 invariant U precondition).
- **Contract test `tests/contracts/lgpd-art-18-retroactive-deletion.test.mjs` (20 assertions)**:
  - 11 static migration: file existence + ALTER COLUMN IF NOT EXISTS + both RPCs SECDEF + dual manage_member gate + notification context literal + deletion context + artifacts write + transcription clear + idempotent reject + cross-app validate + 7 required deletion_artifacts keys + sanity DO with dual-overload defense + NOTIFY pgrst.
  - 4 static docs: file exists + interim_v1 + affected uuids + pt-BR + en-US + Art 11 + Art 18 + RPC names in op checklist.
  - 2 forward-defense: no future migration DROPs deletion_artifacts column; no future migration redeclares either RPC without manage_member gate.
  - 3 DB-gated: live audit helper lists both RPC names; misuse call (service-role) to notification RPC returns "Unauthorized: no member record"; misuse call to deletion RPC returns same.
- **Test baseline:** offline **1879/1820/0/59 → 1899/1837/0/62** (+20 / +17 pass / +3 skip; matches expected delta).
- **Live smoke 2/2 PASS** (via direct DO-block PERFORM):
  1. `lgpd_record_retroactive_notification([application_id redigido — #332]..., 'interim_v1', 'pt-BR', 'email')` from service-role context → raises "Unauthorized: no member record for caller" (correct — service-role has no member record).
  2. `lgpd_execute_retroactive_deletion([application_id redigido — #332]..., 6afb7e26..., 'CI smoke')` from service-role context → same Unauthorized error (gate fires before any data modification — transcription is NOT cleared by the smoke).
- **NO live notification recorded**: PM dispatches the email manually + calls the RPC themselves to anchor the audit chain at the actual send moment. Logging a "sent" event before the email goes out would create a false audit chain.
- **NO video/audio reprocessing**: per PM directive — the deletion path is offered, not pre-executed. [candidato]'s transcription remains until he requests deletion or 30-day window passes (tacit retention; right to delete remains exercisable indefinitely).
- **Invariants:** 19/19 = 0 throughout (no FK changes, no constraint additions, only nullable column + 2 new RPCs).
- **SEDIMENT-238.A APPLIED CLEANLY**: apply_migration MCP created shadow row `20260524025931:p238_332_lgpd_art18_retroactive_deletion_log` at NOW(). This time I INSERTed canonical `20260805000023` WITH the `statements` column copied FROM the shadow row in a single SQL statement, then DELETEd the shadow. This is the recipe that emerged from p238 #331 close-review CI failure — no need for a second CI cycle this PR.
- **Out of scope (carries forward — operator-level)**:
  1. PM dispatches email to o candidato afetado (#332) via nucleoia@pmigo.org.br using the interim_v1 template.
  2. PM calls `lgpd_record_retroactive_notification` RPC with the actual `p_dispatched_at` to anchor the audit chain.
  3. PM closes #332 in a follow-up brief commit (registry row ratchet to `(closed)` + P162 close-evidence entry referencing the pii_access_log.id of the notification anchor).
  4. If [candidato] responds requesting deletion: PM deletes Drive file `[drive_file_id redigido — #332]` as Workspace admin, captures confirmation URL/screenshot as `p_drive_deletion_ref`, calls `lgpd_execute_retroactive_deletion`, replies to [candidato] with the pii_access_log.id confirming deletion. Sibling C3 (#333 invariant U) ratchets cleanly post-deletion.
  5. If [candidato] does not respond within 30 days: PM may optionally log a `lgpd_art_18_retroactive_notification_tacit_close` follow-up call with template_version='interim_v1.tacit_close'. Right to deletion remains exercisable at any future moment.
  6. Drive API integration for automated file deletion is intentionally NOT in this PR (1 row is manual-grade; if PMI scales to N retroactive deletions, a follow-up issue can wire pmi-video-init-upload-like EF for Drive admin auth + automated removal).
  7. Future enhancement: if a structured query over `deletion_artifacts` becomes needed (e.g., "all deletions for cycle X in pt-BR"), the jsonb column supports indexing; current usage is store-and-prove only.
- **Sediment learnings (0 NEW)**: SEDIMENT-238.A recipe re-applied cleanly without CI cycle this time. SEDIMENT-238.B avoided (no inline comments in function bodies this migration; if any had been present, I would have copy-pasted the file content verbatim into apply_migration).
- **PR:** governance/p238b-332-lgpd-art18-retroactive-deletion-log → main (standard CI gate expected, 0 bypass).
- **Verify-on-next-boot**: `pii_access_log` has `deletion_artifacts jsonb` column · `pg_proc.lgpd_record_retroactive_notification` has 5 args + only 1 overload · `pg_proc.lgpd_execute_retroactive_deletion` has 4 args + only 1 overload · both gate on can_by_member('manage_member') in body · invariants 19/19=0 · npm test 1899/1837/0/62 offline · migration `20260805000023` registered (canonical) with statements populated · no shadow rows at `20260524*` · `tests/contracts/lgpd-art-18-retroactive-deletion.test.mjs` present in `package.json` `test` + `test:contracts` scripts · `docs/audit/lgpd-art11-remediation/notification_art18_interim_template.md` present · `pii_access_log` row count grew by 0 from this PR (notification anchor row written by PM later, not by this PR).
- **Cross-ref:** GH #332 (this leaf, OPEN with status `ready-to-dispatch`) · GH #331 (sibling shipped p238) · GH #221 + #218 (parent umbrella, decomposed p236; P162 #203 line 2174; P162 #205 #331 close line ~2253+) · GH #333 (sibling — depends on [candidato]'s deletion completing for the affected row to allow invariant U to land green; if [candidato] opts for retention, #333 needs the 1-row allowlist branch instead) · GH #334 (sibling — Angeline produces legal-grade template that supersedes interim_v1) · `supabase/migrations/20260805000023_p238_332_lgpd_art18_retroactive_deletion_log.sql` · `docs/audit/lgpd-art11-remediation/notification_art18_interim_template.md` · `pmi_video_screenings.id=[video_screening_id redigido — #332]` (the affected row) · `selection_applications.id=[application_id redigido — #332]` (o candidato afetado (#332)) · LGPD Art. 11 §I (sensitive data) · Art. 18 §IV (deletion right) · Art. 9 (informed processing) · Art. 48 §1 (ANPD pre-disclosure — coordinated under sibling #334) · ADR-0007 (V4 authority via can_by_member, used as gate here).
...
- **Operator runbook (PM-facing)** for actually closing #332:
  1. PM dispatches the email to o candidato afetado (#332) via `nucleoia@pmigo.org.br` using the interim_v1 template from `docs/audit/lgpd-art11-remediation/notification_art18_interim_template.md`. Save the Gmail "sent" confirmation outside the platform for the private chain (Gmail Sent folder + optional PDF screenshot).
  2. From an authenticated MCP-Claude session, invoke the notification tool:
     ```
     lgpd_record_retroactive_notification(
       p_application_id: "[application_id redigido — #332]",
  ...
  4. If [candidato] replies requesting deletion (sentinel "SOLICITO EXCLUSÃO LGPD ART 18" or equivalent): PM removes Drive file `[drive_file_id redigido — #332]` as Workspace admin (captures Trash URL or audit message-id), then invokes the deletion tool in preview mode first (omit `confirm`, inspect the preview envelope for `cross_app_check.matches: true`), then re-invokes with `confirm: true` + `p_drive_deletion_ref: "<trash URL or audit ref>"`. Replies to [candidato] confirming deletion with the `pii_access_log_id` from the deletion call. Sibling C3 (#333 invariant U) ratchets cleanly post-deletion.
  5. If [candidato] does not respond within 30 days: PM may optionally log a tacit-close follow-up via `lgpd_record_retroactive_notification(..., p_template_version: "interim_v1.tacit_close")`. Right to deletion remains exercisable at any future moment.
- **Out of scope (carries forward)**:
...
### Close evidence (added 2026-05-24, p239b ratchet)

- **First PM-authenticated dispatch attempt FAILED with FK violation**: `Error: insert or update on table "pii_access_log" violates foreign key constraint "pii_access_log_accessor_id_fkey"`. Diagnosis (independently identified by PM via web Claude before this session inspected pg_constraint): the p238b RPCs INSERT with `accessor_id := auth.uid()`, but the FK targets `members(id)` not `auth.users(id)`. Both RPCs already had `v_caller_member_id` declared at the top of the body (used for the gate check) — they just forgot to use it in the INSERT.
- **Hotfix shipped**: migration `20260805000024_p239b_332_hotfix_accessor_id_fk.sql` (single 4-character change per RPC — `auth.uid()` → `v_caller_member_id` in the INSERT VALUES; CREATE OR REPLACE preserves signature; sanity DO re-asserts the post-fix pattern + dual-overload defense). Live applied + SEDIMENT-238.A shadow row `20260524044033` → canonical `20260805000024` via INSERT WITH statements copied + DELETE in single SQL.
- **Audited other 4 `pii_access_log` writers** (`_log_application_pii_access`, `log_pii_access`, `log_pii_access_batch`, `list_initiative_engagements_by_kind`) — ALL correctly source `accessor_id` from `members.id` via either `v_caller_member_id` local OR explicit parameter. Pattern matches canonical helper `public.log_pii_access` exactly. The 2 p238b RPCs were the only offenders.
- **9 forward-defense contract assertions** in new `tests/contracts/p239b-332-hotfix-accessor-id-fk.test.mjs`: hotfix file existence + signature preservation × 2 + INSERT VALUES starts with v_caller_member_id × 2 + no auth.uid() in any pii_access_log INSERT block + sanity DO body assertion + COMMENT provenance + forward-defense (no future migration redeclares either RPC with auth.uid() inside INSERT INTO pii_access_log VALUES).
- **SEDIMENT-239b.A LEARNING**: contract tests for SECDEF RPCs that INSERT into FK-constrained tables MUST assert the source of every FK column, not just the gate ladder presence + context literal + dual-overload defense. The p238b test (`lgpd-art-18-retroactive-deletion.test.mjs`) asserted all of those but missed the INSERT column source — would have caught this class at p238b CI if it had. Added to checklist for future LGPD audit-row RPCs.
- **Test baseline**: offline **1916/1854/0/62 → 1925/1863/0/62** (+9 / +9 pass / 0 skip — all from the new hotfix test file).
- **Invariants**: 19/19 = 0 throughout (no schema changes, only function body rewrites).
- **Post-hotfix PM retry succeeded**: `lgpd_record_retroactive_notification(p_application_id='[application_id redigido — #332]', p_template_version='interim_v1', p_lang='pt-BR', p_notification_method='email', p_dispatched_at='2026-05-24T00:29:00-03:00')` returned `{success:true, pii_access_log_id:"ee6a1541-bef6-4f7c-be69-c1e7efbc6568", application_id, template_version, lang, notification_method, dispatched_at:"2026-05-24T03:29:00+00:00", target_member_id:null}`. `target_member_id` NULL is correct because [candidato] is applicant-only (no members row yet).
- **Live audit row verified** post-dispatch via `SELECT FROM pii_access_log WHERE id = 'ee6a1541-...'`:
  * `accessor_id` = `880f736c-3e76-4df4-9375-33575c190305` → resolves to `Vitor Maia Rodovalho` (redacted-086@example.com) in members table (real human actor, FK OK)
  * `target_member_id` = NULL (applicant pre-conversion, correct)
  * `fields_accessed` = `['email']`
  * `context` = `'lgpd_art_18_retroactive_notification'`
  * `reason` = `'template=interim_v1; lang=pt-BR; method=email; application_id=[application_id redigido — #332]; applicant_email=[e-mail do candidato redigido — #332]'`
  * `accessed_at` = `2026-05-24 03:29:00+00` (= 00:29 BRT real send moment — backdating worked)
  * `deletion_artifacts` = NULL (not a deletion event)
- **Email evidence**: PDF of Gmail "Sent" thread (1866042393540389608) saved by PM to org offline records under `LGPD/Art_18_remediation/art18-332-candidate_p238/` — PII ([candidato]'s email + UUIDs) deliberately NOT committed to repo. PDF metadata shows sent at "24 de maio de 2026 às 00:29" Brasília time, matching the `p_dispatched_at` passed to the RPC. PM made one minor body edit pre-send: "um vídeo de resposta (pilar Background)" → "os vídeos de resposta (pilar Background e as demais perguntas)" — factual clarification ([candidato] recorded all 5 pillars, only Background was transcribed pre-block); preserves `template_version='interim_v1'` semantics.
- **30-day window opens 2026-05-24**: if [candidato] responds requesting deletion within window, PM removes Drive file `[drive_file_id redigido — #332]` (Workspace admin) + invokes `lgpd_execute_retroactive_deletion` MCP tool (preview first → confirm:true) → writes `lgpd_art_18_deletion_executed` audit row with full `deletion_artifacts` jsonb → satisfies sibling C3 #333 invariant U precondition cleanly. If [candidato] does not respond by 2026-06-23, tacit retention applies; right to deletion remains exercisable indefinitely; #333 PM sequencing call defaults to path (b) 1-row allowlist.
- **#332 CLOSED** via PR #342 squash-merge 2026-05-24 (PR description uses `Closes #332` keyword — intentional/desired auto-close because the genuine close criteria are satisfied; SEDIMENT-235.A still respected because the discipline is about avoiding the keyword for issues that should STAY open, not for issues that genuinely close).
- **Sibling status post-close**:
  * #331 — closed p238 (W2 forward UI)
  * #332 — closed p239b (this entry) ✓
  * #333 — still OPEN, blocked on PM sequencing call (wait for [candidato] response vs immediate allowlist)
  * #334 — still OPEN, blocked external (Angeline async)
  * #335 — still OPEN, blocked on C1/C2/C3 closes (C4 NOT required); C1+C2 now both closed, C3 (#333) is the remaining blocker
- **Cross-ref additions**: hotfix migration `supabase/migrations/20260805000024_p239b_332_hotfix_accessor_id_fk.sql` · hotfix contract test `tests/contracts/p239b-332-hotfix-accessor-id-fk.test.mjs` · PR #342 (3 commits squash) · `pii_access_log.id = ee6a1541-bef6-4f7c-be69-c1e7efbc6568` (the canonical chain anchor for this dispatch).

---

### 208. RESOLVED-#251 — Selection interview status transition gap fixed via trigger + 14-row backfill (p240)

- **Date**: 2026-05-24 (p240 close)
- **Status**: RESOLVED — #251 CLOSED via this PR (PR #TBD on merge)
- **Origin**: PM reproduced 2026-05-24 in `/admin/selection` cycle4-2026 view. Luíse Quintana (`4943a94e`) + William Junio researcher (`6187b0b2`) labelled `Aguardando Entrevista` despite both having a `selection_interviews` row with `status='completed'` + `conducted_at` set + a submitted `selection_evaluations(evaluation_type='interview', submitted_at IS NOT NULL)`. p226 audit (Henrique + Fabricio framing) did not surface this manifestation; PM's reframe found the live mismatch directly.
- **Root cause** (3 silent gates fragment status-advance for `selection_interviews → selection_applications.status`):
  1. **Calendar webhook (#116 path)**: inserts `selection_interviews` rows directly without touching `selection_applications.status`. Webhook service_role bypasses `schedule_interview()` RPC entirely (WATCH-116.A documented this for `gate_attempts` visibility, but did not flag the status-sync gap).
  2. **`mark_interview_status('completed')` precondition**: `UPDATE selection_applications SET status='interview_done' WHERE id=NEW.application_id AND status IN ('interview_scheduled','interview_done')` silently no-ops when source status is `interview_pending`. The PM operational flow is "click 'completed' in /admin/selection interview tab" → fires this RPC → no app-status advance.
  3. **`submit_interview_scores()` interviewer-array gate**: only sets `selection_applications.status='final_eval'` when `v_all_interviewers_submitted` is true (i.e., every uuid in `selection_interviews.interviewer_ids` array has submitted an interview eval). Cycle4 default is Vitor + Fabricio as evaluators with only Vitor submitting → gate is false → no advance. Same RPC also gates the `selection_interviews.status='completed'` UPDATE on the same condition, but `mark_interview_status('completed')` runs that independently — so the interview row reaches `completed` while the app stays `interview_pending`.
- **Impact at audit time (cycle4-2026, 38 apps)**:
  - 10 apps with submitted interview eval but app status NOT in {interview_done, final_eval, approved, rejected, …} ("evaluated but stale")
  - 10 apps with conducted interview but same stale-status family
  - 11 apps with `interview_rows` but app status still `screening` (never advanced past objective)
  - 2 in `interview_pending` with conducted interview + submitted eval (William, Luíse — primary PM-reported cases)
  - 1 in `interview_pending` with scheduled-not-yet-conducted interview (Rafael Bellotti — expected `interview_scheduled`)
- **Fix shipped**:
  - Migration `20260805000025_p240_251_interview_status_transition_trigger.sql`:
    1. Trigger function `public._trg_sync_interview_to_app_status()` — SECDEF + `SET search_path TO 'public','pg_temp'` + REVOKE EXECUTE FROM PUBLIC.
    2. Trigger `trg_sync_interview_to_app_status` AFTER INSERT OR UPDATE OF (status, conducted_at) ON `public.selection_interviews` FOR EACH ROW.
    3. Terminal-status no-op guard: if `v_app_status IS NULL OR v_app_status IN ('approved','rejected','converted','withdrawn','cancelled','waitlist','final_eval')` → RETURN NEW. Per PM directive 2026-05-24 ("trigger nunca toca terminal").
    4. Evidence ladder (mirrors backfill ladder): conducted_at set OR status='completed' → UPDATE app to `interview_done` WHERE status IN ('screening','objective_eval','objective_cutoff','interview_pending','interview_scheduled'); status IN ('scheduled','rescheduled') → UPDATE app to `interview_scheduled` WHERE status IN ('screening','objective_eval','objective_cutoff','interview_pending') (narrower — excludes `interview_scheduled`/`interview_done` to prevent regressing on follow-up rows).
    5. Backfill scope: cycle4-2026 + cycle3-2026-b2 (PM-approved 2026-05-24; both cycles in phase='evaluating'). CTE applies the same evidence ladder + idempotency guard (`f.target_status <> a.status`).
    6. Audit trail: 14 rows in `admin_audit_log` (10 cycle4 + 4 cycle3-b2) with `action='p240_251_backfill_interview_status'` + `target_type='selection_application'` + `metadata.reason='p240_251_interview_status_transition_backfill'` + `metadata.migration='20260805000025'` + `metadata.new_status` + `metadata.cycle_code`.
    7. Sanity DO block: `RAISE EXCEPTION` if any row in scope still has `status='interview_pending'` AND has a submitted interview eval — fails loud at apply time.
    8. `NOTIFY pgrst, 'reload schema'`.
  - Contract test `tests/contracts/p240-251-interview-status-transition-trigger.test.mjs` (11 static + 2 DB-gated). Static asserts: migration file present + SECDEF/search_path on trigger fn + AFTER INSERT OR UPDATE OF narrow-scope trigger declaration + all 7 terminal-status guards present + conducted/completed branch preconditions + scheduled/rescheduled branch preconditions + REVOKE EXECUTE FROM PUBLIC + backfill scope cycle4+cycle3-b2 (with CTE evidence ladder mirror) + audit canonical action + admin_audit_log shape (actor_id/target_type — NOT actor_member_id/target_kind which don't exist on this table; learned during apply when first migration attempt failed atomic-rollback) + sanity DO RAISES + NOTIFY pgrst.
  - Package.json `npm test` + `test:contracts` whitelist updated (SEDIMENT-186.C class — script is hardcoded, not glob-based; new files must be added explicitly).
  - Test baseline: **1925/1863/0/62 → 1938/1874/0/64** offline (+13 tests / +11 pass / +2 skip).
- **Live smoke (post-apply)**: trigger registered (2 rows in information_schema.triggers — one per INSERT + UPDATE event manipulation) · Luíse → `interview_done` · William researcher → `interview_done` · Rafael Bellotti → `interview_scheduled` · William *leader* preserved as `rejected` (terminal guard fires) · cycle4 stale-count = 0 · cycle3-b2 stale-count = 0 · audit_rows_written = 14 · invariants 19/19 = 0.
- **Cycle4 cross-tab post-fix**:
  - interview_done: 10 (was 0)
  - interview_scheduled: 4 (was 0)
  - interview_pending: 13 (was 16; 3 advanced; the 13 remaining are apps with NO interview activity yet)
  - screening: 6 researchers (was 17; 11 advanced) + 2 leaders (unchanged)
  - objective_cutoff: 1 (unchanged)
  - rejected: 2 leaders (unchanged)
  - Total: 38 apps (unchanged; math closes)
- **What was NOT touched this session** (deferred per PM directive "trigger primeiro, RPC depois"):
  - `submit_interview_scores()` interviewer-array gate (`v_all_interviewers_submitted`) — trigger now blinds this, but the RPC body still gates `selection_interviews.status='completed'` on all-submitted; harmless since `mark_interview_status` is the actual path used in PM operational flow.
  - `mark_interview_status()` precondition narrowing — trigger now blinds this too, but the RPC body still gates `selection_applications.status` UPDATE on `status IN ('interview_scheduled','interview_done')`; harmless because trigger handles the advance from `interview_pending` directly.
  - `schedule_interview()` RPC — already correctly advances to `interview_scheduled`; trigger duplicates the work safely (UPDATE `WHERE id=NEW.application_id AND status IN (...)` is idempotent when target=current).
- **Sediment learnings (1 NEW)**:
  - **SEDIMENT-186.C confirmation** (NOT new — already documented as "manual ratchet" backlog): `npm test` script is a hardcoded whitelist of test file paths in `package.json`. New contract test files MUST be added to BOTH `"test"` and `"test:contracts"` scripts. Initial `npm test` run this session reported 1925/1863/0/62 (unchanged from p239b baseline) because the new file was created but not added to the whitelist — only when both edits landed did the count bump to 1938/1874/0/64. Add to pre-commit checklist for any session that introduces a new test file.
  - **SEDIMENT-240.A NEW (admin_audit_log shape)**: `admin_audit_log` uses `actor_id` (not `actor_member_id`) + `target_type` (not `target_kind`). First apply_migration attempt this session failed atomic-rollback on `INSERT INTO admin_audit_log (action, actor_member_id, target_kind, ...)`. Hot-fixed in place before second apply. Other audit tables (`pii_access_log` per p239b) have different column names — always probe `information_schema.columns WHERE table_name=...` before writing INSERTs to audit tables.
- **Sediment NOT triggered this session** (forward-defense respected):
  - SEDIMENT-235.A: PR title uses `Closes #251` intentionally — the issue is genuinely closing, so the auto-close keyword is the desired behavior. SEDIMENT-235.A is about avoiding the keyword for issues that should STAY open, not for genuine closes.
  - SEDIMENT-238.A: `supabase migration repair --status applied 20260805000025` registered the migration with `statements` text[] column populated (8 stmt_count verified). No shadow row + no INSERT path was used.
  - SEDIMENT-238.B: live `pg_proc.prosrc` for `_trg_sync_interview_to_app_status` matches the local file byte-for-byte (apply_migration was the only path; no in-place body edit).
- **Carries forward**:
  1. **WATCH-240.A** (defense-in-depth for `submit_interview_scores`): consider relaxing the `v_all_interviewers_submitted` gate so a single submitting evaluator (when 2 are seeded) also advances the app to `interview_done` (or `final_eval` if it's the gate's actual goal). Trigger already blinds this for the status-advance dimension, but the `selection_interviews.status='completed'` flip would still depend on all-submitted via this RPC. Low priority; PM operational flow uses `mark_interview_status` explicitly.
  2. **WATCH-240.B** (cycle3-2026-b2 close audit): backfill scope included cycle3-b2 because it's also `phase='evaluating'`. The 4 affected cycle3-b2 rows now show `interview_done` — PM should verify these are the right candidates next session (could be valid late-evaluation, could be backfill catching a long-tail). Not blocking.
  3. **WATCH-240.C** (PERT cutoff + finalize_decisions gap): 11 cycle4 apps advanced from `screening` directly to `interview_scheduled` or `interview_done` via the backfill (skipping `objective_eval`/`objective_cutoff`/`interview_pending`). They had `objective_score_avg` set (range 93.5–184.5) but were never finalized through the PERT cutoff workflow. Separate cluster issue (p230 fast-follow carry already exists for the cron schedule + threshold evaluation of `selection_cutoff_approved` — RESOLVED-179 entry #196).
  4. **All p239b carries unchanged**: #333 W4 invariant U still blocked on PM sequencing call (wait [candidato] response 30d vs immediate allowlist); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
- **PR**: `governance/p240-251-interview-status-transition` → main (standard CI gate path, 0 bypass).
- **Verify-on-next-boot**: HEAD includes PR #TBD · `supabase/migrations/20260805000025_p240_251_interview_status_transition_trigger.sql` present · `tests/contracts/p240-251-interview-status-transition-trigger.test.mjs` present · package.json whitelist contains new test in both `"test"` + `"test:contracts"` · npm test 1938/1874/0/64 offline · invariants 19/19=0 · `pg_trigger` has `trg_sync_interview_to_app_status` (INSERT + UPDATE rows in information_schema.triggers) · `pg_proc._trg_sync_interview_to_app_status` exists with SECDEF + search_path pinned · `admin_audit_log` has 14 rows with `action='p240_251_backfill_interview_status'` · cycle4-2026 + cycle3-2026-b2 have 0 apps in `interview_pending` with a submitted interview eval · GH #251 state=CLOSED stateReason=COMPLETED closedAt=2026-05-24T0[3-9]:* · ISSUE_REGISTRY "Selection reliability Cycle 4" cluster narrative mentions `p240 #251 closed`.
- **Cross-ref**: GH #251 · p226 audit (precursor — close-candidate disposition before PM re-reproduction): docs/audit/CYCLE4_TRUST_AUDIT_P226.md · sibling trigger `trg_sync_interview_to_event` (selection_interviews → events; independent scope, unaffected) · RPCs analyzed: `schedule_interview()`, `mark_interview_status()`, `submit_interview_scores()`, `submit_evaluation()` · calendar webhook entry point #116 (RESOLVED-116 entry #195) · selection cluster handoff thread (p228 W2 7-leaf shipping + p229 #116 close + p230 #179 close + p232 #229 close + p233-235 #230 children close + p251 p240 close).

---

### 209. RESOLVED-WATCH-240.A — submit_interview_scores hoists conducted_at out of the all-submitted branch (p241)

- **Date**: 2026-05-24 (p241)
- **Status**: RESOLVED — WATCH-240.A defense-in-depth shipped (PR #TBD on merge)
- **Origin**: WATCH-240.A carry from p240 #251 close. The p240 trigger `trg_sync_interview_to_app_status` keys on changes to `selection_interviews.conducted_at` OR `.status` and is the canonical owner of `selection_applications.status` sync to `interview_done`. But pre-WATCH-240.A, `submit_interview_scores()` only set `conducted_at = now()` INSIDE the `IF v_all_interviewers_submitted` branch (step 8 of the body). A partial submission (1 of N interviewers) never set `conducted_at` → the p240 trigger never fired → app stuck in `interview_pending`. p240 backfill healed the past cycle4 + cycle3-b2 rows but the residual fragility remained for any NEW interview where an evaluator never finalizes (cycle4 default seeds Vitor + Fabricio; if Fabricio never submits, the gap returns).
- **Fix shipped**:
  - Migration `20260805000026_p241_watch_240_a_submit_interview_scores_relax_status_gate.sql`:
    1. `CREATE OR REPLACE FUNCTION public.submit_interview_scores(uuid, jsonb, text, text, jsonb) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','pg_temp'` — same signature as p113 (no DROP+CREATE — preserves consumers).
    2. Hoist new step 7 `IF v_interview.conducted_at IS NULL THEN UPDATE public.selection_interviews SET conducted_at = now() WHERE id = p_interview_id; END IF;` BEFORE the all-submitted check (step 8). Idempotency guard on `v_interview.conducted_at IS NULL` (the record snapshot from step 2) prevents overwriting an earlier `conducted_at` set by `mark_interview_status('completed')` or a previous `submit_interview_scores` call from a different evaluator.
    3. In the all-submitted branch (renumbered step 9), the `UPDATE selection_interviews SET status = 'completed'` no longer carries `conducted_at = now()` — already set in step 7. Sole responsibility of this branch is sealing the interview row + computing PERT + UPDATEing `selection_applications.status='final_eval'` + dispatching the lead-committee notification.
    4. `NOTIFY pgrst, 'reload schema'`.
  - Contract test `tests/contracts/p241-watch-240-a-submit-interview-scores-relax.test.mjs` (15 tests: 11 static + 2 forward-defense + 2 DB-gated). Static asserts cover: file presence + 5-arg signature preserved (CREATE OR REPLACE, not DROP+CREATE) + SECDEF + pinned search_path + hoisted block has correct shape + hoisted block precedes all-submitted check (chronological in body) + all-submitted boolean still computed via NOT EXISTS pattern + all-submitted branch UPDATEs interview status='completed' + PERT formula preserved + final_eval transition preserved + header cross-refs (WATCH-240.A + p240 trigger 20260805000025 + p113 origin) + notification path preserved + return envelope preserved + NOTIFY pgrst. Forward-defense (2): regression catch that `conducted_at = now()` is NOT re-introduced inside the all-submitted branch + UPDATE inside the branch is status-only (no other columns sneak in to re-couple). DB-gated (2): live body hoist position precedes all-submitted check + single overload (signature stability proxy). DB-gated tests degrade gracefully if helper RPC `_audit_list_public_function_bodies()` is not exposed (skip rather than fail for tooling gap).
  - Package.json `npm test` + `test:contracts` whitelist updated (SEDIMENT-186.C class — new file added to both hardcoded lists).
  - Test baseline: **1938/1874/0/64 → 1953/1887/0/66** offline (+15 tests / +13 pass / +2 skip).
- **Live smoke (post-apply)**:
  - `pg_proc.submit_interview_scores.prosrc` length 6522 (was 5288 pre-WATCH; +1234 chars from new comment block + hoisted UPDATE).
  - Body-hash drift check (Phase C): live `md5(regexp_replace(prosrc,'\s+',' ','g')) = f9cf4bde13a3145f6702d5543661cf79` matches migration file body md5 byte-for-byte (no SEDIMENT-238.B drift).
  - Hoist position 3525 < all-submitted assign position 3722 < all-submitted branch 4470 < UPDATE status='completed' 4554 < UPDATE status='final_eval' 5437 (chronological ordering confirmed).
  - Forward-defense scan: regex check confirms NO `conducted_at = now()` substring inside the all-submitted branch.
  - Single overload: `count(*) FROM pg_proc WHERE proname='submit_interview_scores' AND pronamespace='public'::regnamespace = 1`.
  - Direct invocation: `SELECT public.submit_interview_scores('0000…'::uuid, '{}'::jsonb, NULL, NULL, '{}'::jsonb)` returned `ERROR P0001: Unauthorized: member not found CONTEXT: PL/pgSQL function submit_interview_scores(uuid,jsonb,text,text,jsonb) line 24 at RAISE` — auth gate fires at line 24 (members lookup), signature `(uuid,jsonb,text,text,jsonb)` reported by Postgres matches the 5-arg shape, body parses and executes.
  - Invariants 19/19 = 0.
- **What was NOT touched this session** (deferred per WATCH-240 scope):
  - `mark_interview_status('completed')` precondition still gates on `status IN ('interview_scheduled','interview_done')` — trigger blinds this for new app-status advance, the RPC body left as-is.
  - `submit_interview_scores` `v_all_interviewers_submitted` gate still governs the `selection_interviews.status='completed'` flip + PERT + `final_eval` transition. WATCH-240.A specifically relaxed only the `conducted_at` side-effect; advancing to `final_eval` on a single submission would be a SEMANTIC change (final_eval = ready for committee decision = requires all interview data) and was NOT in scope per PM directive ("manter escopo pequeno").
- **Sediment learnings (0 NEW)**:
  - SEDIMENT-238.A applied cleanly: apply_migration created shadow row at `20260524150542` (= timestamp of MCP call); canonical row `20260805000026` was INSERTed via `WITH shadow AS (SELECT statements FROM … WHERE version='20260524150542') INSERT … SELECT FROM shadow` then `DELETE FROM … WHERE version='20260524150542'`. Statements column populated (8653 chars, 1 statement element containing the full SQL — the MCP doesn't split on `;`).
  - SEDIMENT-238.B respected: live `prosrc` matches the migration file body byte-for-byte via md5-of-normalized-whitespace.
  - SEDIMENT-186.C respected: new test file added to BOTH `"test"` and `"test:contracts"` whitelists in `package.json` before running `npm test`. First run reported 1953/1887/0/66 immediately (no second-attempt needed since the lesson from p240 carried into this session's setup).
  - SEDIMENT-235.A respected: PR title uses `Closes WATCH-240.A` (internal tag, not a GitHub issue) — no auto-close keyword on any open GH issue.
- **Carries forward (unchanged)**:
  1. **WATCH-240.B** (cycle3-2026-b2 close audit): backfill scope included cycle3-b2 because it's also `phase='evaluating'`. The 4 affected cycle3-b2 rows now show `interview_done` — PM should verify these are the right candidates next session.
  2. **WATCH-240.C** (PERT cutoff + finalize_decisions gap): 11 cycle4 apps advanced from `screening` directly to `interview_scheduled` or `interview_done` via the p240 backfill (skipping `objective_eval`/`objective_cutoff`/`interview_pending`). Separate cluster issue (p230 fast-follow carry already exists for the cron schedule + threshold evaluation of `selection_cutoff_approved`).
  3. **All p239b carries unchanged**: #333 W4 invariant U still blocked on PM sequencing call (wait [candidato] response 30d vs immediate allowlist); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
...
- **PM action items** (deferred to committee at next meeting):
  1. **Per-app cutoff dispatch (audit-corrected workflow)**: For each of the 5 above-target apps in cycle4 screening, run `SELECT public.notify_selection_cutoff_approved('<app_id>'::uuid)` from authenticated MCP-Claude session (gate: committee lead OR `manage_member`; Vitor satisfies the latter as platform admin). RPC is idempotent (no-op if `cutoff_approved_email_sent_at` already set), dispatches the booking email via `campaign_send_one_off` → Resend, and writes `admin_audit_log` row `'selection.cutoff_approved_email_dispatched'`. Status advance happens *naturally* afterward when the candidate books via the calendar webhook (#116) → `schedule_interview` → p240 trigger advances app `screening → interview_scheduled`. **NOT** `finalize_decisions` (that's for FINAL committee decisions `approved/rejected/waitlisted/converted` AFTER interviews — different stage of the funnel).
  2. **2 below-band apps** (Alexandre Fortes 119.50, Carla Rosa 117.50): no canonical `screening → objective_cutoff` RPC exists in current codebase (`'objective_done'` is referenced only as a variable name `v_objective_done int` inside `notify_selection_cutoff_approved`, never written as a status enum value). PM picks (a) leave them in `screening` until committee runs `finalize_decisions` with `decision='rejected'` after broader cohort review, OR (b) manual UPDATE via authenticated session with audit row.
  3. **1 in-band borderline app** (Hector Rigon 140.50): committee judges in next meeting.
  4. **Pre-dispatch check**: confirm `cycle4-2026.interview_booking_url` is set (otherwise `notify_selection_cutoff_approved` raises `CUTOFF_NO_BOOKING_URL` errcode P0020): `SELECT cycle_code, interview_booking_url FROM public.selection_cycles WHERE cycle_code='cycle4-2026';`.
  5. **Cycle3-b2 minor**: verify the 2 cycle3-b2 apps still in scope are the late-evaluation candidates from p240 backfill (`WATCH-240.B` parallel carry).
...
- **Carries forward (open follow-ups)**:
  ...
  5. **All p239b carries unchanged**: #333 W4 invariant U still blocked on PM sequencing call (wait [candidato] response 30d vs immediate allowlist); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
...
### 211. RESOLVED-WATCH-240.B — cycle3-2026-b2 late-eval audit closes as audit-only no remediation (p244)

- **Date**: 2026-05-24 (p244)
- **Status**: RESOLVED — **audit-only, no remediation needed**. WATCH-240.B closes.
- **Origin**: P162 #208 carry — *"backfill scope included cycle3-b2 because it's also `phase='evaluating'`. The 4 affected cycle3-b2 rows now show `interview_done` — PM should verify these are the right candidates next session (could be valid late-evaluation, could be backfill catching a long-tail). Not blocking."* Carried through p241/p242/p243 handoff text. p244 boot picked Option A "WATCH-240.B + cycle3-b2 dispatch packet"; mid-session live audit forced a scope redirect to "audit-only no dispatch" (Option A1) after data revealed both cycle3-b2 apps were already `approved` (dispatch moot).
- **Audit findings (live, 2026-05-24 ≈17:00 UTC)**:
  1. **The p240 backfill touched 0 (zero) cycle3-2026-b2 rows.** Live `admin_audit_log WHERE action='p240_251_backfill_interview_status' GROUP BY metadata->>'cycle_code'` returns 14 rows all `cycle4-2026` (10 → `interview_done` + 4 → `interview_scheduled`). The original P162 #208 line "10 cycle4 + 4 cycle3-b2" was an attribution error that propagated through 4 handoffs without live verification. The migration `20260805000025` CTE scope DOES include cycle3-b2 (line "Backfill scope: cycle4-2026 + cycle3-2026-b2 (PM-approved 2026-05-24)"), but the idempotency guard `f.target_status <> a.status` filtered cycle3-b2 to 0 candidates because both cycle3-b2 apps were already past `interview_done` (`status='approved'`) by the time the trigger landed.
  2. **cycle3-2026-b2 has exactly 2 applications, both already `approved`.** Herlon Alves de Sousa (`d1d72a91…`, leader, `objective_score_avg=252.00`) + João Uzejka dos Santos (`a06b9a26…`, researcher, `objective_score_avg=137.00`). Both advanced via `application.status_manual_advance` on 2026-05-09 (p129-s1 session) per PM directives *"extend formal acceptance pre-Cycle 4 — already running CPMAI study group operationally"* (Herlon) and *"João is cycle 3 active researcher (test journey user) — manual advance unblocks UI interview tab + onboarding workflow test"* (João U.).
  3. **"4 late-evaluation" reframed**: not the original "4 cycle3-b2 backfilled apps". The closest live pattern is **Herlon's 4 non-interview evaluation rows** submitted 2.4–3.2 days post-interview. Breakdown: 1 leader_extra by Vitor (-16s pre, basically synchronous) + 1 leader_extra by Fabricio (+2.4d) + 1 objective by Fabricio (+2.4d) + 1 objective by Vitor (+3.2d). Interview itself (Vitor, synchronous with `conducted_at`) is canonical. All 4 non-interview evals are legitimate PM/curator post-hoc submissions in the administrative onboarding window.
  4. **No structural remediation needed.** cycle3-b2's standard funnel ordering (objective → screening → interview → leader_extra → decision) does not apply because both apps bypassed the funnel via `application.status_manual_advance`. The "out-of-canonical-phase" evals are an artifact of administrative path, not a system bug.
- **What was NOT touched (per Option A1 scope)**:
  - No migrations, no schema changes, no code changes.
  - No dispatch packet — both apps already `approved`, `notify_selection_cutoff_approved` would be no-op (and they bypassed the canonical workflow anyway).
  - No `cycle3-b2.interview_booking_url` population — not needed for administrative path (both interviews already conducted).
- **Sediment learnings (1 NEW)**:
  - **SEDIMENT-244.A NEW**: handoff narrative propagation drift — when handoff carry text claims "N rows in cycle/table X", live-query the source-of-truth table BEFORE planning dispatch work around it. The check is cheap (1 SELECT) and prevents wasted scope (drafting a runbook for non-existent work). Mirrors SEDIMENT-242.A (probe RPC body before runbook) in the handoff-text domain. Specifically for cycle-level counts: `admin_audit_log WHERE action='<X>' GROUP BY metadata->>'cycle_code'`. For app-level counts: `selection_applications JOIN selection_cycles GROUP BY status`. Apply at the SCOPE-LOCK phase (right after PM picks Option A), not at the planning phase.
- **Sediment NOT triggered this session** (forward-defense respected):
  - SEDIMENT-242.A respected — probed `pg_proc.prosrc` / `admin_audit_log.action` for canonical symbols before writing audit doc references.
  - SEDIMENT-235.A respected — WATCH-240.B is an internal P162 tag, no GitHub issue tracks it explicitly (no auto-close regex risk).
  - SEDIMENT-238.A / 238.B N/A (no apply_migration this session).
  - SEDIMENT-241.A N/A (no DB-gated tests added).
  - SEDIMENT-186.C N/A (no new test files).
  - SEDIMENT-226.C N/A (no service-role SECDEF dispatch).
- **Correction to P162 #208 attribution**: line in §208 "**Verify-on-next-boot**: … `admin_audit_log` has 14 rows with `action='p240_251_backfill_interview_status'` · cycle4-2026 + cycle3-2026-b2 have 0 apps in `interview_pending` with a submitted interview eval" is partially correct (14 rows total) but the prior line "Backfill scope: cycle4-2026 + cycle3-2026-b2" misled subsequent handoff narrative to interpret "4 of the 14 are cycle3-b2 rows" — the CTE scope is the migration body, the audit row attribution is the live data. Future audits should distinguish between "scope" (what the migration body intends to consider) and "rows actually written" (what the audit log shows after idempotency filter).
- **Carries forward**:
  1. **WATCH-240.C cron observability** (unchanged from p243): cron jobid 47 first fire Mon 2026-05-25 13:00 UTC ≈20h from this audit. Remote agent `trig_01GCFWXgzN1dWndKXRv34eVC` armed; auto-validates 14:00 UTC + files GH issue with outcome.
  2. **OPP-244.A NEW** (deferred): consider an ADR documenting the "administrative onboarding cycle" pattern (cycle3-b2 was first case; possible `selection_cycles.cycle_kind` enum `competitive` vs `administrative` so standard cutoff workflow can be selectively disabled per cycle). Not blocking — current behavior tolerates manual bypass cleanly.
  3. **#348 roadmap Steps 2-4** (unchanged): per-evaluator/committee booking_url schema + auto-select per trail + reschedule/cancel in Hub. PM-gated (3 schema options in #348 require ADR decision).
  4. **All p239b carries unchanged**: #333 W4 invariant U blocked on PM sequencing call ([candidato] 30d window opens 2026-05-24); #334 W5 Angeline legal-ops external 3-5d; #335 ADR-0094 blocked on #333.
  5. **0/38 emails for cutoff_approved** (orthogonal, partially advanced by p243 cycle4 5/38 live dispatch): will resolve naturally as candidates book + cycle4 progresses.
...
### 213. RESOLVED-#229b-FOUNDATION — Final-score PERT régua by track (p246)

- **Date**: 2026-05-24 (p246)
- **Status**: RESOLVED — Foundation shipped (schema + RPC + payload). Frontend visibility deferred to PR2 (p247). PM-dispatched as internal #229b tag (Foundation + Frontend split per PM Option A in conversation 2026-05-24).
- **Origin**: PM dispatch — backend régua PERT do final_score por tipo de vaga/trilha (researcher vs leader, no interview band, no objective rule for final_score, no hiding interview score in modal, no mixing leader_extra into objective, no automatic status changes without PM-approved policy).
- **Foundation shipped (migration `20260805000027`)**: 6 new `final_score_pert_*` columns on `selection_applications` (track-resolved per app); `_compute_pert_cutoff_core` extended with `v_is_final_score` branch (UPDATE track-scoped via `AND role_applied = p_role`; fallback target lookup also track-aware); `recompute_all_active_pert_cutoffs` now orchestrates 4 dimensions (objective + leader_extra + final_score×researcher + final_score×leader); `get_selection_dashboard.cycle` gains `final_score_cutoff_researcher` + `final_score_cutoff_leader` blocks; `get_selection_dashboard.applications[]` chunk-2 gains `interview_score` + 6 `final_score_pert_*` per-app fields (chunk-2 placement preserves PG 100-arg cap on chunk-1).
- **Live smoke**: cycle4 researcher final régua target=**258.12** band=[**232.31, 283.93**] cohort_n=19 method=dynamic rows_updated=34; leader cohort_n=6<10 method=disabled rows_updated=4 (expected); cycle3-b2 researcher target=259.19 band=[233.28, 285.11] cohort_n=18 dynamic rows_updated=1; leader disabled rows_updated=1. Per-app verification: Flavio researcher final=288.50 → target=258.12; Francisleila leader final=309.00 → target=NULL disabled (track separation works end-to-end).
- **Contract tests** (`tests/contracts/p246-229b-final-score-pert-by-track.test.mjs`): 16 tests (15 static + 1 DB-gated suite skipped offline) covering schema columns + 5-arg signature with all 4 defaults preserved + v_is_final_score discriminator + track-scoped UPDATE regression catch + track-aware fallback lookup + recompute orchestration both tracks + audit metadata 4-dimension array + 2 cycle blocks scoped per track + 7 per-app fields + chunk-2 placement + SECDEF + search_path pinning + NOTIFY pgrst + 3 forward-defense regressions (no `interview_score_pert_*` columns; exactly 1 `ELSIF v_is_final_score`; no `UPDATE selection_applications SET status =` honoring PM "no auto status changes" rule). Test baseline **1983/1917/0/66 → 1999/1932/0/67 offline** (+16 / +15 pass / +1 skip).
- **SEDIMENT cleanup**: **SEDIMENT-238.A respected** (3-shadow consolidation recipe — apply_migration created 3 shadow rows `20260524175502/175530/175651`; CTE merge into canonical `20260805000027` + DELETE shadows). **NEW LESSON for sediment-238 family** (escalate if recurs as SEDIMENT-238.C): first apply_migration attempt failed atomic-rollback because original `_compute_pert_cutoff_core` had parameter DEFAULTs (`p_role text DEFAULT 'researcher'::text` + 3 others) that my CREATE OR REPLACE didn't preserve — Postgres errors "cannot remove parameter defaults from existing function". Hot-fixed inline + re-applied. Run `SELECT pg_get_function_arguments(oid)` BEFORE writing migration to capture full signature including DEFAULTs. SEDIMENT-186.C respected (test in both whitelists pre-npm-test). SEDIMENT-235.A respected (internal P162 tag). SEDIMENT-244.A respected (live-queried PERT state BEFORE migration). SEDIMENT-242.A respected (probed RPC bodies BEFORE writing extensions).
- **Data quality observation (deferred, NOT in p246 scope — OPP-246.A NEW)**: Francisleila Melo Santos (leader, cycle4) has `final_score=309.00` which doesn't match declared `leader_score_formula: research_score * 0.7 + leader_extra_pert * 0.3` (would yield 158.3 = her `leader_score`). 309 = 164 + 145 = naive sum of research_score + leader_extra_pert_score. Worth follow-up audit — may reveal historical computation inconsistency OR stale value.
- **Carries forward**:
  1. **PR2 p247 Frontend** (next session): consume new payload to render Interview score + Final score + Final régua classification chips per row. PM dispatched 2-PR split (p246 Foundation → p247 Frontend); Foundation in this PR unblocks Frontend.
  2. **OPP-246.A NEW** (deferred): audit Francisleila `final_score=309` drift vs declared composite formula.
  3. **OPP-245.A** (unchanged from p245): browser-driven Playwright test for /admin/selection.
  4. **WATCH-240.C cron observability** (unchanged): cron jobid 47 Mon 2026-05-25 13:00 UTC.
  5. **#348 / #333 / #334 / #335** (unchanged).
  6. **OPP-244.A** (unchanged): ADR for "administrative onboarding cycle" pattern.
- **PR**: `agent/p246-229b-foundation-final-score-pert-by-track` → main (Foundation only — schema + RPC + payload + contract test, standard CI gate path, 0 bypass).
- **Verify-on-next-boot**: HEAD includes p246 PR · migration `20260805000027` present (565 lines) · 6 new `final_score_pert_*` columns on `selection_applications` · `supabase_migrations.schema_migrations` has version `20260805000027` registered (no shadow rows at `20260524175*`) · `recompute_all_active_pert_cutoffs` body declares 4 dimensions in audit metadata · `get_selection_dashboard.cycle` payload has 2 new final_score_cutoff blocks · cycle4 researcher-track apps have `final_score_pert_target=258.12` populated · cycle4 leader-track apps `final_score_pert_cutoff_method='disabled'` · npm test 1999/1932/0/67 offline · invariants 19/19=0 · MCP `/health` unchanged.
- **Cross-ref**: PM dispatch p246 conversation 2026-05-24 (post-p245 close) · p219 Phase 1 (leader_extra cohort separation precedent — same pattern applied to final_score) · p232 #229 Phase 2 (read-surface for leader_extra; this PR extends pattern to final_score) · p197b (objective cohort PERT) · p245 #229a (per-candidate UI baseline — Frontend PR p247 will consume new payload) · `selection_cycles.scoring_formula.tracks` (CR-047 — track definitions used for cohort separation) · OPP-246.A drift (Francisleila final_score=309).

### 214. RESOLVED-#229b-FRONTEND — Per-candidate final-score chip + interview score visibility (p247)

- **Date**: 2026-05-24 (p247)
- **Status**: RESOLVED — Frontend PR2 of PM-dispatched 2-PR split shipped. Consumes p246 Foundation payload to render Interview score (plain number, no chip per PM rule) + Final score + finalScoreChip (track-resolved per app, classifies above/within/below band with delta, or surfaces "Régua final: n=N<10" disabled message when cohort_n<10 for the track). PM Option A picked (2 new columns between Líder and Rank) over inline expansion or rank-column reorg.
- **Origin**: PM dispatch verbatim (handoff p246 §Carries): *"Abrir p247 Frontend para consumir o payload novo do #229b e renderizar em /admin/selection: Nota da Entrevista, Score Final e chip de régua final por candidato/trilha. Não criar régua para entrevista. Manter PERT Objetiva como gate para entrevista e PERT Final como comparação pós-entrevista."*
- **Shipped (frontend-only, 0 migrations, 0 schema changes)**:
  - `src/pages/admin/selection.astro` — new helper `finalScoreChip(row)` (~25 lines) consuming `r.final_score_pert_*` per-app fields (track-resolved by p246 backend `_compute_pert_cutoff_core` UPDATE scoped via `AND role_applied = p_role` — no manual switching needed); handles `method === 'disabled'` branch (mirrors `leaderExtraChip` pattern); delegates classification to `bandClassify`; reuses `pertFullTooltip` for hover-rich operator visibility (cohort_n + method).
  - 2 new TH headers between `data-sort="leader_score"` (Líder) and `data-sort="rank"` (Rank): `Nota Entrevista` (sortable + i18n hint) + `Final` (sortable + i18n hint).
  - 2 new TDs in row render between leader_score TD and rank TD: interview_score plain number (PM rule: no régua for interview); final_score + finalScoreChip(r) below.
  - `colspan="13"` → `colspan="15"` in both error + empty state placeholders (was 1 short of total even pre-p247, now bumped by +2 cols).
  - T object (frontmatter + fallback inside script) wires 2 helper-consumed keys (`finalScoreCutoffTooltip` + `finalScoreDisabledChip`).
  - 6 new i18n keys × 3 langs (18 inserts): `admin.selection.colInterviewScore` + `colInterviewScoreHint` + `colFinal` + `colFinalHint` + `finalScoreCutoffTooltip` + `finalScoreDisabledChip`. Reuses `pertBandAbove/Within/Below` labels already shipped p245.
- **Contract tests** (`tests/contracts/p247-229b-frontend-per-candidate-final-score.test.mjs`): **35 assertions** (28 static + 6 i18n parity + 3 forward-defense regressions locking PM "do-not" rules) — helper signature + reads per-app fields + disabled branch + bandClassify delegation + pertFullTooltip reuse + 2 TH wiring + TH ordering (leader→intvScore→final→rank) + 2 TD render + colspan bump + T object wiring frontmatter + T object fallback + 6 keys × 3 langs + {n} placeholder parity + **3 forward-defense**: (1) interview_score TD must NOT call any chip helper (no régua for interview); (2) finalScoreChip body must NOT reference `currentCycle?.pert_cutoff` or `leader_extra_cutoff` (PM rule: PERT Objetiva = gate to interview, separate from PERT Final = post-interview comparison); (3) no `interview_score_pert_*` columns or `interviewScoreChip` helper anywhere on the page (locks against future drift that would conflate dimensions). Test baseline **1999/1932/0/67 → 2034/1967/0/67 offline** (+35 pass).
- **SEDIMENT cleanup**: SEDIMENT-186.C respected (new test added to BOTH `"test"` + `"test:contracts"` whitelists in package.json BEFORE running npm test — caught nothing, but discipline preserved). SEDIMENT-235.A respected (#229b is internal P162 tag — no GitHub issue, PR title uses neutral phrasing "p247 — Per-candidate Final + Interview score visibility" without `close[sd]?|fix(?:es|ed)?|resolve[sd]?` + `#N` for any GitHub issue that should stay open). SEDIMENT-238.C avoided (no migrations this PR — frontend-only).
- **PM rules locked by 3 forward-defense regressions** (next session won't accidentally drift these):
  1. **interview score = plain number, never a chip** — `pertBandChip/leaderExtraChip/finalScoreChip/bandClassify` calls forbidden inside interview_score TD block.
  2. **finalScoreChip uses per-app track-resolved cutoff, never cycle-level objective cutoff** — `currentCycle?.pert_cutoff` reference forbidden inside finalScoreChip body. Same for `leader_extra_cutoff` (separate dimension).
  3. **No interview_score_pert_* schema column or interviewScoreChip helper exists** — locks against the temptation to add an "interview régua" via this column naming pattern.
- **Carries forward** (unchanged from p246 + 1 new):
  1. **OPP-247.A NEW (low priority)**: pre-existing 1-off in colspan (was "13" for 14 actual TDs; now "15" for 16 actual TDs — same 1-off bug preserved; out of scope to fix).
  2. **OPP-246.A** (unchanged): audit Francisleila `final_score=309` drift vs declared composite formula.
  3. **OPP-245.A** (unchanged from p245): browser-driven Playwright test for /admin/selection visual rendering.
  4. **WATCH-240.C cron observability** (unchanged): cron jobid 47 Mon 2026-05-25 13:00 UTC.
  5. **#348 / #333 / #334 / #335** (unchanged).
  6. **OPP-244.A** (unchanged): ADR for "administrative onboarding cycle" pattern.
- **PR**: `agent/p247-229b-frontend-per-candidate-final-score` → main (Frontend leaf — selection.astro helper + 2 TH + 2 TD + 6 i18n keys × 3 langs + 35-assertion contract test, standard CI gate path, 0 bypass).
- **Verify-on-next-boot**: HEAD includes p247 PR · `src/pages/admin/selection.astro` has `finalScoreChip(row: any): string` helper · 2 new TH with `data-sort="interview_score"` + `data-sort="final_score"` placed between leader_score TH and rank TH · 2 new TDs in row render (interview_score plain, final_score + finalScoreChip) · `colspan="15"` in both error + empty state (no leftover `colspan="13"`) · T object wires `finalScoreCutoffTooltip` + `finalScoreDisabledChip` in frontmatter + fallback · 6 new i18n keys present in pt-BR/en-US/es-LATAM with `{n}` placeholder parity in finalScoreDisabledChip · `tests/contracts/p247-229b-frontend-per-candidate-final-score.test.mjs` present (35 assertions) · package.json whitelist has test in BOTH `"test"` + `"test:contracts"` · npm test **2034/1967/0/67** offline · invariants **19/19=0** · MCP `/health` unchanged.
- **Cross-ref**: PM dispatch handoff p246 §Carries (2026-05-24) · p246 #229b Foundation (backend payload extension shipped — this PR consumes) · p245 #229a (per-candidate PERT classification baseline — reuses bandClassify + pertBandChip + pertFullTooltip helpers + pertBandAbove/Within/Below i18n keys) · p219 Phase 1 (leader_extra cohort separation — established disabled-state policy via cohort_n<10 threshold; finalScoreChip applies same pattern) · `selection_cycles.scoring_formula.tracks` (CR-047 — track definitions backend uses for cohort separation; frontend just reads track-resolved per-app fields).

### 215. RESOLVED-OPP-246.A — submit_evaluation canonical final_score (drops naïve sum race) + Francisleila cleanup (p248)

- **Date**: 2026-05-24 (p248, same UTC day as p247)
- **Status**: RESOLVED — PM Option C+B picked + executed (structural fix in `submit_evaluation` + immediate cleanup of Francisleila's drifted row + admin_audit_log evidence). Audit doc `docs/audit/OPP_246_A_FRANCISLEILA_FINAL_SCORE_DRIFT.md` (440+ lines) captured root cause, scope (1 row), cohort risk quantification (currently 0 contamination), and the 4 remediation options PM ratified.
- **Origin**: PM call-out post-p247 ship: *"ponto de atenção real agora é o carry OPP-246.A: auditar o final_score=309 da Francisleila antes de o cohort líder chegar em n>=10, porque isso pode contaminar uma futura régua dinâmica de líderes se for drift antigo."*
- **Root cause** (audit doc detail): `submit_evaluation` (leader_extra + interview branches) wrote `final_score = obj + interview + leader_extra` (naïve sum) in its own UPDATE on `selection_applications`. Same RPC's INSERT on `selection_evaluations` fires `_trg_recompute_application_scores` → `compute_application_scores` → canonical `final_score = COALESCE(leader_score, research_score)` (CR-047 weighted: `leader_score = research × 0.7 + leader_extra × 0.3`). The trigger writes correct value BEFORE submit_evaluation's own UPDATE OVERWRITES it with naïve sum. **Structural race within the same RPC.** Self-heal exists via `submit_interview_scores` (modern interview path explicitly calls `compute_application_scores`), but apps stuck pre-interview after `submit_evaluation('leader_extra')` retain naïve sum. Francisleila (cycle4 leader, screening) was the only live row in this stuck state.
- **Shipped (migration `20260805000028`)**:
  - **Step C — Structural fix**: CREATE OR REPLACE FUNCTION `submit_evaluation` (same 6-arg signature with 3 DEFAULTs preserved, SECDEF + search_path=public preserved per minimum-diff). Interview + leader_extra branches now: (1) UPDATE only the discrete columns they own (interview_score / leader_extra_pert_score / status='final_eval'); (2) explicitly `PERFORM public.compute_application_scores(p_application_id)` for defense-in-depth (the trigger already invokes it; explicit call documents intent and survives any future trigger change).
  - **Step B — Cleanup**: DO block `PERFORM public.compute_application_scores('72ea1a45-...')` on Francisleila's row, then INSERT INTO `admin_audit_log` row with `action='selection.final_score_canonical_reconciliation'`, `target_type='selection_application'`, full metadata (before_final_score=309, after_final_score=158.30, canonical_expected=158.30, compute_result envelope, rpc_version=p248, migration=20260805000028, audit_doc=path). Idempotent on re-apply (skips audit insert if value didn't change).
  - **Sanity DO** RAISES if any leader row has `final_score` drift > 0.5pt after refactor + cleanup. Atomic rollback if drift survives.
  - `NOTIFY pgrst, 'reload schema'` trailer.
- **Live smoke post-apply**: Francisleila `final_score: 309.00 → 158.30` ✓ (matches `leader_score` canonical exactly). Audit row created at 2026-05-24 19:43:59 UTC with full evidence. Cross-cycle re-audit query: 0 leader drift, 1 researcher drift (Fabiano cycle3-2026, partial evals, rejected — separate edge case documented in audit doc, NOT in any current/future cohort).
- **Contract test** (`tests/contracts/p248-opp-246-a-submit-evaluation-canonical-final-score.test.mjs`): **20 assertions** (14 static migration body + 3 forward-defense + 3 DB-gated). Static: signature with 3 DEFAULTs preserved + SECDEF+search_path=public preserved + interview branch removes naïve-sum line + leader_extra branch removes naïve-sum line + both branches add PERFORM compute_application_scores + status='final_eval' preserved in interview + interview_score/leader_extra_pert_score column writes preserved + objective branch unchanged + cleanup DO targets Francisleila ID + cleanup uses compute_application_scores (not direct UPDATE) + audit row shape + idempotent guard + sanity DO RAISE EXCEPTION + NOTIFY pgrst. **Forward-defense (3)**: (1) no `UPDATE...SET interview_score=...,final_score=...` (regex catches interview SET clause with final_score in same UPDATE — would catch p248 fix regression); (2) same regex for leader_extra branch; (3) submit_evaluation body (code only, comments stripped) must NOT contain any `final_score = ` assignment — canonical write lives ONLY in compute_application_scores. DB-gated: Francisleila live final=158.30 + audit row present + 0 leader drift goal. Test baseline **2034/1967/0/67 → 2054/1986/0/68 offline** (+20/+19/+1 skip).
- **Side-effect** (legitimate test contract update): `tests/contracts/selection-blind-eval.test.mjs:206-211` previously asserted `submit_evaluation` body must contain `final_score = obj + intv + le` naïve-sum formula. Contract intent rewritten to assert canonical delegation: must contain `PERFORM public.compute_application_scores(p_application_id)`. Same intent (final_score gets computed when interview/le submitted) via different mechanism. Cited in updated test docstring.
- **SEDIMENT cleanup**: **SEDIMENT-238.C respected** — probed `pg_proc.prosrc + pg_get_function_arguments(oid) + prosecdef + proconfig` BEFORE writing migration; preserved 6-arg signature with all 3 DEFAULTs exactly + SECDEF + search_path=public; no atomic-rollback this session. **SEDIMENT-238.A respected** — apply_migration created no shadow rows this time (single CREATE OR REPLACE + 2 DO blocks; check passed clean). **SEDIMENT-186.C respected** — new test added to BOTH `"test"` + `"test:contracts"` whitelists in package.json BEFORE running npm test. **SEDIMENT-235.A respected** — OPP-246.A is internal P162 tag (no GitHub issue, no auto-close keyword risk).
- **Cohort contamination risk now CLOSED**: with Francisleila reconciled + structural bug fixed, future `submit_evaluation` calls cannot re-introduce drift. The leader cohort (currently 8 approved leaders all-time, 0 drift) stays clean as new approvals enter. When `cohort_n` hits 10, the leader final régua becomes dynamic from a guaranteed-canonical input set. PM's stated risk: "isso pode contaminar uma futura régua dinâmica de líderes se for drift antigo" — neutralized.
- **Carries forward** (unchanged from p247 + 1 NEW):
  1. **OPP-248.A NEW (LOW)**: Fabiano Bressiani (cycle3-2026 researcher, status=rejected, final_score=35, research_score=NULL, interview_score=5) — partial-eval edge case where canonical formula returns NULL but final_score is not NULL. NOT in any cohort (rejected, no researcher cohort affected). Worth a one-shot UPDATE to NULL if PM wants strict canonical, or just leave (no operational impact). Documented in audit doc side-finding section.
  2. **OPP-246.A** — **CLOSED p248**, can be removed from carry list.
  3. **OPP-247.A** (unchanged): pre-existing 1-off colspan in selection.astro (low priority cosmetic).
  4. **OPP-245.A** (unchanged): browser-driven Playwright test for /admin/selection.
  5. **WATCH-240.C cron observability** (unchanged): cron jobid 47 first fire Mon 2026-05-25 14:00 UTC.
  6. **#348 / #333 / #334 / #335** (unchanged).
  7. **OPP-244.A** (unchanged): ADR for "administrative onboarding cycle" pattern.
- **PR**: `agent/p248-opp-246-a-submit-evaluation-canonical-final-score` → main (standard CI gate path, 0 bypass).
- **Verify-on-next-boot**: HEAD includes p248 PR · `supabase/migrations/20260805000028_p248_opp_246_a_submit_evaluation_canonical_final_score.sql` present (313 lines) · `supabase_migrations.schema_migrations` has version `20260805000028` registered · live `pg_proc.submit_evaluation.prosrc` interview + leader_extra branches contain `PERFORM public.compute_application_scores` and NO `final_score = ` assignment · Francisleila row `final_score=158.30 = leader_score` (canonical) · 1 audit row `selection.final_score_canonical_reconciliation` for Francisleila · 0 leader drift across all cycles · `tests/contracts/p248-opp-246-a-submit-evaluation-canonical-final-score.test.mjs` present (20 assertions) · `tests/contracts/selection-blind-eval.test.mjs:206-216` updated for canonical delegation · package.json whitelist has p248 test in BOTH `"test"` + `"test:contracts"` · npm test **2054/1986/0/68** offline · `check_schema_invariants()` **19/19=0** · MCP `/health` unchanged.
- **Cross-ref**: PM dispatch post-p247 close 2026-05-24 ("ponto de atenção real agora é o carry OPP-246.A") · audit doc `docs/audit/OPP_246_A_FRANCISLEILA_FINAL_SCORE_DRIFT.md` (440+ lines, 4 remediation options) · p247 #229b Frontend (made the drift visible via finalScoreChip on /admin/selection) · p246 #229b Foundation (the `final_score_pert_*` columns + cohort recompute that surfaced the cohort contamination risk vector) · `pg_proc.compute_application_scores` (canonical CR-047 weighted writer, unchanged) · `_trg_recompute_application_scores` AFTER INSERT trigger on `selection_evaluations` (unchanged, now no longer overridden) · `pg_proc.submit_interview_scores` (modern interview path that already calls compute, unchanged) · CR-047 scoring_formula (`leader_score = research × 0.7 + leader_extra × 0.3`).

### 216. DISPATCH-SPEC-348 — Booking URL per Evaluator / Committee v1 spec + 4 ready-leaf children (p249)

- **Date**: 2026-05-24 (p249, same UTC day as p243/p244/p245/p246/p247/p248 cluster)
- **Status**: DISPATCH — spec doc drafted + PM-ratified on 3 open decisions (Q1/Q2/Q3 all Recommended defaults) + 4 GitHub child issues spawned with concrete acceptance criteria. Implementation deferred to next session(s); this session strictly schema design + decision capture per PM boot directive ("desenhar v1; deixar reschedule/cancel fora do v1"). No code/migration changes this session.
- **Origin**: Boot directive 2026-05-24 ("Seguir para #348 booking routing/calendar registry spec: registrar decisão de curto prazo: link Núcleo usado no Cycle 4; desenhar v1 com booking_url por avaliador/comitê; researcher -> Vitor/Fabricio individual; leader -> Núcleo/dupla; deixar reschedule/cancel fora do v1."). #348 itself was filed p243 (PR #346 post-merge PM dispatch interruption) with the 4-step roadmap; Step 1 shipped p243 (cycle.interview_booking_url populated for cycle4-2026 to unblock the 5 above-target dispatches).
- **Substrate audit (verified live 2026-05-24)**:
  - `selection_cycles.interview_booking_url text` (only existing URL column — group/cycle level)
  - `selection_committee` (cycle_id, member_id, role, can_interview, organization_id) — **EMPTY for cycle4-2026 live** (p226 committee seed carry deprioritized; PM operates via SEDIMENT-226.C JWT-claim injection pattern for cycle4 cutoff dispatches)
  - `members` has NO booking URL column today
  - De facto cycle4 evaluators (from `selection_evaluations`): Vitor (51 evals, member `880f736c-3e76-4df4-9375-33575c190305`) + Fabricio (40 evals, member `92d26057-5550-4f15-a3bf-b00eed5f32f9`)
  - `notify_selection_cutoff_approved` raises `CUTOFF_NO_BOOKING_URL` errcode P0020 when cycle URL is NULL/empty; email template (pt/en/es) hardcodes `{{interview_booking_url}}` placeholder
- **Spec shipped** (`docs/specs/SPEC_348_BOOKING_URL_PER_EVALUATOR.md`, ~310 lines): 10 sections covering v0 baseline (Step 1 already shipped) + current state audit + v1 IN/OUT scope + schema proposal Option B Híbrido (per-cycle + per-member with precedence + per-committee override field) + routing decision tree (`role_applied='leader'` → cycle URL; `role_applied='researcher'` → LRD round-robin among committee members with resolvable URLs; fallback to cycle URL when researcher candidates empty) + test plan (offline + DB-gated + live smoke) + 3 ratified decisions + migration plan (4 children) + cross-ref + PM A/B/C/D streak section honoring boot directive constraints.
- **PM ratifications (3 questions all Recommended)**:
  - **Q1 (routing policy)** → **LRD round-robin** via `selection_dispatch_url_log` `LEFT JOIN LATERAL ... MAX(dispatched_at)` lookback (deterministic, fair across time, observable via audit log, sub-ms cost with proposed index, streaks self-correct after each dispatch). Rejected: Random (visible streaks), Least-loaded (requires defining "loaded" across off-platform commitments).
  - **Q2 (URL field)** → **`members.interview_booking_url text`** nullable, app-level regex `^https?://`, no DB CHECK (preserves grep parity with `selection_cycles.interview_booking_url`; deep-link providers remain compatible). Rejected: `calendar_booking_url` (breaks grep symmetry), DB CHECK (traps future schemes), junction table `member_booking_pools` (overkill for 2 evaluators).
  - **Q3 (cycle4 reseed)** → **Seed committee Vitor+Fabricio researcher with URLs NULL** as SPEC #348 Child #4 DML migration; routing falls back to `cycle.interview_booking_url` until PM populates personal URLs via Child #3 admin UI. Zero immediate behavior change. Rejected: Defer seed (creates orphan leaf), Seed with real URLs (blocked on PM-provided URLs).
- **GitHub child issues spawned (4, in execution order)**:
  - **#354** Child #1 Foundation: DDL only — `ALTER TABLE members ADD COLUMN interview_booking_url text` + `ALTER TABLE selection_committee ADD COLUMN interview_booking_url text` + `CREATE TABLE selection_dispatch_url_log (id, application_id, cycle_id, track CHECK ('researcher','leader'), resolved_url, resolution_path CHECK ('committee_override','member_global','cycle_fallback'), resolved_evaluator_id, dispatched_at, organization_id)` + 2 indexes (app_idx + cycle_round_robin_idx) + RLS enable + SELECT policy gated on `view_selection`/`manage_member` + NOTIFY pgrst. Estimated +13 offline contract assertions. Independently shippable (no RPC body changes, no behavior change).
  - **#355** Child #2 RPC body: `CREATE OR REPLACE FUNCTION notify_selection_cutoff_approved` with track-aware routing per §5.1 decision tree + LRD pick + INSERT into `selection_dispatch_url_log` BEFORE `campaign_send_one_off`. Same 1-arg signature preserved (SEDIMENT-238.C — probe `pg_get_function_arguments` + `prosecdef` + `proconfig` BEFORE migration). Estimated +12 offline + DB-gated subset. **Blocked by #354.** Forward-defense regressions lock LRD policy + "leader → cycle-only" PM rule.
  - **#356** Child #3 Admin UI: editable form field on `/admin/members/[id]` + 6 i18n keys × 3 langs (18 entries) + client-side `^https?://` regex validation. **Blocked by #354** (column must exist before UI can edit it).
  - **#357** Child #4 Cycle4-2026 committee reseed: one-shot DML `INSERT INTO selection_committee` for Vitor + Fabricio (`role='researcher'`, `can_interview=true`, URLs NULL) with `NOT EXISTS` idempotency guard + audit row `action='selection.cycle4_committee_reseed_spec_348'` + sanity DO RAISE if count != 2. **Blocked by #354 + #355** (defensive ordering: routing logic must exist before committee transitions from empty to non-empty).
- **Out of v1 (deferred to Steps 3+ children — not spawned this session)**:
  - Step 4: Hub-side reschedule/cancel flows (PM directive: out of v1)
  - Google Calendar API integration (read availability, create event server-side)
  - Per-evaluator availability matrix (today/next-7d busy windows in UI)
  - Round-robin alternatives (least-loaded by open interview count, weighted by capacity, manual sticky)
  - Auto-trigger from cron jobid 47 → cutoff dispatch (p230 fast-follow; governance question)
  - `cycle.interview_booking_url` deprecation — v1 is purely additive
  - p226 committee seed automation for cycles other than cycle4-2026
- **Carries forward** (unchanged from p248 + #354/#355/#356/#357 NEW as ready-leaves):
  1. **#354/#355/#356/#357 NEW** — SPEC #348 children spawned this session; ready-leaves blocked only by intra-spec ordering (#354 first; #355/#356/#357 parallel after #354)
  2. **WATCH-240.C cron observability** — cron jobid 47 first fire Mon 2026-05-25 13:00 UTC ~17h post-this-session (boot directive said 13:00; handoff p243 said 14:00; cron schedule `0 13 * * 1` confirms 13:00 UTC); pre-fire arm verification done this session (0 prior runs, active=true, schedule armed correctly)
  3. **OPP-248.A** — Fabiano partial-eval edge (LOW)
  4. **OPP-247.A** — colspan 1-off (LOW)
  5. **OPP-245.A** — Playwright browser test for /admin/selection
  6. **OPP-244.A** — ADR for administrative onboarding cycle
  7. **#333/#334/#335** — LGPD Wave 4/5 + ADR-0094 (all blocked externally or on PM sequencing)
- **SEDIMENT respected this session**: SEDIMENT-235.A (PR-narrative-close-keyword-avoidance) — child issues #354/#355/#356/#357 reference parent #348 as "Parent" NOT "closes" so parent stays open until ALL 4 children ship; gh issue comment on #348 uses "ready for implementation" framing, no auto-close keywords.
- **PR**: None this session (spec-only delivery + GH issue spawn + doc updates).
- **Verify-on-next-boot**: HEAD unchanged from p248 (`93c8e942`) · `docs/specs/SPEC_348_BOOKING_URL_PER_EVALUATOR.md` present (~310 lines, 10 sections, §7 marked RATIFIED with 3 PM decisions) · 4 GH issues #354/#355/#356/#357 OPEN with labels `enhancement,selection` (or `enhancement,ux` for #356) and parent #348 cross-ref · `gh issue view 348` shows new comment "SPEC v1 + child split (2026-05-24 p249)" with all 4 child links + Q1/Q2/Q3 ratifications · `docs/project-governance/ISSUE_REGISTRY.md` header narrative includes p249 SPEC #348 section · `docs/audit/P162_GAP_OPPORTUNITY_LOG.md` entry #216 DISPATCH-SPEC-348 present · `check_schema_invariants()` 19/19=0 (unchanged) · `npm test` 2054/1986/0/68 offline (unchanged) · `selection_committee` for cycle4-2026 still EMPTY pre-Child-#4 · `members.interview_booking_url` does NOT exist pre-Child-#1 · `selection_dispatch_url_log` table does NOT exist pre-Child-#1 · cron jobid 47 active + scheduled `0 13 * * 1` + 0 prior runs (first fire Mon 13:00 UTC).
- **Cross-ref**: #348 parent (4-step roadmap from p243) · `docs/ops/CYCLE4_CUTOFF_DISPATCH_P243.md` (v0 runbook) · `docs/audit/CYCLE4_PERT_CUTOFF_P242_WATCH_240_C.md` (origin of #348) · `supabase/migrations/20260805000011_p228_260_w2_leaf4_selection_cutoff_approved.sql` (current RPC body that v1 extends) · p226 carry (cycle4 committee seed, now executed as SPEC #348 Child #4) · SEDIMENT-226.C JWT-claim injection (no longer needed post-Child #4 once committee seeded) · p230 fast-follow (cron auto-dispatch governance — separate decision deferred).

### 217. RESOLVED-#354 — SPEC #348 Child #1 Foundation DDL shipped: per-evaluator booking URL schema + audit log (p250)

- **Date**: 2026-05-24 (p250, ~3.5h post-p249 spec dispatch)
- **Status**: RESOLVED — migration `20260805000029` applied live + local file + repair done; 27 new contract assertions (23 static + 4 DB-gated) all pass; baseline 2054/1986/0/68 → **2081/2009/0/72 offline**; check_schema_invariants 19/19=0 unchanged; astro build 0 new errors. Independently shippable: zero behavior change until Child #2 (#355) lands RPC body.
- **Origin**: PM boot directive 2026-05-24 ("Se cron OK, iniciar #354 Child #1 Foundation: DDL only; ALTER members ADD interview_booking_url text; ALTER selection_committee conforme spec; CREATE selection_dispatch_url_log; constraints/regex conforme spec; contract tests offline; zero behavior change até #355").
- **Pre-flight state**: HEAD `93c8e942` unchanged from p249; cron jobid 47 confirmed armed `0 13 * * 1` (Mon 13:00 UTC), `cron.job_run_details` empty (no prior fires), first natural fire pending Mon 2026-05-25 13:00 UTC (~16.5h post-this-session at boot time 20:33 UTC); members.interview_booking_url + selection_committee.interview_booking_url + selection_dispatch_url_log all absent pre-apply.
- **Migration shipped** (`supabase/migrations/20260805000029_p250_354_spec_348_v1_foundation_booking_url_per_evaluator.sql`, ~175 lines incl header):
  - `ALTER TABLE public.members ADD COLUMN interview_booking_url text` + COMMENT
  - `ALTER TABLE public.selection_committee ADD COLUMN interview_booking_url text` + COMMENT
  - `CREATE TABLE IF NOT EXISTS public.selection_dispatch_url_log` (9 columns: id, application_id, cycle_id, track, resolved_url, resolution_path, resolved_evaluator_id, dispatched_at, organization_id) with PK + 4 FKs (application_id → selection_applications, cycle_id → selection_cycles, resolved_evaluator_id → members (nullable for cycle_fallback/leader), organization_id → organizations) + 2 CHECK (track IN ('researcher','leader'), resolution_path IN ('committee_override','member_global','cycle_fallback')) + table COMMENT
  - 2 indexes: `selection_dispatch_url_log_app_idx (application_id)` + `selection_dispatch_url_log_cycle_round_robin_idx (cycle_id, track, resolved_evaluator_id, dispatched_at DESC) WHERE track = 'researcher'` (partial — supports LRD pick in Child #2)
  - **RLS V4 canonical pair** (deny-all + org-scope) — matches every other selection_* table (selection_applications, selection_committee, selection_cycles, selection_evaluations, selection_interviews, selection_evaluation_ai_suggestions, selection_diversity_snapshots). NOT spec literal `rls_can('view_selection') OR rls_can('manage_member')` — see RLS divergence below.
  - Sanity DO block: 7 EXISTS checks raising EXCEPTION on missing artifacts (members col, committee col, table, 2 indexes, 2 policies) → migration won't apply if any artifact missing
  - `NOTIFY pgrst, 'reload schema'` trailer
- **RLS divergence from spec literal** (PM Option A ratified mid-session 2026-05-24): spec §4.1 draft proposed `CREATE POLICY ... FOR SELECT TO authenticated USING (rls_can('view_selection') OR rls_can('manage_member'))`, but live audit of `engagement_kind_permissions` showed `view_selection` is NOT in the action catalog (only 17 actions: award_champion, curate_content, manage_*, participate_in_governance_review, promote, sign_chain_leader, view_chapter_dashboards, view_internal_analytics, view_pii, write, write_board). Using it literally would silently deny all reads except `manage_member` holders. Audit of pg_policy across `selection_*` tables revealed the dominant convention is the V4 deny-all + org-scope pair (8 tables follow it; only 2 audit-snapshot tables `selection_membership_snapshots`/`selection_ranking_snapshots` have explicit admin SELECT policies using a 4-action OR). PM picked Option A (canonical V4 pair); all reads via SECDEF RPC; admin audit RPC can be added in Child #2 or later if direct SELECT becomes useful. Spec §4.1 amended in same commit to match shipped migration with explanatory note documenting the rejected `view_selection` path.
- **Contract test** (`tests/contracts/p250-354-spec-348-v1-foundation.test.mjs`, 27 assertions):
  - **Static (23)**: file existence + header (WHAT/WHY/ROLLBACK + #348/#354 refs) + spec doc anchor + 2 ALTER + 2 COMMENT + CREATE TABLE IF NOT EXISTS + 9-column declaration + 2 CHECK enum exactness + resolved_evaluator_id nullable + table COMMENT + app_idx exact + cycle_round_robin_idx exact partial predicate + RLS ENABLE + 2 RLS policy declarations + sanity DO 5 RAISE strings + NOTIFY trailer + **4 forward-defense regressions**: (a) active SQL must NOT contain `CREATE OR REPLACE FUNCTION` or `CREATE FUNCTION` (Child #2 scope guard); (b) active SQL must NOT reference `rls_can('view_selection')` (locks the spec-divergence regression — strips line comments so header RLS NOTE can document the rejected path); (c) members.interview_booking_url + selection_committee.interview_booking_url must NOT carry DB CHECK (Q2: validation app-level); (d) migration must NOT touch `selection_cycles.interview_booking_url` (cycle URL stays intact for fallback + leader default).
  - **DB-gated (4)**: PostgREST column-probe for members + selection_committee + table (with all 9 columns named in select) + `supabase_migrations.schema_migrations` row presence via `_audit_list_schema_migrations` helper RPC.
  - Test added to BOTH `"test"` and `"test:contracts"` whitelists in package.json (SEDIMENT-186.C respected).
- **ADR-0012 allowlist** (`tests/contracts/schema-cache-columns.test.mjs`): `members.interview_booking_url` added to `STORAGE_ONLY_ALLOWLIST` with rationale "Personal evaluator calendar/booking pool URL (SPEC #348); user-set via admin form, no derivation source". This is the third pattern-44 (storage-only opt-in) allowlist entry on members (sibling: `gamification_opt_out`, `notify_weekly_digest`, `notify_delivery_mode_pref`). Caught by full `npm test` — the static contract enforces every new column on members/engagements/initiatives ships either with a maintenance trigger OR with an explicit storage-only declaration.
- **Live DB post-apply verification** (8/8 EXIST checks + 7/7 constraints + 19/19 invariants):
  - members.interview_booking_url ✓
  - selection_committee.interview_booking_url ✓
  - selection_dispatch_url_log table + 9 cols ✓
  - selection_dispatch_url_log_app_idx ✓
  - selection_dispatch_url_log_cycle_round_robin_idx ✓ (partial WHERE track='researcher')
  - RLS enabled ✓
  - rpc_only_deny_all policy ✓
  - selection_dispatch_url_log_v4_org_scope policy ✓
  - 7 constraints (2 CHECK + 4 FK + 1 PK) ✓
  - `check_schema_invariants()` 19/19=0 unchanged
- **Sediment**: 0 NEW sediments (escalation-eligible). **SEDIMENT-238.A recurred** (caught by CI on first push of PR #358; cleanup applied; same pattern, no new letter assignment): `apply_migration` via MCP created a shadow row at wall-clock timestamp `20260524203814` (same name as canonical), and `supabase migration repair --status applied 20260805000029` did NOT delete it — only registered the canonical entry. ADR-0097 missing-file drift gate (`tests/contracts/rpc-migration-coverage.test.mjs:748`) detected the shadow as `NEW missing-file drift`. Cleanup: `DELETE FROM supabase_migrations.schema_migrations WHERE version = '20260524203814' AND name = 'p250_354_spec_348_v1_foundation_booking_url_per_evaluator'`. Re-ran CI. The three-step apply_migration ritual is now (1) write local file at canonical timestamp; (2) `supabase migration repair --status applied <canonical>`; (3) **DELETE shadow row at wall-clock timestamp** — step 3 is the easy-to-miss one. Future sessions: after every `apply_migration`, query `SELECT version FROM supabase_migrations.schema_migrations WHERE name = '<name>' ORDER BY version DESC LIMIT 2;` to find both rows; delete the wall-clock one. Other sediments respected: SEDIMENT-238.C (no CREATE OR REPLACE FUNCTION in this migration so the parameter-DEFAULT preservation lesson doesn't apply directly, but its general principle — "probe live shape BEFORE migration" — was applied via the pre-migration audit of `engagement_kind_permissions` and existing `selection_*` RLS policies before deciding on the V4 canonical pair); SEDIMENT-186.C (package.json whitelist pre-`npm test`); SEDIMENT-235.A (PR title/body uses neutral wording — child #354 is a child of #348, not a close trigger for the parent); SEDIMENT-244.A (live-queried `engagement_kind_permissions` BEFORE planning RLS policy text — caught `view_selection` non-existence at SCOPE-LOCK phase, not at planning phase).
- **Carries forward** (unchanged from p249):
  1. **#355 Child #2 RPC body** — next dispatch; blocked by #354 (now resolved); same 1-arg `notify_selection_cutoff_approved` signature must be preserved per SEDIMENT-238.C (probe `pg_get_function_arguments` before migration). Schema is now ready for `INSERT INTO selection_dispatch_url_log` + LRD `LEFT JOIN LATERAL` pick.
  2. **#356 Child #3 Admin UI** — unblocked by #354 (column exists); can ship in parallel with #355.
  3. **#357 Child #4 Cycle4-2026 committee reseed** — still blocked by #354 + #355 (defensive ordering: routing logic must exist before committee transitions from empty to non-empty).
  4. **WATCH-240.C cron observability** — cron jobid 47 first fire ~Mon 2026-05-25 13:00 UTC ~16h post-this-session; pre-fire arm verified again p250 (schedule + active + 0 prior runs unchanged from p249 audit).
  5. **OPP-248.A / OPP-247.A / OPP-245.A / OPP-244.A** — unchanged (LOW priority deferred items).
  6. **#333/#334/#335** — LGPD Wave 4/5 + ADR-0094 (all blocked externally or on PM sequencing).
- **PM A/B/C/D streak**: PM picked Option A (V4 canonical RLS pair) on the spec-divergence mid-session question → **17 consecutive Recommended**.
- **PR**: #358 (this session, post-merge link added on commit).
- **Verify-on-next-boot**: HEAD advances to PR #358 merge commit · 27 contract tests pass offline · members.interview_booking_url + selection_committee.interview_booking_url + selection_dispatch_url_log + 2 indexes + 2 RLS policies all present live · check_schema_invariants 19/19=0 · spec doc §4.1 documents V4 pair as shipped + rejected `view_selection` path · ADR-0012 allowlist has `members.interview_booking_url` entry · cron jobid 47 still pre-fire (or fired naturally if boot happens >Mon 13:00 UTC; check `cron.job_run_details` jobid=47) · `selection_dispatch_url_log` table is EMPTY (no INSERTs yet — Child #2 will start populating it) · `members.interview_booking_url` is NULL for all 73 members (no admin UI yet — Child #3 will land it).
- **Cross-ref**: #348 parent · #354 close issue · SPEC `docs/specs/SPEC_348_BOOKING_URL_PER_EVALUATOR.md` §4.1 (amended) · migration `20260805000029_p250_354_spec_348_v1_foundation_booking_url_per_evaluator.sql` · test `tests/contracts/p250-354-spec-348-v1-foundation.test.mjs` · ADR-0012 contract `tests/contracts/schema-cache-columns.test.mjs` allowlist updated · entry #216 DISPATCH-SPEC-348 (p249 spec source).

### 218. RESOLVED-#355 — SPEC #348 Child #2 RPC body wired (track-aware routing + LRD + dispatch_log audit) (p251)

- **Date**: 2026-05-24 (p251, ~2-3h post-p250 close at boot HEAD `c1e84efc`).
- **Status**: RESOLVED — migration `20260805000030_p251_355_spec_348_v1_rpc_routing.sql` applied live + local file + repair done + shadow row cleaned; **32 new contract assertions** (28 static + 4 DB-gated) all pass; offline baseline 2081/2009/0/72 → **2113/2037/0/76** (+32/+28/0/+4 skip — matches the test exactly); with-DB env 32/32 pass 0 skip; `check_schema_invariants()` 19/19=0; `astro build` ✓ 0 new errors. Independently shippable; routing wires per-track selection while leaving cycle-level URL as researcher-fallback AND leader-only default.
- **Origin**: PM boot directive 2026-05-24 ("Primeiro dispatch: #355 Child #2 RPC body. Preservar assinatura de `notify_selection_cutoff_approved(app_id)`. Antes de editar, auditar `pg_get_function_arguments`/def da função atual. Implementar seleção LRD via `selection_dispatch_url_log`: researcher -> escolher Vitor/Fabricio conforme menor uso recente; leader -> usar committee/cycle URL; registrar URL escolhida no dispatch log; manter fallback seguro para `cycle.interview_booking_url` se policy incompleta, conforme spec. Adicionar testes de contrato e forward-defense. Não iniciar #357 antes de #355 + #356.").
- **Pre-flight state**: HEAD `c1e84efc` (p250 PR #358 squash); `notify_selection_cutoff_approved` lived at the p228_w2_leaf4 body; `proconfig=["search_path=public"]` (single-value, NOT `'public', 'pg_temp'`); `(p_application_id uuid) RETURNS jsonb` SECDEF; ACL `{authenticated=X,service_role=X}`. Cycle4 inventory: 38 apps (34 researcher + 4 leader); 5 with `cutoff_approved_email_sent_at` populated by p243 manual dispatch; `selection_dispatch_url_log` empty (0 rows).
- **SPEC drift mid-session (PM Option A 2026-05-24)**: SPEC §5.1 draft filtered `sc.role = 'researcher'`, but live schema has `selection_committee_role_check CHECK (role IN ('evaluator','lead','observer'))` — committee POSITION, not candidate TRACK. PM Q3 ratification (p249) also said `role='researcher'` for Child #4 seed, which would violate the existing CHECK. Audit of 12 RPCs using `selection_committee.role` showed ALL filter `role='lead'` for committee-lead authority gates (no RPC filters by `evaluator`/`observer`). Live data: `selection_committee.role` distinct values today = `{evaluator,lead,observer}`; `selection_applications.role_applied` distinct values = `{researcher,leader}`. PM picked Option A (filter by position, no schema change): researcher branch filters `role IN ('evaluator','lead') AND can_interview=true`. Child #4 will seed Vitor+Fabricio as `role='evaluator'` (not `'researcher'`). Spec §5.1 query + §8 Child #4 note amended in the same commit as the migration to lock the live-schema interpretation. **SEDIMENT-250.B** (NEW lesson, candidate for escalation if recurs): live-query `pg_constraint` / distinct values BEFORE assuming spec literal SQL strings are valid against current schema; ask via AskUserQuestion at SCOPE-LOCK phase when divergence found.
- **Migration shipped** (`supabase/migrations/20260805000030_p251_355_spec_348_v1_rpc_routing.sql`, ~225 lines incl header):
  - `CREATE OR REPLACE FUNCTION public.notify_selection_cutoff_approved(p_application_id uuid) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public` — **signature locked verbatim per SEDIMENT-238.C** (1 arg, jsonb return, no DEFAULTs; CREATE OR REPLACE preserves ACL).
  - **Preserved verbatim**: committee role='lead' authority probe; can_by_member('manage_member') fallback; idempotency short-circuit (`cutoff_approved_email_sent_at IS NOT NULL → reason='already_sent'`); email null check; `v_objective_done` count; first_name fallback chain; `UPDATE selection_applications SET cutoff_approved_email_sent_at = now()`; admin_audit_log canonical action `selection.cutoff_approved_email_dispatched`; return envelope keys (success / application_id / cycle_id / email_sent / recipient_email_redacted / objective_done / research_score).
  - **NEW v1**: 3 routing locals (`v_resolved_url`, `v_resolution_path`, `v_resolved_evaluator_id`); `IF role_applied='leader' / ELSIF 'researcher' / ELSE defensive` block; researcher LRD picker via `LEFT JOIN LATERAL (SELECT MAX(dispatched_at) AS last_dispatched FROM selection_dispatch_url_log l WHERE l.cycle_id = v_cycle.id AND l.track='researcher' AND l.resolved_evaluator_id = sc.member_id) lrd ON true` + filter `sc.role IN ('evaluator','lead') AND sc.can_interview=true AND COALESCE(sc.interview_booking_url, m.interview_booking_url) IS NOT NULL` + `ORDER BY lrd.last_dispatched NULLS FIRST, sc.member_id LIMIT 1`; precedence via `CASE WHEN sc.interview_booking_url IS NOT NULL THEN 'committee_override' ELSE 'member_global' END`; sub-fallback inside researcher branch when committee empty/null → cycle URL with `cycle_fallback`; defensive ELSE for unknown role_applied → cycle URL; widened gate `IF v_resolved_url IS NULL OR length(trim(v_resolved_url))=0 THEN RAISE 'CUTOFF_NO_BOOKING_URL...'` with ERRCODE P0020 preserved; `INSERT INTO selection_dispatch_url_log (...) VALUES (...)` BEFORE `PERFORM campaign_send_one_off(...)` (audit-before-send semantic); `campaign_send_one_off` `interview_booking_url` variable now receives `v_resolved_url` (was `v_cycle.interview_booking_url`); admin_audit_log metadata gains 3 keys (`resolution_path`, `resolved_evaluator_id`, `role_applied`); `rpc_version` bumped `'p228_w2_leaf4'` → `'p251_355'`; return envelope gains 2 keys (`resolution_path`, `resolved_evaluator_id`).
  - NOTIFY pgrst trailer.
- **Contract test** (`tests/contracts/p251-355-spec-348-v1-rpc-routing.test.mjs`, 32 assertions: 28 static + 4 DB-gated, including **3 forward-defense regressions**):
  - **Static (28)**: file existence + header WHAT/WHY/SPEC DRIFT RESOLVED/ROLLBACK/INVARIANTS + #348/#355/#354 refs + spec doc anchor + CREATE OR REPLACE FUNCTION (not DROP+CREATE — preserves ACL) + signature `(p_application_id uuid) RETURNS jsonb` + LANGUAGE plpgsql + SECURITY DEFINER + SET search_path = public + committee lead authority verbatim + can_by_member manage_member fallback verbatim + idempotency short-circuit verbatim + leader branch sets cycle URL + cycle_fallback + null evaluator + researcher ELSIF + LRD LEFT JOIN LATERAL with MAX(dispatched_at) lateral subquery filtered to cycle/track=researcher/evaluator_id + researcher filter `role IN ('evaluator','lead') AND can_interview = true` + COALESCE committee_override→member_global + CASE-derived resolution_path + ORDER BY lrd.last_dispatched NULLS FIRST + member_id tiebreak LIMIT 1 + sub-fallback IF v_resolved_url IS NULL → cycle URL + defensive ELSE → cycle URL + INSERT INTO selection_dispatch_url_log with 7 mutable columns + track from v_app.role_applied + campaign_send_one_off passes v_resolved_url (not v_cycle.interview_booking_url) + CUTOFF_NO_BOOKING_URL RAISE + ERRCODE P0020 + admin_audit_log gains 3 metadata keys + `rpc_version='p251_355'` (and old `p228_w2_leaf4` ABSENT) + return envelope adds 2 keys (resolution_path + resolved_evaluator_id) preserving original 7 + NOTIFY pgrst.
  - **Forward-defense (3)**: **F1**: migration body MUST NOT contain literal `sc.role = 'researcher'` OR `sc.role = 'leader'` (the SPEC drift class — locks live-schema filter going forward); **F2**: leader branch (between IF role_applied='leader' and ELSIF role_applied='researcher') MUST NOT reference `selection_committee` OR `selection_dispatch_url_log` (PM directive: leader → cycle URL only, no LRD for leader v1 per SPEC §5.3); **F3**: `INSERT INTO selection_dispatch_url_log` MUST appear BEFORE `PERFORM public.campaign_send_one_off` (audit-before-send semantic — email failure with retry could double-dispatch otherwise).
  - **DB-gated (4)**: live RPC signature via `_audit_list_public_function_bodies()` helper (1 overload, identity_args = `p_application_id uuid`, is_secdef true) + live body has v1 branches (role_applied='leader', role_applied='researcher', selection_dispatch_url_log refs, LEFT JOIN LATERAL, rpc_version='p251_355') + selection_dispatch_url_log PostgREST surface readable with all 9 columns + migration version `20260805000030` registered in `supabase_migrations.schema_migrations` via `_audit_list_schema_migrations` RPC.
  - Test added to BOTH `"test"` and `"test:contracts"` whitelists in package.json (SEDIMENT-186.C respected).
- **Live DB post-apply verification**:
  - signature `p_application_id uuid` ✓ (single overload; ACL preserved)
  - `proconfig=["search_path=public"]` preserved ✓
  - `secdef=true` preserved ✓
  - body_hash via `md5(regexp_replace(prosrc, '\s+', ' ', 'g'))` = `6a0c013f9b6e6bceebd953809e128179` (matches local file after fix; see Sediment §3 below)
  - `check_schema_invariants()` 19/19=0 unchanged
  - **Live smoke** via SEDIMENT-226.C JWT-injection (`PERFORM set_config('request.jwt.claims', ..., true)` simulating Vitor) on one of the 5 already-dispatched cycle4 apps: RPC returned `{success:true, email_sent:false, reason:'already_sent', previously_sent_at:'...'}`; `selection_dispatch_url_log` rows unchanged at 0 (idempotency short-circuits before INSERT); zero side effects.
  - `selection_dispatch_url_log` row count: 0 (correct — only new dispatches populate it; the 5 cycle4 sends predate this RPC).
- **SPEC #348 amendments in same commit**:
  - §5.1 query: `AND sc.role = 'researcher'` → `AND sc.role IN ('evaluator', 'lead')  -- p251 #355 PM Option A` (live-schema filter).
  - §8 Q3 ratification: text + section header changed `role='researcher'` → `role='evaluator'`; new explanatory note in §8 Q3 ("p251 #355 correction") documents the rejected literal `role='researcher'` path + cites the existing CHECK constraint as the blocker + clarifies `role_applied` is on `selection_applications` (track) vs `role` on `selection_committee` (position).
- **Sediment**:
  - **0 NEW escalation-eligible sediments**; one **NEW lesson SEDIMENT-250.B** documented above (live-query schema before assuming spec literals; mirrors SEDIMENT-244.A handoff-narrative pattern in the schema-literal domain).
  - **SEDIMENT-238.A recurred** (apply_migration MCP shadow row): cleanup applied. After `apply_migration`, the MCP wrote `version='20260524212026'` (wall-clock UTC) AND my `supabase migration repair --status applied 20260805000030` added the canonical row; ADR-0097 missing-file drift gate caught the shadow. `DELETE FROM supabase_migrations.schema_migrations WHERE version='20260524212026' AND name='p251_355_spec_348_v1_rpc_routing'` resolved it. **Three-step apply ritual reinforced** (per p250's same lesson): (1) write file, (2) repair canonical, (3) DELETE wall-clock shadow.
  - **SEDIMENT-246.B recurred** (apply_migration MCP silently strips inline `--` comments inside `AS $$ ... $$`): caught by Phase C body-hash drift gate (file body 7851 normalized chars vs live 7809 — 42-char diff). Diagnosed via byte-level diff: the line `v_app.role_applied,  -- CHECK aligned: ('researcher','leader')` was stripped to `v_app.role_applied,` in live `prosrc`. Minimum-diff fix per p246 sediment: stripped the trailing inline comment from the local file to match live exactly. Body now `length=8583 bytes / 7809 normalized chars`, byte-for-byte equivalent to `pg_proc.prosrc`. **Lesson reinforced**: avoid trailing inline `--` comments on the SAME line as code inside `AS $$ ... $$` bodies; put them on separate lines if intent must be documented (leading `-- ...` lines are preserved by apply_migration).
  - SEDIMENT-238.C respected (`pg_get_function_arguments` probed BEFORE migration; same 1-arg signature preserved + no DEFAULTs to lose; `CREATE OR REPLACE` used). SEDIMENT-186.C respected (test added to BOTH whitelists pre-`npm test`). SEDIMENT-235.A respected (commit/PR title uses neutral wording — child #355 of #348, not a close trigger for parent). SEDIMENT-244.A applied (live-queried committee_role distinct values + 12-RPC role usage audit BEFORE planning migration body — caught the spec drift at SCOPE-LOCK phase).
- **Carries forward** (some overlap with p250; refreshed):
  1. **#356 Child #3 Admin UI** — unblocked by #354 (column exists). Now also unblocked by #355 (routing is live but accepts NULL member URLs gracefully via cycle_fallback). Can ship in parallel.
  2. **#357 Child #4 Cycle4-2026 committee reseed** — unblocked by #354+#355. PM directive Q3 (post-correction): seed Vitor+Fabricio as `role='evaluator'` (not `'researcher'`); `members.interview_booking_url` stays NULL (PM populates later via Child #3 admin UI). Zero immediate behavior change — committee will exist but no per-evaluator URLs yet, so researcher branch still falls back to cycle URL until Step 2 URLs land.
  3. **WATCH-240.C cron observability** — cron jobid 47 first natural fire still pending **Mon 2026-05-25 13:00 UTC**.
  4. **#243 E2 / #254 D-children** — PM decisions pending (5 + 4 questions, respectively).
  5. **#333 / #334 / #335** — LGPD Wave 4/5 + ADR-0094 (blocked).
  6. **OPP-248.A / OPP-247.A / OPP-245.A / OPP-244.A** — unchanged LOW priority.
- **PM A/B/C/D streak**: PM picked Option A (filter by position, no schema change) on the SPEC §5.1 drift mid-session question → **18 consecutive Recommended**.
- **PR**: pending squash-merge (will be assigned by `gh pr create`).
- **Verify-on-next-boot**: HEAD advances to PR #(p251) merge commit · 32 contract tests pass offline (28 static + 4 DB-gated when env present) · `notify_selection_cutoff_approved` body has v1 routing branches live · `selection_dispatch_url_log` table count stays 0 until a new (non-idempotent) dispatch fires · `check_schema_invariants()` 19/19=0 · spec doc §5.1 + §8 amended per Option A correction · cron jobid 47 first natural fire pending Mon 2026-05-25 13:00 UTC.
- **Cross-ref**: #348 parent · #354 predecessor (foundation, p250) · #355 close issue · #356 successor (admin UI) · #357 successor (committee reseed) · SPEC §5.1 (amended) + §8 Q3 (amended) · migration `20260805000030_p251_355_spec_348_v1_rpc_routing.sql` · test `tests/contracts/p251-355-spec-348-v1-rpc-routing.test.mjs` · entry #217 RESOLVED-#354 (p250 foundation source).
