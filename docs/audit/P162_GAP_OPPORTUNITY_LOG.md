...
- **Sediment**: 0 NEW sediments. SEDIMENT-238.C respected (no CREATE OR REPLACE FUNCTION in this migration so the parameter-DEFAULT preservation lesson doesn't apply directly, but its general principle — "probe live shape BEFORE migration" — was applied via the pre-migration audit of `engagement_kind_permissions` and existing `selection_*` RLS policies before deciding on the V4 canonical pair). SEDIMENT-186.C respected (package.json whitelist pre-`npm test`). SEDIMENT-235.A respected (PR title/body uses neutral wording — child #354 is a child of #348, not a close trigger for the parent). SEDIMENT-244.A respected (live-queried `engagement_kind_permissions` BEFORE planning RLS policy text — caught `view_selection` non-existence at SCOPE-LOCK phase, not at planning phase).
