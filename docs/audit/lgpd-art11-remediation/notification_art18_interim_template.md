# Aviso retroativo LGPD Art. 18 §IV — candidato <CANDIDATE_NAME> (ciclo 4-2026)
...
| `selection_evaluation_ai_suggestions` row (background) | `<AI_SUGGESTION_ID>` (claude-haiku-4-5; `consumed_at=NULL`; não consumida — não influenciou nota humana final) |
...
- ID da candidatura: <APPLICATION_ID>
...
2. PM dispatches the pt-BR text via email from `nucleoia@pmigo.org.br` to `<CANDIDATE_EMAIL>`. EN fallback offered if <CANDIDATE_FIRST_NAME> requests it.
...
6. If <CANDIDATE_FIRST_NAME> requests deletion:
   - PM logs into the organization's Google Workspace as admin and deletes the Drive file `<DRIVE_FILE_ID>` (move to trash or hard-delete; capture confirmation screenshot).
   ...
   - PM replies to <CANDIDATE_FIRST_NAME> confirming the operation, citing the `pii_access_log.id` of the deletion audit row as the chain identifier.
