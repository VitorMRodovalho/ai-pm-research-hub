...
**Decisão:** Consolidar a auditoria de arquitetura de permissionamento (Onda 2, achados F1–F9 do plano `~/.claude/plans/onda-2-auditoria-keen-kahn.md`) como **evolução do V4** (RBAC + ABAC já existente), **sem** criar tabela por-usuário `tier × tag × função`. As decisões institucionais:

1. **Sponsor read-only de fato (FU-1 — ADR-0110, mig `20260805000291`).** A designação `sponsor` deixa de conceder escrita: o seed `sponsor×sponsor` perde `manage_finance` + `manage_partner` e ganha as novas actions de **leitura** `view_finance` + `view_partner` (split read/write). Descoberta-chave: `manage_*` gateava leitura *e* escrita (~12 RPCs + 6 gates MCP) — um `DELETE` ingênuo regrediria a leitura; por isso o split, não a revogação simples. Writes permanecem com `manage_*` para quem é GP/liaison.
2. **Auditor institucional agregado (FU-3 — ADR-0111, mig `20260805000292`).** Novo `engagement_kind` `institutional_auditor` (role `auditor`) + nova action dedicada `view_aggregate_analytics`, seedada **apenas** a esse par e honrada por **8 RPCs agregadas zero-PII/zero-escrita** (allowlist por construção). Criado **dormante** (0 engagements) para a apresentação do LIM. Auditoria ao vivo derrubou a premissa original de "reusar `view_internal_analytics` agregado": essa action abre 37 RPCs com PII individual + 5 escritas — daria a um parceiro externo o diretório de PII. A allowlist estreita substitui o seed amplo.
3. **Assimetria de visibilidade federada formalizada (FU-4 — este registro).** Três eixos distintos: **sede** PMI-GO + GP (visão consolidada, controladora) · **capítulo parceiro** (agregado do próprio capítulo, controlador dos próprios filiados — Anexo R3 #641) · **auditor institucional externo** (agregado program-wide, sem PII, sem recorte nominal — não é capítulo parceiro). Documentado em `docs/legal/INSTITUTIONAL_AUDITOR_COOPERATION_AND_PROVISIONING.md` + Anexo R3 #641 §5.3.
4. **SLA de reassignment function-anchored (FU-4).** Autoridade atada ao **cargo**, nunca ao indivíduo: na troca de presidência de capítulo, o engagement `sponsor` é **revogado do titular anterior e reassignado ao novo em ≤ 15 dias úteis da posse** (prazo a confirmar com DPO/governança). Aplica-se à sede e aos parceiros.
5. **`end_date` obrigatório em engagements institucionais (FU-4).** Para `institutional_auditor` é **CHECK no banco** (`engagements_institutional_auditor_end_date_check`: `kind <> 'institutional_auditor' OR end_date IS NOT NULL`). Para `sponsor` é disciplina operacional (recomenda-se `end_date` = fim do mandato).
6. **Gate de provisionamento do auditor (FU-4 — pré-requisito de governança).** O tier técnico está vivo e behavior-neutral, mas **provisionar** o primeiro auditor é GP-only e exige, ANTES do 1º `INSERT`: (P1) acordo/memo de cooperação formal com o órgão externo; (P2) ciência formal dos 4 capítulos parceiros (ata/e-mail — template no doc); (P3) ratificação do RoPA/LIA pelo DPO; (P4) `end_date` + pessoa nomeada. ⚠️ `auto_expire_behavior='notify_only'` → o acesso não é auto-revogado no vencimento; o GP **revoga manualmente** no/antes do `end_date` (revisão trimestral). Checklist de go-live no doc.

A Onda 2 também incluiu (PRs #951/#953/#954, ADR-0109): correção da ladder "governança vence" (`chapter_board → chapter_liaison` acima de `researcher`/`observer`), `admin.full → system.global_config`, `voluntariado_director` sede program-wide, recusa de COI presidente-candidato na seleção, e o chapter-scope das RPCs de PII de membro (FU-2). Esta entrada (FU-4) fecha a **camada de governança/processo** da Onda 2.

**Justificativa:** Minimização LGPD (parceiro externo recebe só agregado não identificável; PII individual nunca sai); separação **read ≠ write** (tornar um papel read-only exige split, não revoke — armadilha de action que gateia ambos); autoridade **function-anchored** (sobrevive à rotação de cargos — o bug Ivan da Onda 1 foi designação sem rank na ladder); accountability institucional para o LIM **sem** expor dado pessoal. Tabela por-usuário foi rejeitada por unanimidade do conselho (regressão "Path-4", quebra o invariante function-anchored, cria drift com `engagement_kind_permissions`).

**Impacto técnico:** Migrations `20260805000291` (FU-1) e `20260805000292` (FU-3); ADR-0110, ADR-0111 (+ ADR-0023 Amendment C paridade A3, ADR-0109 COI); docs legais novo `INSTITUTIONAL_AUDITOR_COOPERATION_AND_PROVISIONING.md` (RoPA/LIA + protocolo de provisionamento + template de ciência dos parceiros) e Anexo R3 `641_MANUAL_R3_DATA_PROTECTION_ANNEX_DRAFT.md` §5.3; `PERMISSIONS_MATRIX.md` (persona `institutional_auditor`); testes de contrato `institutional-auditor-aggregate-scope` + `role-ladder-parity`. Action `view_aggregate_analytics` honrada por exatamente 8 RPCs (`get_cycle_report`, `get_annual_kpis`, `get_selection_pipeline_metrics`, `get_diversity_dashboard`, `get_portfolio_items`, `get_in_dashboard`, `get_comms_to_adoption_funnel`, `exec_role_transitions`).
