...
- `ADR-0018-mcp-threat-model.md` — Análise de risco do `nucleo-mcp` (EF + Cloudflare proxy) contra vulnerabilidade MCP reportada em abril/2026. Vetores aplicáveis vs não-aplicáveis, mitigações canônicas (OAuth 2.1, CSRF middleware, canV4 gate). **Status: Partially Accepted (2026-04-24 p44+T) — D1-D5 análise + convenções aceitas; **W1 confirmation step shipped (MCP v2.24.0, p44)** (5 destructive tools default to preview, confirm=true executes); **W3 prerequisite `mcp_usage_log.result_kind` shipped (MCP v2.24.1 + migration `20260511010000`, p44 Track T)**; W2 rate limit + W3 anomaly cron ainda pendentes.**
