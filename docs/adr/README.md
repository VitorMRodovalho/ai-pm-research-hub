...
- `ADR-0077-auth-org-caller-derived-contract.md` — `auth_org()` caller-derived semantics post-p136 rewrite: active member → org, ghost/inactive/service_role → NULL; fail-closed posture; Group A (strict, financial) vs Group B (permissive, ops logs) per-table NULL handling; 30+ V4 tables blast radius. **Status: ACCEPTED (2026-05-10 p138).**
- `ADR-0078-external-reviewer-onboarding-pattern.md` — External reviewer (advogada voluntária, peer-reviewer) onboarding pattern. Schema layer já shipped p130 (engagement_kind + permission); ADR spec'a camada user-facing: invite RPC + magic-link auth + `/external/governance/[chainId]` landing + ReviewChainIsland variant `external` + NDA-lite agreement + revoke + audit. Sucessor estratégico do DOCX export workflow (p128 D2). **Status: PROPOSED Round 2.5 (2026-05-10 p148) — PM 5/5 sign-off + legal-counsel parecer incorporado (`docs/council/decisions/2026-05-10-p148-legal-counsel-adr0078-external-reviewer-nda.md`); aguardando Ângelina curation + Ivan DPO sign-off para Round 3 (ACCEPTED).**
- `ADR-0079-subjective-scoring-via-video-transcription.md` — Subjective scoring via video transcription (4th AI frontier para selection): pipeline transcript → score qualitativo + LGPD Art. 20 safeguards. **Status: PROPOSED · AWAITING-RATIFICATION (council 4-lens review concluído; 11 blockers + 10 decisões consolidadas; 4 human gates outstanding antes de flip → ACCEPTED — ver §11 Acceptance Gate).**
- `ADR-0080-v4-engagement-canonical-deprecate-members-initiative-id.md` — V4 engagement como canonical: deprecar `members.initiative_id` em favor de N:N `engagements`. Plano em Fase A (frontend cutover) → Fase B (invariant + 7d shadow) → Fase C (drop column). Resolve V3/V4 hybrid design documentado em `memory/feedback_members_initiative_id_v3_v4_hybrid_design.md` (p155). **Status: PROPOSED (2026-05-14 p159 Sessão #3) — pending PM sign-off + Phase A/B/C cutover.**
...
- `ADR-0085-exec-cross-initiative-comparison-metric-scoping.md` — `exec_cross_initiative_comparison` metric scoping contract: §1 event-derived metrics strict-scoped via `ev.initiative_id = i.id` (total_hours + members_inactive_30d + meetings_count + last_meeting_date + days_since_last_meeting); §2 attendance_rate retém NULL fallback para research_tribes (intentional V3-inheritance asymmetry); §3 XP cohort-scoped por limitação de schema (`gamification_points` sem initiative_id — backlog GAP-194.B); §4 cards/members scoped via parent table. Closes LOW-194.E rate/hours asymmetry doc. **Status: Accepted (2026-05-18 p195) — codifica decisões PM Option B GAP-192.C + Option A GAP-194.A.**
- `ADR-0086-curation-manual-structured-review-pattern.md` — Curation pipeline alinhado ao Manual §4.2: §1 FSM canônica `draft→peer_review→leader_review→curation_pending→published`; §2 peer review COLEGIADO (sem reviewer_id; feedbacks via card_comments); §3 leader review NOMINAL (leader_reviewer_id FK + decision approved/returned/waived + notes); §3.6 OPP-197.D decisão PM — leader pode chamar de draft (implicit review aceita); §4 waiver path com motivo obrigatório; §5 distinct lifecycle actions (peer_review_completed + leader_review_completed) para analytics safety; §6 authority hierarchy 4-tier V4 (assignee/assignments/engagement leader/governance reviewer); §7 trigger safety-net pattern; §8 canonical button via submit_for_curation; §9 notification semantics contract (source_type literal); §10 recalculate_card_dates contract. Closes GAP-197.A. **Status: Accepted (2026-05-18 p196/p197/p198) — caso Débora end-to-end LIVE, 8 migrations + council fix bundle.**
- `ADR-0087-v4-curate-content-action.md` — V4 `curate_content` action substitui V3 `'curator' = ANY(designations)` gate em 14 fns. §1 seed `committee_coordinator × {coordinator, leader} + observer.curator` scope='organization'; §2 sweep agrupado em 4 batches (A board, B cert, C governance, D _can_sign_gate + assign_curation_reviewer); §3 8 fns "data ref" não migradas (curator é projeção/enum, não auth gate); §4 rollback per-batch via git revert; §5 não-decisões (co_gp/tribe_leader/communicator continuam V3 — backlog separado). Migrations `20260519182455` seed + `20260519182828` scope-fix + `20260519183234`/`183819`/`183955`/`184046` batches A-D. Closes OPP-196.E. **Status: Accepted (2026-05-19 p200) — 14/14 fns V4-swapped, invariantes 16/16=0, 3 curators parity preserved.**
- `ADR-0088-gamification-points-initiative-scoping.md` — Adiciona `initiative_id uuid REFERENCES initiatives(id)` em `gamification_points` + backfill via `events.initiative_id` + trigger forward-strategy; permite strict-scope XP em `exec_cross_initiative_comparison` (resolve ADR-0085 §3 cohort-scoping carry). Scaffold issue #166 (p202). **Status: Proposed (2026-05-19 p202) — implementation deferida; PM Q1 (NULLable forever vs eventual NOT NULL).**
- `ADR-0089-champion-criteria-catalog-semantics.md` — Formaliza `champion_criteria_catalog` table + 3 CRUD RPCs (create/update/archive) + audit em `admin_audit_log` + authority `committee_coordinator × {coordinator,leader} + manage_platform` (baseline). Substitui hardcoded criteria por catalog read. Cross-ref ADR-0081/0084. Scaffold issue #166 (p202). **Status: Proposed (2026-05-19 p202) — PM Q2 (authority scope catalog edit).**
- `ADR-0090-effective-cycle-bounds-helper.md` — Single canonical "active in cycle" semantic via VIEW `effective_cycle_bounds` (member × initiative × cycle com effective_start_date/end_date + bound_state open/closed_in_cycle/pre_cycle) + rollup view `member_effective_cycles`. RPCs migram opt-in. Scaffold issue #166 (p202). **Status: Proposed (2026-05-19 p202).**
- `ADR-0091-tribe-bridge-remaining.md` — Decisão final sobre C2/C4 ADR-0015: Option A (accept bridge permanent) / Option B (sweep C4 drop members.tribe_id + tribes) / Option C (partial: drop members.tribe_id + tribes vira view, history mantém tribe_id). Recommended Option C. Scaffold issue #166 (p202). **Status: Proposed (2026-05-19 p202) — PM Q3 decision A/B/C required.**
- `ADR-0092-document-permissions-v4-sweep.md` — Sweep das 3 document_* RLS policies V3 (operational_role + chapter_board/chapter_witness designations) para V4 actions seedadas em engagement_kind_permissions. Shadow window 48-72h + smoke 22 governance MCP tools. Closes ADR-0087 §5 carry + audit item #29. Scaffold issue #166 (p202). **Status: Proposed (2026-05-19 p202) — PM Q4 (deprecation horizon V3 policies).**
- `ADR-0093-canonical-rpc-facade-approval-orchestration.md` — Padrão "canonical RPC as facade": para subsistemas com lifecycle transitions críticas (≥4 side-effects + ≥2 entry points + drift = invariant violation + side-effects acopladas), uma RPC canonical concentra a orquestração e todos os entry points (UI/MCP/bulk/cron/REST) DELEGAM. Pareado com schema invariants que detectam bypass. Catalog inicial: volunteer approval (`approve_selection_application` / 2 facades / R+S watchdogs). Cross-ref ADR-0007/0011/0028/0080. Pairs com PR #198 + PR #199. Closes GAP-204.A. **Status: Accepted (2026-05-20 p204) — padrão JÁ implementado live em produção.**
- `ADR-0094` — *(numbering reservado; sem arquivo associado. Numbering jump intencional de 0093 → 0095, confirmado 2026-05-22 audit p223: nenhuma referência em docs/src/supabase nem em git history.)*
- `ADR-0095-member-alternate-emails.md` — Member alternate emails: `member_emails` table (1:N) + 3 kinds (personal/institutional/chapter/other) + sync trigger mantém `members.email` ↔ primary; 6 MCP tools (resolve/list/add/remove/set_primary/update_kind); SECDEF gates: self / `manage_member` / `view_pii` (list); invariant T garante exactly-one-primary. Migrations `20260802000008`/`009`/`010`/`011`/`013`. Live: 73 members → 73 primary rows backfilled. **Status: Accepted (2026-05-21 p213-p216 — 4-PR sequence shipped end-to-end with Council Tier 1 GO+AMENDMENTS).**
- `ADR-0096-impact-hours-total-view-accepted-risk.md` — `impact_hours_total` view: aceitação formal do risco advisor (`security_definer_view` ERROR). Sibling do ADR-0024 (mesmo pattern, conteúdo agregado escalar sem PII). 5 callsites preservados sem refactor; `anon` já REVOKE'd; threat surface materialmente menor que `public_members`. Migration `20260805000002` atualiza COMMENT ON VIEW referenciando este ADR + preserva rationale p170 BUG-HOI. **Status: Accepted (2026-05-22 p223 audit MED #10 close).**
