...
- `ADR-0028-service-role-bypass-adapter-pattern.md` — Padrão canônico V4 para 37 fns (30 admin_* clean Batch 1 + 7 extended Batch 2) chamáveis via cron/EF (service_role) E por usuários admin. Adapter pattern: `IF auth.role()='service_role' THEN bypass; ELSE can_by_member(...)`. Closes "29 service-role-bypass" backlog. Enforcement em 4 camadas (allowlist + size guard + COMMENT sentinel + invariant G novo). Conceitualmente "ADR-0011 Amendment C" mas standalone para discoverability. **Status: Accepted (ratified 2026-04-26 p64 via Opção A council-validated).**
