...
- `ADR-0018-mcp-threat-model.md` — Análise de risco do `nucleo-mcp` (EF + Cloudflare proxy) contra vulnerabilidade MCP reportada em abril/2026. Vetores aplicáveis vs não-aplicáveis, mitigações canônicas (OAuth 2.1, CSRF middleware, canV4 gate). **Status: Partially Accepted (2026-04-24 p44+T+W3) — D1-D5 análise + convenções aceitas; **W1 confirmation step shipped (MCP v2.24.0, p44 Track R)** (5 destructive tools default to preview); **W3 anomaly detection cron shipped (migration `20260511020000`, pg_cron `mcp-anomaly-detection-15min`, p44 Track W3)** (4 patterns: burst_execute / canv4_enumeration / destructive_burst / preview_without_execute, alerts em admin_audit_log); W2 rate limit (Cloudflare KV counters) ainda pendente.**
