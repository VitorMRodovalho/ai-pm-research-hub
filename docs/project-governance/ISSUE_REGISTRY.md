...
**Last updated:** 2026-05-23 (p233 — #321 Gap A closed: AFTER INSERT trigger on certificates flips matching onboarding_progress.volunteer_term to completed + 30 phantom rows backfilled live; next #322 (Gap B classification + forward guard, now unblocked))
...
|---|---|---|---|
| Selection reliability Cycle 4 | #292, #251, #298(closed), #260(qa-window), #116(closed), #318(closed), #179(closed), #229(closed), #230(umbrella) + #321(closed)/#322/#323 children, #243, #254 | P0 sequencing program | #251 audit + #298 fix + #260 W2 ALL 7 LEAVES SHIPPED + replay live (p228) — qa-window for 7d production smoke only. #116 calendar webhook smoke PASS + closed p229. #318 A3 invariant fixed p230 (PR #320). **p230 close #179 + reframe #230**: #179 closed as IMPLEMENTED (5/5 ACs met live; canonical RPC + delegating legacy paths + invariants R/S/A3 + contract test). #230 reframed into umbrella with 3 ready-leaf children — #321 phantom-row sync trigger, #322 classification leftovers + forward guard, #323 catalog config. **p232 #229 Phase 2 closed**: leader_extra cohort visibility in 3 read RPCs (get_pert_cutoff_summary dual-track CHECK + math; get_application_score_breakdown adds leader_extra_cutoff block; get_selection_dashboard.cycle exposes sibling block); MCP v2.79.1 with `leader_extra_pert_score` in z.enum + dual-dim descriptions; frontend `/admin/selection` 2-band chip; +12 contract tests; live smoke 3/3 PASS (method=disabled expected — cohort_n=6 < 10). **p233 #321 Gap A closed**: migration `20260805000018` installs AFTER INSERT trigger on certificates with `WHEN (NEW.type='volunteer_agreement')` that atomically flips matching onboarding_progress.volunteer_term to status=completed using NEW.issued_at (idempotent via status!=completed guard on unique (member_id, step_key) index); in-tx backfill of 30 phantom rows via DISTINCT ON latest-cert-per-member CTE; sanity DO block RAISES EXCEPTION on remaining phantom (fails apply at write time). 12 contract tests (11 static + 1 DB-gated). Live smoke: 0 phantom rows post-apply; 30 backfill audit rows; invariants 19/19=0. Next p233+ dispatch order: **#322 (Gap B classification leftovers + forward guard in `approve_selection_application` + offboarding extension; now unblocked by #321 close)**, then #323 (Gap C catalog config: PM decision per kind on `study_group_*`); keep #243/#254 spec/read-only behind #221/#218 |
...
| Volunteer lifecycle | #177, #179(closed), #180, #181, #182, #183, #205, #213, #230(umbrella) + #321(closed)/#322/#323 | Foundation sequence | Start after #221 containment; #179 closed as IMPLEMENTED p230 (canonical contract gate shipped); #230 reframed into umbrella with 3 ready-leaf children — #321 sync trigger closed p233 (trigger + backfill live, invariants 19/19=0), now #322 unblocked (classification + guard), then #323 catalog config |
