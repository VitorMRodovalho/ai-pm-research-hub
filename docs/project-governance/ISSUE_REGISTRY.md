...
**Last updated:** 2026-05-23 (p234 — #322 Gap B closed: 8 phantom-pending vol_term rows backfilled to status=skipped (reason=no_requires_agreement_engagement) + forward guard in approve_selection_application + offboarding extension in admin_offboard_member auto-skips open vol_term + get_my_onboarding harmonizes skipped≡completed; next #323 (Gap C catalog config))
...
| #322(closed) | closed | Foundation / Governance / Data-integrity | none | none | p234 — migration `20260805000019` (1) backfills 8 phantom-pending vol_term rows (4 active + 4 inactive — none with cert, none with active requires_agreement engagement) to `status='skipped'` with metadata `reason='no_requires_agreement_engagement'`, `completed_via='p234_322_backfill_no_agreement_path'`, `migration='20260805000019'` (PM directive 2026-05-23 — NEVER `completed` unless cert exists or real signing event); (2) forward guard in `approve_selection_application` Path A SELECT adds `AND NOT (s.id = 'volunteer_term' AND NOT COALESCE(v_requires_agreement, FALSE))` — forward-defense for any future dynamic engagement_kind change; (3) offboarding extension in `admin_offboard_member` auto-skips open vol_term steps with metadata `reason='offboarded_pre_signing'`, idempotent via `status='pending'` filter, respects #321 trigger ordering; (4) `get_my_onboarding` harmonizes `skipped`≡`completed` for `completed_steps` + `all_complete` (mirrors get_onboarding_status pattern). 17 contract test assertions (16 static + 1 DB-gated). Sanity DO block RAISES EXCEPTION on remaining goal-metric violations. Live smoke: goal metric = 0 active members with pending vol_term and no active requires_agreement engagement; 8 backfill audit rows present; invariants 19/19=0. | Closed 2026-05-23 (p234); no re-open unless forward guard fails to prevent new Gap B occurrences (audit goal metric ratchet against 0 going forward) |
