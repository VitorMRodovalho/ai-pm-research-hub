...
**Last updated:** 2026-05-23 (p234 — #322 Gap B closed via PR #327 (2 migrations: 20260805000019 initial + 20260805000020 close-review patch): 8 phantom-pending vol_term rows backfilled to status=skipped (reason=no_requires_agreement_engagement) + forward guard in approve_selection_application + offboarding extension in admin_offboard_member auto-skips open vol_term + get_my_onboarding harmonizes skipped≡completed AND mirrors forward guard in auto-seed (PM curator caught the unguarded auto-seed re-entry vector before merge); next #323 (Gap C catalog config))
...
| #230 | umbrella | Foundation / Governance / QA | volunteer lifecycle reliability after selection approval | #323 (only remaining child to ship) | p230 audit refuted original Herlon-specific premise (his engagements don't require agreement; A3 fixed by #318/#320). Reframed into 3 narrow children covering actual gaps: #321(closed p233) phantom onboarding rows, #322(closed p234) classification leftovers + forward guard + offboarding extension + UX harmonize, #323 catalog config (`study_group_*` requires_agreement+null template) | Close when #323 ships; do NOT mint a Herlon term |
...
|---|---|---|---|
| Selection reliability Cycle 4 | #292, #251, #298(closed), #260(qa-window), #116(closed), #318(closed), #179(closed), #229(closed), #230(umbrella) + #321(closed)/#322(closed)/#323 children, #243, #254 | P0 sequencing program | #251 audit + #298 fix + #260 W2 ALL 7 LEAVES SHIPPED + replay live (p228) — qa-window for 7d production smoke only. #116 calendar webhook smoke PASS + closed p229. #318 A3 invariant fixed p230 (PR #320). **p230 close #179 + reframe #230**: #179 closed as IMPLEMENTED (5/5 ACs met live; canonical RPC + delegating legacy paths + invariants R/S/A3 + contract test). #230 reframed into umbrella with 3 ready-leaf children — #321 phantom-row sync trigger, #322 classification leftovers + forward guard, #323 catalog config. **p232 #229 Phase 2 closed**: leader_extra cohort visibility in 3 read RPCs (get_pert_cutoff_summary dual-track CHECK + math; get_application_score_breakdown adds leader_extra_cutoff block; get_selection_dashboard.cycle exposes sibling block); MCP v2.79.1 with `leader_extra_pert_score` in z.enum + dual-dim descriptions; frontend `/admin/selection` 2-band chip; +12 contract tests; live smoke 3/3 PASS (method=disabled expected — cohort_n=6 < 10). **p233 #321 Gap A closed** (PR #326): migration `20260805000018` AFTER INSERT trigger on certificates + 30-row backfill + 12 contract tests + sanity DO block + live smoke 0 phantom + invariants 19/19=0. **p234 #322 Gap B closed (via PR #327, 2 migrations)**: **Initial migration `20260805000019`** — (1) backfills 8 phantom-pending vol_term rows (4 active + 4 inactive — no cert, no active requires_agreement engagement) to `status='skipped'` with `metadata.reason='no_requires_agreement_engagement'` (PM directive — NEVER `completed` unless cert exists); (2) forward guard in `approve_selection_application` Path A; (3) offboarding extension in `admin_offboard_member` with `metadata.reason='offboarded_pre_signing'`; (4) `get_my_onboarding` harmonizes `skipped`≡`completed`. **Close-review patch `20260805000020`** — PM curator review of PR #327 caught that `get_my_onboarding()` universally auto-seeded all `onboarding_steps` on first call, reintroducing Gap B for any future member whose first hit lands there. Patch declares `v_has_req_agreement_engagement boolean` (EXISTS on engagements+engagement_kinds: status=active + requires_agreement=true) and adds `AND NOT (s.id='volunteer_term' AND NOT v_has_req_agreement_engagement)` to the auto-seed INSERT — mirrors approve_selection_application guard. 20 contract tests (19 static + 1 DB-gated). Sanity DO block RAISES EXCEPTION on remaining goal-metric violations. Live smoke: goal metric = 0 active members with pending vol_term and no active requires_agreement engagement; 8 backfill audit rows present; invariants 19/19=0. Next p234+ dispatch: **#323 (Gap C catalog config — PM per-kind decision on `study_group_owner`/`study_group_participant` requires_agreement+null template)**; keep #243/#254 spec/read-only behind #221/#218 |
...
| Volunteer lifecycle | #177, #179(closed), #180, #181, #182, #183, #205, #213, #230(umbrella) + #321(closed)/#322(closed)/#323 | Foundation sequence | Start after #221 containment; #179 closed as IMPLEMENTED p230 (canonical contract gate shipped); #230 reframed into umbrella with 3 ready-leaf children — #321 sync trigger closed p233 (trigger + backfill live, invariants 19/19=0), #322 closed p234 (8-row backfill + forward guard + offboarding extension + UX harmonize, goal metric=0 live), now #323 catalog config remains |
