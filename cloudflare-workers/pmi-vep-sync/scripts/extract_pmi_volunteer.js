...
//        Auto-discovery continua listando tudo do recruiter, mas vagas
//        FORA da allowlist só entram na varredura com confirmação
//        explícita (modal). Minimização LGPD: dados de candidatos de
//        vagas alheias ao Núcleo nem sequer são baixados por default.
//   W2 — nota de minimização LGPD no header do JSON gerado (meta.lgpd).
//   W3 — auto-POST /ingest é PULADO quando NUCLEO_INGEST_SECRET está
//        vazio ou é o placeholder <...> (antes: POSTava o placeholder e
//        recebia {error:"unauthorized"} — o aviso "Phase A unauthorized"
//        dos imports; ver #224 e #1175 Wave 4). Fluxo normal continua:
//        baixar o JSON e subir via admin UI (/admin → Import VEP JSON).
//   W4 — serviceHistory rows agora carregam applicationId + roleName,
//        casando com o contrato do worker (mapServiceHistory filtrava
//        por applicationId e lia roleName; o script só emitia
//        applicantId/title/roleTitle → 0 inserts silenciosos desde
//        2026-05-12). Phase B também re-emite rows para aplicações
//        subsequentes do mesmo applicant (dual-track).
//
// CHANGELOG p131:
...
(async () => {
  ...
  const CONFIG = {
    ...
    // Wave 4 W1 (#1175 F7) — allowlist das vagas do Núcleo. Vagas
    // auto-descobertas FORA desta lista exigem confirmação explícita no
    // modal antes de terem candidaturas baixadas (minimização LGPD:
    // vaga de diretoria de capítulo, ex. 72562, não é do Núcleo e o
    // worker /ingest a skiparia de qualquer forma — opportunity_not_active).
  ...
  async function runPhaseB() {
    ...
    // Wave 4 W4 (#1175 F7): shape de row casando com o contrato do worker
    // (mapServiceHistory filtra por applicationId e lê roleName). Campos
    // legados (applicantId/title/roleTitle) preservados para os CSVs.
    const histRow = (a, h) => ({
    ...
    for (const a of apps) {
      ...
      if (profileCache.has(cacheKey)) {
        ...
        Object.assign(a, c.flat);
        // Wave 4 W4: dual-track — re-emite as rows de history para ESTA
        // application (o worker faz match por applicationId).
        for (const h of (c.hist || [])) serviceHistoryRows.push(histRow(a, h));
        continue;
      }
      stats.total++;
      const cached = { flat: {} };
      ...
      if (r2?.isSuccess && Array.isArray(r2.result?.volunteerHistory)) {
        ...
        cached.hist = hist;   // Wave 4 W4 — para re-emitir em cache-hit (dual-track)
        ...
        for (const h of hist) serviceHistoryRows.push(histRow(a, h));
      }
      await sleep(200);
      ...
      if (profileOk) {
        stats.ok++;
        // Wave 4 W4 — timestamp de enrichment (worker usa como captured_at /
        // pmi_data_fetched_at; antes só o phaseB.enrichedAt global existia).
        cached.flat.pmiDataFetchedAt = new Date().toISOString();
      }
    ...
    if (serviceHistoryRows.length) {
      const histCols = ['applicationId','applicantId','applicantName','applicantEmail','roleId','title','roleTitle','roleName',
  ...
  if (allowlistSet.size) {
    ...
    if (CONFIG.OPPORTUNITY_IDS && unknownIds.length) {
      // Lista manual = confirmação explícita; só avisa.
      console.warn(`⚠️ [W1] OPPORTUNITY_IDS manual contém ${unknownIds.length} vaga(s) fora da allowlist do Núcleo: ${unknownIds.join(', ')} — prosseguindo por serem explícitas.`);
    } else if (unknownIds.length) {
      const oppById = new Map(opportunityRows.map(o => [String(o.opportunityId), o]));
      const chosen = await new Promise((resolve) => {
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,0.65);display:flex;align-items:center;justify-content:center;font-family:system-ui,-apple-system,Segoe UI,sans-serif';
        const rows = unknownIds.map(id => {
          const o = oppById.get(String(id));
          const label = o ? `${id} — ${o.name} (${o.chapterName}, ${o.numberOfApplications ?? '?'} apps)` : String(id);
          return `<label style="display:block;text-align:left;font-size:13px;margin:6px 0;color:#333"><input type="checkbox" class="__pmi_unk" value="${id}" style="margin-right:8px">${label}</label>`;
        }).join('');
        const card = document.createElement('div');
        card.style.cssText = 'background:#fff;padding:28px 36px;border-radius:12px;max-width:680px;box-shadow:0 24px 64px rgba(0,0,0,0.45)';
        card.innerHTML = `
          <h2 style="margin:0 0 12px;color:#b35c00;font-size:20px;font-weight:700">⚠️ Vagas fora da allowlist do Núcleo</h2>
          <p style="margin:0 0 12px;color:#333;font-size:14px;line-height:1.5">
            A auto-descoberta encontrou ${unknownIds.length} vaga(s) que não estão na allowlist
            (${[...allowlistSet].join(', ')}). Por minimização LGPD, candidaturas dessas vagas
            <strong>não serão baixadas</strong> a menos que você as marque explicitamente.</p>
          ${rows}
          <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:20px">
            <button id="__pmi_only_allow" style="background:#0a7d2c;color:#fff;border:0;padding:10px 24px;border-radius:6px;cursor:pointer;font-size:14px;font-weight:600">Só allowlist (recomendado)</button>
            <button id="__pmi_include_sel" style="background:#fff;color:#b35c00;border:1px solid #b35c00;padding:10px 20px;border-radius:6px;cursor:pointer;font-size:14px">Incluir marcadas</button>
          </div>`;
        overlay.appendChild(card); document.body.appendChild(overlay);
        card.querySelector('#__pmi_only_allow').addEventListener('click', () => { overlay.remove(); resolve([]); });
        card.querySelector('#__pmi_include_sel').addEventListener('click', () => {
          const sel = [...card.querySelectorAll('.__pmi_unk:checked')].map(cb => cb.value);
          overlay.remove(); resolve(sel);
        });
      });
      const keep = new Set([...allowlistSet, ...chosen.map(String)]);
      for (const id of opportunityIds) {
        if (!keep.has(String(id))) excludedOpportunities.push(String(id));
      }
      opportunityIds = opportunityIds.filter(id => keep.has(String(id)));
      // Remove também as rows das excluídas (não vão pro JSON/CSV)
      for (let k = opportunityRows.length - 1; k >= 0; k--) {
        if (!keep.has(String(opportunityRows[k].opportunityId))) opportunityRows.splice(k, 1);
      }
      if (excludedOpportunities.length) {
        console.log(`🛡️ [W1] ${excludedOpportunities.length} vaga(s) excluída(s) da varredura (minimização LGPD): ${excludedOpportunities.join(', ')}`);
      }
      if (chosen.length) {
        console.warn(`⚠️ [W1] Vagas fora da allowlist INCLUÍDAS por confirmação explícita: ${chosen.join(', ')} (lembrete: o worker /ingest só processa vagas registradas em vep_opportunities com is_active=true).`);
      }
    }
  }
  meta.opportunityIds = opportunityIds;
  meta.excludedOpportunityIds = excludedOpportunities;
  // Wave 4 W2 (#1175 F7) — nota de minimização LGPD gravada no header do JSON
  meta.lgpd = {
    minimization_note:
      'Extração restrita às vagas do Núcleo IA & GP (allowlist ' +
      [...allowlistSet].join('/') +
      '). Vagas fora da allowlist exigem confirmação explícita e ficam registradas em ' +
      'meta.excludedOpportunityIds quando excluídas. Dados pessoais coletados (perfil PMI ' +
      'Community + candidaturas VEP) destinam-se exclusivamente ao processo seletivo e à ' +
      'gestão de voluntários do Núcleo (base legal: consentimento no fluxo de onboarding; ' +
      'LGPD art. 6º III — minimização). Não redistribuir este arquivo; importar via admin ' +
      'UI e apagar cópias locais após o import.',
    allowlist: [...allowlistSet],
    excluded_opportunity_ids: excludedOpportunities,
  };
    ...
    for (const a of applications) {
      ...
      if (wantsEnrichment && !communityEnrichmentBlocked && a.applicantId) {
        ...
        if (cached.history) {
          ...
          // Rows planas para o CSV relacional + ingest.
          // Wave 4 W4 (#1175 F7): applicationId + roleName são o contrato do
          // worker (mapServiceHistory); legados mantidos para os CSVs.
  ...
  // Wave 4 W3 (#1175 F7): secret placeholder (<...>) ou vazio = NÃO tenta o
  // POST. Antes o placeholder era truthy, o script POSTava e recebia
  // {error:"unauthorized"} — que ficava gravado em ingestResult no JSON e
  // aparecia como aviso "Phase A unauthorized" no import via admin UI (#224).
  const ingestSecretUsable =
    !!CONFIG.NUCLEO_INGEST_SECRET &&
    !/^\s*<.*>\s*$/.test(CONFIG.NUCLEO_INGEST_SECRET);
  let ingestResult = null;
  if (CONFIG.NUCLEO_INGEST_URL && CONFIG.NUCLEO_INGEST_SECRET && !ingestSecretUsable) {
    console.log(`\n⏭️ [W3] NUCLEO_INGEST_SECRET é placeholder — pulando auto-POST. Fluxo normal: suba o JSON baixado via admin UI (/admin → Import VEP JSON), que injeta o secret server-side.`);
  }
  if (CONFIG.NUCLEO_INGEST_URL && ingestSecretUsable) {
