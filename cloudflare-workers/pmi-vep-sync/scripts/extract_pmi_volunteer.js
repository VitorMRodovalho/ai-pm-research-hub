...
//   VOLUNTEER-ONLY: a Fase B (runPhaseB, community.pmi.org) foi REMOVIDA. Fluxo agora é
//        1 fase: rodar no volunteer.pmi.org → o pmi_volunteer_full_<data>.json já sai
//        enriquecido (Seção 3c) → importar direto. As flags FETCH_PROFILE_* (community
//        inline 3b) ficam DESLIGADAS por default e o path 3b/smoke fica inerte/legado
//        (não removido para minimizar risco num script sem cobertura de teste).
//
...
(async () => {
  ...
  checkTokenExpiry();
  // #1368 volunteer-only: uma fase só, no recruiter dashboard do volunteer.pmi.org.
  if (HOST !== 'volunteer.pmi.org') {
    console.error(`❌ Domínio errado (${HOST}). Rode no recruiter dashboard: ` +
                  `https://volunteer.pmi.org/recruiter-dashboard/...`);
    return;
  }
