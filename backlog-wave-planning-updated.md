...
| S-REP1 | Exportação VRMS (PMI) | High | Partial (advanced v3 2026-03-08) | VRMS no `/admin` suporta filtros opcionais por capítulo/tribo no preview e CSV (com sufixo no nome), presets rápidos (7d, 30d, ciclo atual) e persistência automática de filtros/período via `localStorage`. |
