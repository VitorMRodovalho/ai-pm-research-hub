...
| S-COM6 | Dashboard Central de Mídia | Medium | Partial (S-COM9 started 2026-03-08) | `/admin/comms` já suporta KPI via endpoint/RPC (V1) + backbone V2 (`sync-comms-metrics`, `comms_metrics_ingestion_log`, `comms_metrics_latest_by_channel`) + jornada no-code (`/admin/comms/data-entry`, CSV preview/publish) e agora ganhou workflow de publicação/auditoria (`publish_comms_metrics_batch` + `comms_metrics_publish_log`); próximo passo: validação operacional do ciclo completo com o time de Comms. |
