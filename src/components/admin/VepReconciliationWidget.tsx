  ...
  return (
    <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--surface-card)] p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-sm font-extrabold text-navy">{t('comp.vepReconciliation.title')}</h3>
        <span className={`ml-auto text-[9px] px-2 py-0.5 rounded-full font-semibold ${healthy ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
          {healthy ? t('comp.vepReconciliation.healthy') : `${total} ${t('comp.vepReconciliation.total')}`}
        </span>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 mb-3">
        ...
        <div className="bg-[var(--surface-section-cool)] rounded-lg px-2 py-2 text-center">
          <div className="text-[9px] text-[var(--text-muted)] font-semibold uppercase">{t('comp.vepReconciliation.tabOnboarding')}</div>
        ...
        <div className="bg-[var(--surface-section-cool)] rounded-lg px-2 py-2 text-center">
          <div className="text-[9px] text-[var(--text-muted)] font-semibold uppercase">{t('comp.vepReconciliation.tabOfferRetracted')}</div>
          <div className={`font-bold text-base ${(summary.offer_retracted_active_count || 0) > 0 ? 'text-amber-700' : 'text-[var(--text-muted)]'}`}>{summary.offer_retracted_active_count ?? 0}</div>
        </div>
      </div>
      <a
        href={target}
      ...
      >
        {t('comp.vepReconciliation.open')}
      </a>
      {hasVepData === false && (
        <div className="mt-2 text-[10px] text-[var(--text-muted)] italic text-center">{t('comp.vepReconciliation.notMonitored')}</div>
      )}
    </div>
  );
}
