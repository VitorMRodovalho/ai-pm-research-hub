...
2. `npm test` baselines (last updated **p218 — 2026-05-22** via RESOLVED-217.B `/certificates` PDF download wire (PR #262) + RESOLVED-WATCH-257.A engagements.legal_basis catalog↔runtime harmonization (PR #263) + reconcile divergence docs PR #261):
   - **Offline CI** (no DB env): **1606 pass / 0 fail / 43 skip** *(p218 — p217 post-RESOLVED-217.A baseline 1598 + 5 assertions in `tests/contracts/certificates-page-download-wire.test.mjs` (#258 fix) + 2 pass + 1 skip in `tests/contracts/engagements-legal-basis-lgpd-canonical.test.mjs` (WATCH-257.A fix))*
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **~1628 pass / 0 fail / 5 skip** *(p218 — p217 with-DB pin was ~1620 + 8 assertions including DB-aware paths)*
