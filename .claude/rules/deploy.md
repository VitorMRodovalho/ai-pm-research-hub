...
2. `npm test` baselines (last updated **p219 — 2026-05-22** via 3 PRs: RESOLVED-218.A auto-link vol engagements (PR #265) + RESOLVED-257.B Path B normalize 46 contract_volunteer rows + 2 producer RPCs (PR #266) + #229 Phase 1 leader_extra cohort separation (PR #268)):
   - **Offline CI** (no DB env): **1629 pass / 0 fail / 42 skip** *(p219 — p218 baseline 1606 + 8 assertions in `tests/contracts/auto-link-volunteer-engagement-to-cycle-cert.test.mjs` (BUG-218.A) + 7 in `tests/contracts/normalize-legacy-contract-volunteer.test.mjs` (WATCH-257.B) + 9 in `tests/contracts/leader-extra-cohort-separation.test.mjs` (#229 Phase 1); -1 silent drift on skip 43→42)*
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **~1651 pass / 0 fail / 5 skip** *(p219 — p218 with-DB pin was ~1628 + 23 assertions including DB-aware paths)*
