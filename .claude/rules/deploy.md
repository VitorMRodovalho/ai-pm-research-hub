...
2. `npm test` baselines (last updated **p198 — 2026-05-19** via LOW-197.C):
   - **Offline CI** (no DB env): **1449 pass / 0 fail / 46 skip**
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **1501 pass / 0 fail / 5 skip** *(estimated — last confirmed pin was p192's 1499; +2 corresponds to the 2 contract tests added in p194 GAP-192.C + GAP-194.A; verify next time DB env is available)*
