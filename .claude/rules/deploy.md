...
2. `npm test` baselines (last updated **p217 — 2026-05-21** via RESOLVED-217.A `/certificates` page bug fix + Issue #160 path A' catalog reconciliation + Council Tier 1 amendments):
   - **Offline CI** (no DB env): **1598 pass / 0 fail / 42 skip** *(confirmed p217 post-RESOLVED-217.A — p217 post-#160 baseline 1596 + 2 assertions in `tests/contracts/certificates-page-include-volunteer-agreements.test.mjs`: positive on correct RPC call shape + negative on naked-call regression)*
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **~1620 pass / 0 fail / 5 skip** *(p217 — p217 post-#160 with-DB pin was 1618 + 2 static tests run identically in both env modes)*
