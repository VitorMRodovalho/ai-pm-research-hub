...
2. `npm test` baselines (last updated **p212 — 2026-05-21** via #224 PR #236):
   - **Offline CI** (no DB env): **1499 pass / 0 fail / 50 skip** *(confirmed p212 run — last documented offline pin was p198's 1449; correctly tracks p211's intermediate +3 = 1490 and p212's +9 = 1499)*
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **estimated 1510 pass / 0 fail / 5 skip** *(estimated — last confirmed pin was p192's 1499; +11 corresponds to p194 +2 + p211 +3 + p212 +9 contract tests minus DB-aware re-additions; verify next time DB env is available)*
