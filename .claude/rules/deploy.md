...
2. `npm test` baselines (last updated **p220 — 2026-05-22** via 4 PRs: RESOLVED-219.A external reviewer end-to-end (PR #272) + RESOLVED-273 ChainPDF tables (PR #274) + RESOLVED-OAuth-allowlist Perplexity host endsWith (PR #275) + RESOLVED-execution-strip MCP proxy (PR #276)):
   - **Offline CI** (no DB env): **1672 pass / 0 fail / 42 skip** *(p220 — p219 baseline 1629 + 15 assertions in `tests/contracts/external-reviewer-rpc-carve-out.test.mjs` (7) + `tests/contracts/external-reviewer-ux-mode.test.mjs` (8) for BUG-219.A 3-layer + 11 in `tests/contracts/chain-pdf-table-rendering.test.mjs` (#273) + 9 in `tests/contracts/oauth-redirect-uri-allowlist.test.mjs` (Perplexity OAuth) + 8 in `tests/contracts/mcp-tools-list-execution-strip.test.mjs` (MCP proxy strip))*
   - **With DB env** (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY): **~1694 pass / 0 fail / 5 skip** *(p220 — p219 with-DB pin was ~1651 + 43 assertions; all p220 contract tests are static-only so with-DB delta = offline delta)*
