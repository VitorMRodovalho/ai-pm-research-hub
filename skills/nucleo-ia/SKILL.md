...
- **MCP Server:** 52 tools (45 read + 7 write) + 1 dynamic prompt + 1 static resource at `/mcp` — proxied to Supabase EF `nucleo-mcp`, server-side auto-refresh (30-day KV TTL)
...
**MCP Server:** Supabase Edge Function `nucleo-mcp` (52 tools + 1 prompt + 1 resource), proxied via Workers at `/mcp`.
