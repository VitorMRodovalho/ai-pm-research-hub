...
// Dispatched by request_account_claim RPC via pg_net.http_post. Receives {token},
// looks up the account_claim email_verification_pending row, and sends a Resend
// transactional to the MEMBER's registered address (proof-of-possession): only
// someone who controls that inbox can confirm the claim. Marks dispatched_at.
...
function buildHtml(memberName: string, link: string, expiresAt: string): string {
  ...
  return `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 24px; background: #f8f9fa; border: 1px solid #e9ecef;">
        <h2 style="color: #003B5C; font-size: 16px; margin: 0 0 12px 0;">Vincular sua conta à plataforma</h2>
        <p style="color: #495057; font-size: 14px; line-height: 1.6; margin: 0 0 12px 0;">
          Olá, <strong>${escapeHtml(memberName)}</strong>. Alguém solicitou vincular um login à sua conta de
          voluntário na plataforma Núcleo IA &amp; GP — provavelmente você, ao entrar com um e-mail diferente
          do que usou na candidatura.
        </p>
        <p style="color: #495057; font-size: 14px; line-height: 1.6; margin: 0 0 16px 0;">
          Se foi você, confirme para vincular seu acesso. O link expira em <strong>${escapeHtml(expHuman)} (Brasília)</strong>.
        </p>
        ...
        <div style="background: #fff8e1; border-left: 4px solid #ffc107; padding: 10px 14px; margin: 16px 0 0 0; border-radius: 4px;">
          <p style="color: #6b4e00; font-size: 12px; margin: 0; line-height: 1.5;">
            <strong>Não reconhece?</strong> Ignore este e-mail. Nenhuma alteração será feita sem o clique no link,
            e o acesso à sua conta continua protegido.
          </p>
        </div>
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: pending, error: pErr } = await sb
      ...
      .select('id, target_email, target_member_id, purpose, expires_at, consumed_at, dispatched_at')
      .eq('token', token)
      .eq('purpose', 'account_claim')
      .maybeSingle()
    ...
    const { data: member } = await sb
      .from('members')
      .select('name')
      .eq('id', pending.target_member_id)
      .maybeSingle()
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      headers: {
        ...
        'Idempotency-Key': `account-claim/${pending.id}`,
      },
      body: JSON.stringify({
        from: `Nucleo IA e GP <${from}>`,
        to: [pending.target_email],
        subject: 'Vincular sua conta — Nucleo IA & GP',
        html: buildHtml(memberName, link, pending.expires_at),
      }),
    })
    ...
    const { error: updErr } = await sb
      ...
      .eq('id', pending.id)
    if (updErr) console.warn('[send-account-claim] failed to mark dispatched_at:', updErr.message)

    // Do not echo the member's email back to the (internal) caller — it would land in
    // net._http_response / pg_net logs. The RPC only needs success/failure.
    return json({ ok: true })
  } catch (err) {
