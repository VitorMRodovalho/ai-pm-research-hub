...
DECLARE
  ...
  v_url text;
BEGIN
  -- Defense in depth: trigger WHEN clause already gates, but check here too
  -- in case the trigger is re-fired via UPDATE in a future iteration.
  IF NEW.pdf_url IS NOT NULL THEN
    RETURN NEW;
  END IF;

  -- Read shared secret from DB-level GUC (set via ALTER DATABASE SET).
  -- Empty/unset → log + skip; insert still succeeds.
  ...
  -- Fire-and-forget; pg_net background worker processes the request asynchronously.
  -- We don't capture the request_id (no awaiting). If the endpoint returns 200 it
  -- will UPDATE certificates.pdf_url on its own; if not, the cert remains with
  -- pdf_url IS NULL and can be re-generated by running scripts/backfill-cert-pdfs.ts.
