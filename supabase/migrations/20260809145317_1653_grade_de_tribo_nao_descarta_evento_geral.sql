-- #1653 (Onda 1, épica #1652) - a grade de tribo parava de contar o evento de audiência global.
--
-- `get_tribe_attendance_grid`, no CTE `raw_events`, tinha dois filtros de iniciativa em série:
--
--   linha A:  AND (i.legacy_tribe_id = p_tribe_id OR e.type IN ('geral','kickoff') OR e.type = 'lideranca')
--   linha B:  AND (e.initiative_id IS NULL OR i.legacy_tribe_id = p_tribe_id)
--
-- A linha A já abria exceção para os tipos de audiência global; a linha B a desfazia. Um evento
-- `geral` que carrega `initiative_id` de rastreio aponta para uma iniciativa avulsa, cujo
-- `legacy_tribe_id` é NULL: `NULL = p_tribe_id` avalia NULL para QUALQUER tribo, e
-- `e.initiative_id IS NULL` é falso. O evento caía da grade de TODAS as tribos.
--
-- O filtro de iniciativa só faz sentido para o evento cuja audiência É a tribo, isto é, `type='tribo'`.
-- A linha B passa a dizer exatamente isso. Medido em 09/08/2026, antes de aplicar:
--
--   * 1 evento × 14 tribos = 14 pares (tribo, evento) passam a entrar; 0 par deixa de entrar
--   * nenhum outro tipo (`tribo`, `kickoff`, `lideranca`, `webinar`) muda de conjunto
--   * 42 de 66 membros comparáveis (87 ativos no painel) viam percentual divergente entre
--     `get_attendance_panel` e `get_tribe_attendance_grid`; 7 de 12 líderes
--
-- O corpo abaixo é o corpo VIVO capturado por `pg_get_functiondef` em 09/08/2026, com UMA
-- substituição ancorada (a linha B). Verificado por md5: desfazendo a substituição, o corpo
-- reproduz byte a byte o `md5(prosrc)` = 7b0334a9a44f16542188ad357bd1ea53 medido antes de aplicar.
