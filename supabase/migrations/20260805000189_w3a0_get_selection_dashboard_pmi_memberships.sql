-- Wave 3a-0 #740 — surface pmi_memberships (reliable multi-chapter truth) in get_selection_dashboard
--
-- WHAT: Add one additive key `pmi_memberships` (jsonb array) to each
--       applications[].pmi_canonical chunk of public.get_selection_dashboard.
--       Value = COALESCE(a.pmi_memberships, '[]'::jsonb) — the snapshot of the
--       applicant's PMI chapter affiliations taken by the pmi-vep-sync worker
--       (e.g. ["PMI Global","Pernambuco, Brazil Chapter",...]).
--
-- WHY: Issue #740 Wave 3a-0 (chapter SSOT). The existing `chapter_canonical`
--      key is the FIRST non-"PMI Global" token of the free-text
--      service_history_chapters string — an ARBITRARY, order-dependent guess
--      that is demonstrably wrong for multi-chapter members (e.g. Henrique Diniz
--      is from Pernambuco/PMI-PE but service_history happens to list "Goiás,
--      Brazil Chapter" first → the column shows PMI-GO). pmi_memberships is the
--      reliable, complete multi-chapter list (ingest fixed in worker
--      script-mapper.ts:78). Surfacing it lets /admin/selection display the real
--      BR affiliations instead of an arbitrary single token. chapter_canonical is
--      KEPT (additive change; other consumers + the discrepancy hint still read
--      it) but the frontend stops treating it as the authoritative headline.
--
-- SCOPE: Read-side only — no writes, no signature change. Additive jsonb key is a
--      no-op for current consumers. Pairs with src/pages/admin/selection.astro
--      Wave-3a-0 display rework in the same PR.
--
-- SEDIMENT-269.A (Phase C drift): body below = the migration-103 (p279) body
--      verbatim, with ONE new line inserted inside the per-app `pmi_canonical`
--      jsonb_build_object, immediately after the `chapter_canonical` subselect.
--      Diff vs migration 103 = 1 added line. New md5 recorded post-apply.
--
-- ROLLBACK: re-issue CREATE OR REPLACE FUNCTION public.get_selection_dashboard(text)
--      with the pre-Wave-3a-0 body (last shipped via 20260805000103 / p279). The
--      pmi_memberships key then drops from payload; the frontend falls back since
--      `r.pmi_canonical?.pmi_memberships` evaluates to undefined.
