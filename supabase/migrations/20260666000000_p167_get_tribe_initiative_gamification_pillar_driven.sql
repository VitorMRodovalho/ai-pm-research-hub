-- p167 TIER E #1 (β path) — Reshape get_tribe_gamification + get_initiative_gamification
-- pillar-driven via gamification_rules JOIN (mirror of p165 #1 leaderboard refactor).
-- Refs: ADR-0081 (config-driven gamification + 6 pillars).
-- Refs: handoff_p167 Tier E surface novas leaderboard buckets em UI.
--
-- Motivation:
--   - get_gamification_leaderboard (global) went config-driven in p165 #1 (mig 20260664000000).
--   - get_tribe_gamification + get_initiative_gamification (scoped) still used category-literal
--     filters and did not expose the 3 new pillar buckets (producao/curadoria/champions).
--   - TribeGamificationTab consumes these scoped RPCs and could not surface the post-p165 buckets.
--
-- Approach (mirrors p165 #1):
--   1. LEFT JOIN gamification_rules ON (organization_id, slug=category) in points_per_member CTE
--   2. Replace category-literal filters with pillar-driven (gr.pillar = '<canonical>')
--   3. Preserve all current member fields (id/name/total_points/cycle_points/4 legacy buckets +
--      credly_badge_count/has_cpmai/trail_progress) for backward-compat with existing TribeGamificationTab.
--   4. Add 3 new pillar buckets to member shape: producao_points / curadoria_points / champions_points.
--   5. Both RPCs keep RETURNS jsonb signature + outer envelope shape
--      ({ summary, members[], tribe_ranking[], monthly_trend[] }).
--   6. get_initiative_gamification delegates to get_tribe_gamification when initiative resolves
--      to a tribe (preserved behavior); native path mirrors same pillar logic.
--
-- jsonb_build_object arg-count check: member shape = 14 fields = 28 args (well below PG 100-arg ceiling).
--
-- Backward-compat: 4 legacy bucket column names preserved (attendance_points / cert_points /
-- badge_points / learning_points). No frontend break expected — TribeGamificationTab already
-- reads these names; new fields are additive.
--
-- Rollback: re-apply previous bodies (last touched in mig 20260517140000_p118_fix_tribe_initiative_gamification_dropped_view
-- and 20260320100003_tribe_gamification_attendance_rpcs).
...
BEGIN
  ...
  WITH points_per_member AS (
    SELECT
      ...
      -- Legacy 4 buckets (now pillar-driven via JOIN)
...
BEGIN
  ...
  -- Native gamification for non-tribe initiatives. Same shape as get_tribe_gamification.
  ...
  points_per_member AS (
    SELECT
      ...
      SUM(gp.points)::int AS total_points,
      COALESCE(SUM(gp.points) FILTER (WHERE gp.created_at >= v_cycle_start), 0)::int AS cycle_points,
      -- Legacy 4 buckets (pillar-driven via JOIN)
      COALESCE(SUM(gp.points) FILTER (WHERE gr.pillar = 'presenca'), 0)::int AS attendance_points,
      COALESCE(SUM(gp.points) FILTER (WHERE gr.pillar = 'certificacoes' AND gr.slug LIKE 'cert_%'), 0)::int AS cert_points,
      COALESCE(SUM(gp.points) FILTER (WHERE gr.slug = 'badge'), 0)::int AS badge_points,
      COALESCE(SUM(gp.points) FILTER (WHERE gr.pillar = 'trilha'), 0)::int AS learning_points,
...
GRANT EXECUTE ON FUNCTION public.get_tribe_gamification(integer) TO authenticated;
