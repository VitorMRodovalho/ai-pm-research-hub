-- p118: fix get_member_detail referencing dropped public.gamification_leaderboard view
-- Root cause: ADR-0024 follow-up (2026-05-13, mig 20260513050000) dropped the view and
-- replaced with SECDEF RPC get_gamification_leaderboard(). Three RPCs still reference
-- the dropped view (get_member_detail, get_initiative_gamification, get_tribe_gamification).
-- This migration fixes only get_member_detail; the other two should be addressed
-- separately (latent same-class bug).
--
-- Replacement: aggregate from public.gamification_points directly via CTEs.
-- Preserves the original jsonb shape (total_xp, rank, categories).
-- Rollback: re-apply 20260427135518_adr_0036_get_member_detail_v4_view_internal_analytics.sql
...
COMMENT ON FUNCTION public.get_member_detail(uuid) IS
  'p118 fix: gamification section rewrites to aggregate from gamification_points directly (was referencing dropped view public.gamification_leaderboard from ADR-0024 follow-up). Same jsonb shape preserved. Other RPCs with same drift: get_initiative_gamification, get_tribe_gamification (latent).';
