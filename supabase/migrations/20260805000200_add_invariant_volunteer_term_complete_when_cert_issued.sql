-- #766 — formalize the volunteer_term ⟺ issued volunteer_agreement certificate
-- integrity guarantee as a check_schema_invariants() invariant.
--
-- The discovery (PRE_ONBOARDING_JOURNEY_DISCOVERY_2026-06-16, issue #766) flagged a
-- pending "invariant T" for "0 volunteer_term pending with an issued cert". The single
-- letters T and U are already taken (T = exactly-one-primary-email, p213; U = primary
-- chapter affiliation, Wave 3b-ii), so this one is named descriptively with an AA_ prefix.
--
-- Guarantee codified: a member holding an issued volunteer_agreement certificate must have
-- their 'volunteer_term' onboarding_progress step at status=completed, enforced by two
-- triggers shipped earlier:
--   * _trg_complete_volunteer_term_on_cert — AFTER INSERT ON certificates (type=
--     volunteer_agreement) completes the existing step (migration 20260805000018).
--   * _trg_complete_volunteer_term_on_seed — BEFORE INSERT ON onboarding_progress
--     (step_key=volunteer_term, status=pending) completes at seed time if an issued cert
--     already exists (migration 20260805000198 / p233).
-- A non-completed step alongside an issued cert means a trigger was bypassed (e.g.
-- service_role direct INSERT, or a cert backfilled without firing the AFTER trigger).
-- Directional: a member with no volunteer_term row, or a completed step without an issued
-- cert (all certs rejected/superseded), is NOT a violation.
--
-- Applied in-place via a DO-block transform (pg_get_functiondef + anchored replace) to
-- avoid re-transcribing the 22k-char body; this file reproduces the post-transform body
-- verbatim from 20260805000199 + the AA block (file == live, Phase C-safe). Only the AA
-- block is added; all 27 prior invariants are byte-identical to 20260805000199.
-- Live at apply: 28 invariants, AA=0, all 0. Negative probe (UPDATE a completed step to
-- pending inside a rolled-back tx) confirmed AA reports the drift.
--
-- ROLLBACK: re-apply 20260805000199's check_schema_invariants() body (without the AA block).
