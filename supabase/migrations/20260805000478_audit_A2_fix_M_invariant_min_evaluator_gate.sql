-- Audit 2026-07-21 (docs/audit/2026-07-21_scoring_merit_audit.md) -- A2 follow-on (consequence of mig 475)
-- The schema invariant M_application_score_consistency in check_schema_invariants() re-derived
-- research_score with the PRE-min-gate raw AVG(subtotais) formula. After mig 475 nulled the 17
-- single-evaluator apps it reported 17 false violations. Align M with the ratified derivation used
-- by compute_application_scores: objective_score_avg (min-2 gated) + interview_score (min-1 gated),
-- NULL when objective_score_avg is NULL. ONLY the M block of the function changes (verified by diff
-- vs live pg_get_functiondef; surgical single-block replacement, zero transcription of the rest).
