-- p150 hotfix2 (2026-05-12) — second 100-args jsonb_build_object regression.
--
-- Bug: after extra_flags addition (migration 20260529000000), the application
-- jsonb_build_object hit 51 key/value pairs = 102 args. Postgres again rejected
-- with "cannot pass more than 100 arguments to a function". I miscounted the
-- margin in the first hotfix and added another flat field instead of nesting it.
--
-- Bigger refactor: collapse 4 flat booleans (ai_analysis_done +
-- interview_scheduled + token_consumed + video_screening_done) into a single
-- `meta` sub-object. Also moves peer_invites_pending into peer_extra (was
-- 2 args, now 0 outer). Saves 10 args, adds 2 → net -8 args (~94 total now,
-- healthy margin for future additions).
--
-- Frontend update: 6 sites updated to read r.meta.* and r.peer_extra.invites_pending
-- with fallback to flat fields for backwards compat during deploy gap.
--
-- Sediment: feedback_jsonb_build_object_100args_limit.md should be created
-- (will be tracked in session log) — pattern: always prefer sub-objects when
-- extending fat dashboard RPCs. ~94 args is the new healthy ceiling target.
  ...
  SELECT jsonb_build_object(
    ...
    'applications', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        -- p150 hotfix2: meta subgroup collapses 4 flat booleans into 1 outer arg.
