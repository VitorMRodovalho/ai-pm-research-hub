-- #785 PR-2 — Iniciativas confidenciais: camada RLS (gate de visibilidade).
-- PR-1 (mig 20260805000231) entregou a coluna initiatives.visibility ('standard'|'confidential')
-- + o helper rls_can_see_initiative(uuid) (behavior-neutral: true p/ standard e p/ NULL). Esta PR-2
-- liga o gate: reescreve a leitura das 7 tabelas dependentes (+ initiatives) p/ que linhas de uma
-- iniciativa CONFIDENCIAL só apareçam a quem está engajado nela, ao GP (manage_platform) e ao superadmin.
-- O caminho 'standard' fica INALTERADO (helper devolve true) → read-all da curadoria (decisão #5 CLAUDE.md)
-- intacto. Tribos saem de graça (são iniciativas via bridge dual-write ADR-0005). Estende ADR-0005/0007/0012.
--
-- SPEC: ~/.claude/plans/verficar-se-ja-tem-delegated-flask.md §2 + issue #785. Decisões PM (2026-06-22):
--   #1 GP/manage_platform/superadmin SEMPRE vê (oversight). #2 Curadoria EXCLUI confidenciais (PR-3 RPCs).
--   #3 Só o GATE agora; PI-track = follow-up.
--
-- MODELO ESCOLHIDO — policy RESTRICTIVE por tabela (não editar as PERMISSIVE existentes):
--   As policies de leitura que CONCEDEM acesso são PERMISSIVE e várias tabelas têm mais de uma
--   (events=3, meeting_artifacts=2). Policies PERMISSIVE combinam por OR; editar cada uma seria frágil
--   (em especial o ramo anon de events). Em vez disso adicionamos UMA policy RESTRICTIVE de SELECT por
--   tabela — RESTRICTIVE combina por AND, logo só ESTREITA, nunca alarga: standard/NULL → helper=true →
--   AND no-op; confidencial → false → excluído. É o mesmo padrão das policies *_org_scope (RESTRICTIVE)
--   já existentes (isolamento multi-tenant). Como não há iniciativa confidencial hoje (default standard),
--   o efeito imediato é zero — comportamento idêntico até alguém marcar visibility='confidential'.
--
-- 🔴 ANTI-VAZAMENTO POR SUBQUERY (board_items / meeting_artifacts): uma subquery inline numa USING de RLS
--   sofre o RLS da tabela referenciada. Se um project_board confidencial estiver oculto ao usuário, uma
--   subquery inline (SELECT pb.initiative_id ...) retornaria NULL → rls_can_see_initiative(NULL)=true →
--   VAZARIA o board_item. Por isso a resolução board→initiative e event→initiative é feita em helpers
--   SECURITY DEFINER (rls_can_see_board / rls_can_see_artifact_link) que bypassam RLS — o mesmo motivo
--   pelo qual o codebase já usa helpers rls_* SECDEF em policies (rls_can_for_initiative, rls_is_member…).
--
-- ROLLBACK:
--   DROP POLICY IF EXISTS initiatives_confidential_visibility ON public.initiatives;
--   DROP POLICY IF EXISTS events_confidential_visibility ON public.events;
--   DROP POLICY IF EXISTS project_boards_confidential_visibility ON public.project_boards;
--   DROP POLICY IF EXISTS board_items_confidential_visibility ON public.board_items;
--   DROP POLICY IF EXISTS meeting_artifacts_confidential_visibility ON public.meeting_artifacts;
--   DROP POLICY IF EXISTS tribe_deliverables_confidential_visibility ON public.tribe_deliverables;
--   DROP POLICY IF EXISTS recurring_meeting_rules_confidential_visibility ON public.recurring_meeting_rules;
--   DROP POLICY IF EXISTS governance_documents_confidential_visibility ON public.governance_documents;
--   DROP FUNCTION IF EXISTS public.rls_can_see_board(uuid);
--   DROP FUNCTION IF EXISTS public.rls_can_see_artifact_link(uuid, uuid);
--   -- check_schema_invariants: re-aplicar o corpo da mig 219 (reverter o splice AJ).
--   NOTIFY pgrst, 'reload schema';

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Resolvers SECURITY DEFINER — board→initiative e (initiative|event)→initiative, bypassando RLS.
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.rls_can_see_board(p_board_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
AS $$
  SELECT public.rls_can_see_initiative(
    (SELECT pb.initiative_id FROM public.project_boards pb WHERE pb.id = p_board_id)
  );
$$;

COMMENT ON FUNCTION public.rls_can_see_board(uuid) IS
'#785 PR-2: resolve a board to its initiative (bypassing RLS via SECURITY DEFINER) and delegate to '
'rls_can_see_initiative. Used by the board_items RESTRICTIVE SELECT policy — an inline subquery would '
'be subject to project_boards RLS and leak when the board is hidden. NULL board / no board → NULL → '
'rls_can_see_initiative(NULL)=true (org-level / unlinked, visible).';

GRANT EXECUTE ON FUNCTION public.rls_can_see_board(uuid) TO authenticated, anon, service_role;

CREATE OR REPLACE FUNCTION public.rls_can_see_artifact_link(p_initiative_id uuid, p_event_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
AS $$
  SELECT public.rls_can_see_initiative(
    COALESCE(
      p_initiative_id,
      (SELECT e.initiative_id FROM public.events e WHERE e.id = p_event_id)
    )
  );
$$;

COMMENT ON FUNCTION public.rls_can_see_artifact_link(uuid, uuid) IS
'#785 PR-2: resolve a meeting_artifact''s effective initiative — its own initiative_id, else via its '
'event (bypassing RLS via SECURITY DEFINER) — and delegate to rls_can_see_initiative. Used by the '
'meeting_artifacts RESTRICTIVE SELECT policy. The event fallback covers artifacts linked only by '
'event_id (1/12 live at PR-2). Both NULL → rls_can_see_initiative(NULL)=true (org-level, visible).';

GRANT EXECUTE ON FUNCTION public.rls_can_see_artifact_link(uuid, uuid) TO authenticated, anon, service_role;

-- Defensive: ensure the PR-1 helper is executable by the policy-evaluating roles (idempotent).
GRANT EXECUTE ON FUNCTION public.rls_can_see_initiative(uuid) TO authenticated, anon, service_role;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Policies RESTRICTIVE de SELECT — uma por tabela dependente. AND com as PERMISSIVE existentes.
--    standard/NULL → helper=true → no-op; confidencial → false → linha invisível ao não-engajado.
-- ─────────────────────────────────────────────────────────────────────────────

DROP POLICY IF EXISTS initiatives_confidential_visibility ON public.initiatives;
CREATE POLICY initiatives_confidential_visibility ON public.initiatives
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(id));

DROP POLICY IF EXISTS events_confidential_visibility ON public.events;
CREATE POLICY events_confidential_visibility ON public.events
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(initiative_id));

DROP POLICY IF EXISTS project_boards_confidential_visibility ON public.project_boards;
CREATE POLICY project_boards_confidential_visibility ON public.project_boards
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(initiative_id));

DROP POLICY IF EXISTS board_items_confidential_visibility ON public.board_items;
CREATE POLICY board_items_confidential_visibility ON public.board_items
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_board(board_id));

DROP POLICY IF EXISTS meeting_artifacts_confidential_visibility ON public.meeting_artifacts;
CREATE POLICY meeting_artifacts_confidential_visibility ON public.meeting_artifacts
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_artifact_link(initiative_id, event_id));

DROP POLICY IF EXISTS tribe_deliverables_confidential_visibility ON public.tribe_deliverables;
CREATE POLICY tribe_deliverables_confidential_visibility ON public.tribe_deliverables
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(initiative_id));

DROP POLICY IF EXISTS recurring_meeting_rules_confidential_visibility ON public.recurring_meeting_rules;
CREATE POLICY recurring_meeting_rules_confidential_visibility ON public.recurring_meeting_rules
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(initiative_id));

DROP POLICY IF EXISTS governance_documents_confidential_visibility ON public.governance_documents;
CREATE POLICY governance_documents_confidential_visibility ON public.governance_documents
  AS RESTRICTIVE FOR SELECT TO public
  USING (public.rls_can_see_initiative(initiative_id));

-- ─────────────────────────────────────────────────────────────────────────────
-- 3. check_schema_invariants() + invariante AJ_confidential_visibility_gate_present (36 -> 37). Corpo
--    reproduzido VERBATIM da mig 219 com UM RETURN QUERY adicionado antes do END (padrão AG/AH/AI).
--    Phase-C: esta mig 232 vira a captura mais recente; live pós-apply == este corpo.
-- ─────────────────────────────────────────────────────────────────────────────
