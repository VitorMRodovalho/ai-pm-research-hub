-- p160 B1 (2026-05-14): tribe attendance grid now surfaces V3-legacy + historical members
--
-- Root cause: grid_members CTE required V4 engagements rows (active or revoked) on
-- the tribe's initiative. V3-era members had members.initiative_id set (the hybrid
-- design per ADR-0080 / sediment feedback_members_initiative_id_v3_v4_hybrid_design)
-- but never had engagement rows backfilled. Result: 5+ alumni/observer of tribo 5
-- with real attendance rows in tribo 5 events (Diego Menezes, Marcelo Ferreira,
-- Rogério Côrtes, Alexandre Meirelles, Wellinghton Pereira Barboza) silently
-- absent from the presence grid.
--
-- PM complaint (2026-05-14): "pessoas que participaram das tribos na pagina
-- tribe/id presenca nao mostra o nome das pessoas com presencas e ou faltas
-- ate onde ela particiou, acho uma falha para visao estrategica do gestor."
--
-- Fix (grid_members CTE):
-- 1. Branch 1 (active): also accept V3 primary tribe (members.initiative_id) as
--    evidence — matches the hybrid design until ADR-0080 cutover completes.
-- 2. Branch 2 (historical): drop the engagement filter entirely; require only
--    attendance row on a THIS-tribe event (ge.tribe_id = p_tribe_id). Attendance
--    is itself ground-truth evidence the person was a tribo member at the time.
--    Restricting to ge.tribe_id excludes cross-tribe 'geral'/'kickoff'/'lideranca'
--    attendance from counting as tribe-membership evidence.
--
-- Smoke (cycle 3 tribo 5): Alexandre Meirelles + Wellinghton Pereira Barboza
-- now surface in historical roster. The 3 Cycle 2 alumni (Diego/Rogério/Marcelo)
-- still don't show because their attendance is outside the current cycle window
-- (grid_events.date >= cycle_start filter). Backlog: optional p_all_cycles flag
-- to expose all-time strategic view.
--
-- Rollback: revert function body to the prior canonical version (last touched
-- by migration 20260518030000_* or thereabouts). Frontend tolerates either
-- shape — member_status field carries the historical signal regardless.

CREATE OR REPLACE FUNCTION public.get_tribe_attendance_grid(p_tribe_id integer, p_event_type text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  grid_members AS (
    -- Branch 1: active members tied to this tribe via V4 engagement OR V3 primary (hybrid per ADR-0080)
    SELECT m.id, m.name,
    ...
    WHERE m.member_status = 'active'
      AND (
        ...
        OR m.initiative_id = v_tribe_initiative_id  -- p160 B1: V3 hybrid fallback
      )
      AND m.operational_role NOT IN ('sponsor', 'chapter_liaison', 'guest', 'none')
    UNION
    -- Branch 2: historical members (alumni/observer/inactive) — attendance on a THIS-tribe event
    -- is itself ground-truth evidence the person belonged. Drop engagement filter (p160 B1).
    ...
    WHERE m.member_status IN ('observer', 'alumni', 'inactive')
      AND ge.tribe_id = p_tribe_id  -- restrict to events bound to THIS tribe (not cross-tribe geral/lideranca)
  ),
  eligibility AS (
