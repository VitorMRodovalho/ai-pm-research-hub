-- =====================================================================================
-- #676 Slice 2 (Frontend, read-only): admin list RPC for the "Agenda recorrente" screen.
--
-- One UI-shaped read that joins the canonical rule with its human-readable scope name and
-- the same drift metrics as get_recurring_meeting_drift, plus last_reconciled_at from the
-- audit trail. Unlike get_recurring_meeting_drift (active-only diagnostic), this returns
-- ALL rules (active/paused/archived) so the admin can manage the full set.
--
-- Gate: manage_platform (V4 can_by_member) + ADR-0028 cron-context bypass. Read-only.
-- =====================================================================================
...
REVOKE ALL ON FUNCTION public.get_recurring_meeting_admin_list(date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_recurring_meeting_admin_list(date) TO authenticated, service_role;
