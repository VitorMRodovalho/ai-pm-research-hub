-- #1039 / #1026 Fatia B — auto-approve + auto-revoke of Drive grants on offboard (ALUMNI-ONLY).
-- ADR-0107 Amendment 1. Council Tier-3 review 2026-07-02 (legal, security, data-arch, accountability:
-- 4x APPROVE_WITH_CONDITIONS, zero blockers — conditions incorporated below). PM-ratified 2026-07-02.
--
-- What ships here (step order is load-bearing — data-arch MF-2; column DDL must precede every
-- function body that references it):
--   1. drive_offboarding_audit + approval_mode ('manual'|'auto') — provenance column (AL clause 1
--      carve-out) — and + skip_reason ('owner_permission'|'member_reactivated') — structural
--      disambiguation of the dual-use 'skipped' lane (legal COND-2 / data-arch MF-4), enforced by
--      CHECK (skipped requires skip_reason). Zero live skipped rows at authoring time → no backfill.
--   2. site_config seed 'drive_auto_revoke_enabled' = false::jsonb — kill-switch ships DARK
--      (ADR-0116 precedent). Go-live = runbook OPS checklist (site_config flip + admin_audit_log
--      INSERT), gated on legal COND-1 (offboarding comms) + COND-3 (retention issue filed).
--   3. auto_approve_alumni_drive_revocations(p_member_id, p_source) — service-role-only, set-based,
--      NULL-safe FAIL-CLOSED kill-switch (missing key = disabled; security must-fix), alumni-only
--      filter at UPDATE time, self-contained authorization record in admin_audit_log (audit_ids).
--   4. admin_reactivate_member rebuild — cancels OPEN queue rows (pending_revoke/approved →
--      skipped/member_reactivated) BEFORE clearing offboarded_at (AL clause-2 mechanism; closes a
--      pre-existing gap: the invariant description promised queue-clear on reversal, nothing did it).
--   5. admin_list_drive_revocation_audit rebuild — exposes approval_mode + skip_reason (GP provenance).
--   6. get_drive_teardown_overview rebuild — auto_approved metric (per-member + summary) so the GP
--      panel distinguishes in-flight auto approvals (data-arch R-3).
--   7. check_schema_invariants FULL REBUILD (Phase C discipline: body captured from migration
--      20260805000302, verified drift-0 against live via scripts/audit-rpc-body-drift.mjs on
--      2026-07-02) with ONLY the AL block amended: clause 1 (revoked needs revoked_at AND
--      (approved_by OR approval_mode='auto')), clause 1b (auto rows: no human approver, never
--      pending), clause 1c (skipped requires skip_reason), clause 2 unchanged.
--
-- NOT here: no cron change (cron 64 already drains any 'approved' row), no revoke-EF change, no
-- trigger change (trg_drive_teardown_scan keeps firing for alumni AND inactive — the alumni/inactive
-- differentiation happens at auto-approve time, preserving contract test 1026 pins). The detection
-- EF (audit-drive-offboarding-access) gains a non-fatal post-upsert call to the new RPC.
--
-- Refs: #1039 #1026 #209 · ADR-0107 (+Amendment 1) · ADR-0071 Amd 3-D · ADR-0116 · council decision
-- docs/council/decisions/2026-07-02-1039-drive-auto-revoke-alumni-only.md

-- ---------------------------------------------------------------------
-- 1. Schema: approval_mode + skip_reason provenance columns.
-- ---------------------------------------------------------------------
ALTER TABLE public.drive_offboarding_audit
  ADD COLUMN approval_mode text NOT NULL DEFAULT 'manual'
    CONSTRAINT drive_offb_audit_approval_mode_check CHECK (approval_mode IN ('manual','auto')),
  ADD COLUMN skip_reason text
    CONSTRAINT drive_offb_audit_skip_reason_check CHECK (skip_reason IN ('owner_permission','member_reactivated'));

-- A skipped row without a reason is structurally ambiguous (legal COND-2 / data-arch MF-4).
-- Zero skipped rows exist at authoring time (grounded live 2026-07-02) — no backfill needed.
ALTER TABLE public.drive_offboarding_audit
  ADD CONSTRAINT drive_offb_audit_skipped_requires_reason
  CHECK (status <> 'skipped' OR skip_reason IS NOT NULL);

COMMENT ON COLUMN public.drive_offboarding_audit.approval_mode IS
  '#1039/ADR-0107 Amendment 1: ''manual'' = GP human approve (approved_by NOT NULL); ''auto'' = alumni-only system approve via auto_approve_alumni_drive_revocations (approved_by stays NULL — this column IS the provenance; invariant AL clauses 1/1b).';
COMMENT ON COLUMN public.drive_offboarding_audit.skip_reason IS
  '#1039: structural disambiguation of the skipped lane (Art. 37 evidence quality, council COND-2): owner_permission = undeletable grant surfaced for exception review; member_reactivated = open row cancelled by admin_reactivate_member.';

-- ---------------------------------------------------------------------
-- 2. Kill-switch: ships DARK (false). Go-live = runbook OPS checklist (audited flip).
-- ---------------------------------------------------------------------
INSERT INTO public.site_config (key, value)
VALUES ('drive_auto_revoke_enabled', 'false'::jsonb)
ON CONFLICT (key) DO NOTHING;

-- ---------------------------------------------------------------------
-- 3. Alumni-only auto-approve (service-role only; EF/cron path).
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.auto_approve_alumni_drive_revocations(
...
DECLARE
  ...
  v_gp uuid;
BEGIN
  -- Service-role only (same NULL-safe gate as get_offboarded_member_emails(uuid), mig ...317).
  IF public.current_caller_role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'Unauthorized: service-role only';
  END IF;

  -- Kill-switch (security must-fix): NULL-safe FAIL-CLOSED — a missing site_config row means
  -- DISABLED (an accidental DELETE of the key must never enable auto-revoke). JSONB literal is
  -- the boolean form on BOTH sides (seed above uses 'false'::jsonb; the flip writes 'true'::jsonb).
  SELECT value INTO v_enabled FROM public.site_config WHERE key = 'drive_auto_revoke_enabled';
  IF coalesce(v_enabled, 'false'::jsonb) IS DISTINCT FROM 'true'::jsonb THEN
    RETURN jsonb_build_object('enabled', false, 'approved_count', 0);
  END IF;

  -- ALUMNI-ONLY (#1039 PM decision 2026-07-02): inactive is reversible by design (ADR-0071
  -- Amd 3-D sabbatical; ADR-0116 _reacceptance_disengage exits to inactive with NO pipeline
  -- gate before reactivation) and stays on the manual GP lane. member_status is checked at
  -- UPDATE time, closing the detect→approve race (a member reactivated in between no longer
  -- matches). Set-based single UPDATE (data-arch: no N+1). approved_by stays NULL — the
  -- provenance is approval_mode='auto' (invariant AL clauses 1/1b).
  WITH flipped AS (
    UPDATE public.drive_offboarding_audit d
       SET status = 'approved',
           approval_mode = 'auto',
           approved_at = now(),
           updated_at = now()
     ...
     WHERE m.id = d.member_id
       AND d.status = 'pending_revoke'
       AND m.member_status = 'alumni'
       AND m.offboarded_at IS NOT NULL
       AND (p_member_id IS NULL OR d.member_id = p_member_id)
    RETURNING d.id, d.member_id
  )
  SELECT array_agg(id), array_agg(DISTINCT member_id) INTO v_ids, v_member_ids FROM flipped;

  v_count := coalesce(array_length(v_ids, 1), 0);
  IF v_count = 0 THEN
    RETURN jsonb_build_object('enabled', true, 'approved_count', 0);
  END IF;

  -- Self-contained authorization record (security rec): the exact audit_ids flipped, so the
  -- Art. 37 forensic chain (authorization → physical revocation) is an O(1) id lookup, not a
  -- timestamp join. actor_id NULL = system (established pattern: scan pii_log, Fatia A trigger).
  INSERT INTO public.admin_audit_log (actor_id, action, target_type, target_id, changes, metadata)
  VALUES (
    NULL, 'drive_revocation_auto_approved', 'drive_offboarding_audit', NULL,
    jsonb_build_object('audit_ids', to_jsonb(v_ids), 'member_ids', to_jsonb(v_member_ids)),
    jsonb_build_object('source', p_source, 'count', v_count,
                       'policy', 'alumni-only auto-approve (#1039 / ADR-0107 Amendment 1)')
  );

  -- GP visibility (mirrors the mig ...262 upsert notification pattern; V4 SSOT gate).
  FOR v_gp IN
    SELECT m.id FROM public.members m
    WHERE m.member_status = 'active'
      AND public.can_by_member(m.id, 'manage_member')
  LOOP
    -- Explicit casts disambiguate the 3 create_notification overloads.
    PERFORM public.create_notification(
      v_gp,
      'drive_offboarding_auto_approved'::text,
      (v_count::text || ' permissao(oes) Drive de alumni auto-aprovada(s) para revogacao')::text,
      'Auto-approve alumni-only (#1039/ADR-0107 Amendment 1). A revogacao executa em ate ~1h via cron (revoke-drive-drain-hourly). Falhas aparecerao no painel /admin/members/drive-teardown para revisao de excecao.'::text,
      NULL::text,
      'drive_offboarding_audit'::text,
      NULL::uuid);
  END LOOP;

  RETURN jsonb_build_object('enabled', true, 'approved_count', v_count,
                            'member_count', coalesce(array_length(v_member_ids, 1), 0));
END;
$$;

REVOKE ALL ON FUNCTION public.auto_approve_alumni_drive_revocations(uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.auto_approve_alumni_drive_revocations(uuid, text) TO service_role;

COMMENT ON FUNCTION public.auto_approve_alumni_drive_revocations(uuid, text) IS
  '#1039/ADR-0107 Amendment 1: alumni-only set-based auto-approve of pending Drive revocations. Service-role only (detection EF post-upsert). No-op unless site_config drive_auto_revoke_enabled = true::jsonb (NULL-safe fail-closed kill-switch). approved_by stays NULL; provenance = approval_mode=auto (invariant AL). Revocation executes via cron 64 drain. inactive members are NEVER auto-approved (reversible, ADR-0071 Amd 3-D).';

-- ---------------------------------------------------------------------
-- 4. admin_reactivate_member — cancel open Drive queue rows on reactivation.
--    (Body from migration 20260516880000, verified drift-0 vs live 2026-07-02;
--     ONLY the #1039 queue-clear + its audit/return plumbing added.)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- #1039 (ADR-0107 Amendment 1): cancel any OPEN Drive revocation rows BEFORE the
  -- members UPDATE clears offboarded_at. Ordering is load-bearing (invariant AL clause 2):
  -- open rows must never coexist with an active / non-offboarded member, and the hourly
  -- drain (cron 64) must never revoke a reactivated member's access. The drain race is
  -- closed by the revoke EF's status re-read guard (it acts only on status='approved').
  -- skip_reason='member_reactivated' is the structural provenance (council COND-2/MF-4).
  UPDATE public.drive_offboarding_audit
     SET status = 'skipped',
         skip_reason = 'member_reactivated',
         notes = 'auto-cancelled: member reactivated (#1039)',
         updated_at = now()
   WHERE member_id = p_member_id
     AND status IN ('pending_revoke','approved');
  GET DIAGNOSTICS v_drive_cancelled = ROW_COUNT;
  ...
  VALUES (
    ...
    jsonb_strip_nulls(jsonb_build_object('reason_category', 'return', 'pipeline_id', v_pipeline_id, 'drive_rows_cancelled', v_drive_cancelled))
  )
  ...
  RETURN jsonb_build_object(
    ...
    'pipeline_id', v_pipeline_id,
    'drive_rows_cancelled', v_drive_cancelled
  );
END;
$function$;

-- ---------------------------------------------------------------------
-- 5. admin_list_drive_revocation_audit — expose approval_mode + skip_reason.
--    (Body from migration 20260805000262, verified drift-0 vs live 2026-07-02.)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'status', status, 'approval_mode', approval_mode, 'skip_reason', skip_reason, 'google_error', google_error,
...
-- ---------------------------------------------------------------------
-- 6. get_drive_teardown_overview — auto_approved in-flight metric (data-arch R-3).
--    (Body from migration 20260805000318, verified drift-0 vs live 2026-07-02.)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  queue AS (
    SELECT member_id,
      ...
      count(*) FILTER (WHERE status = 'approved' AND approval_mode = 'auto') AS auto_approved,
  ...
  rows AS (
    SELECT
      ...
      coalesce(q.auto_approved, 0)  AS auto_approved,
  ...
  SELECT
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'pending_revoke', pending_revoke, 'approved', approved, 'auto_approved', auto_approved, 'revoked', revoked,
  ...
  SELECT jsonb_build_object(
    ...
    'auto_approved',  (SELECT coalesce(sum((e->>'auto_approved')::int), 0) FROM jsonb_array_elements(v_members) e),
...
-- ---------------------------------------------------------------------
-- 7. check_schema_invariants FULL REBUILD — AL block amended (see header).
--    (Body from migration 20260805000302, verified drift-0 vs live 2026-07-02.
--     Everything except the AL block is byte-identical to that capture.)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 8. PostgREST schema reload (new RPC + changed jsonb surfaces).
-- ---------------------------------------------------------------------
