-- Onda 2 — WS-1: ladder "governança vence" — chapter_board outranks research/observer.
--
-- Wave 1 (mig 282/283) moved `sponsor` above `researcher`. This Wave 2 step moves the
-- `chapter_board` WHEN clause from its old position (below observer/alumni, rank 10) up to
-- immediately AFTER `sponsor` (rank 6, above researcher/observer). PM decision 2026-06-28
-- ("governança sempre vence"): a chapter director who ALSO sits on a committee or observes a
-- research tribe must display as "Ponto Focal do Capítulo" (chapter_liaison), not Pesquisador
-- / Observador. Kept BELOW sponsor (so Ivan/the 5 sponsors stay sponsor) and below the
-- operational leaders (manager/deputy_manager/tribe_leader), mirroring the Wave-1 rationale.
--
-- ADR-0023 parity: the same clause move is applied to check_schema_invariants().A3 byte-for-byte
-- (role-ladder-parity.test.mjs enforces it). Migration carries BOTH functions so the latest
-- CREATE OR REPLACE of each, which the parity test reads, stay in lock-step.
--
-- Backfill (grounded 2026-06-28): the change flips exactly 6 active members, all chapter directors:
--   observer  -> chapter_liaison : Welma Alves, Lorena Souza, Emanuele Melo, Emanoela Kerkhoff
--   researcher-> chapter_liaison : Roberto Macêdo (PMI-CE), João Uzejka (PMI-RS)
-- The 5 sponsors (sponsor outranks chapter_board) and the 5 existing chapter_liaison are unchanged.
-- Collateral (intentional): chapter_liaison is excluded from get_public_trail_ranking cohort
-- (mig 284), so Roberto/João leave the trail ranking — correct (chapter directors are not trail
-- participants). Each flip writes an admin_audit_log row for forensic traceability (#318 pattern).
--
-- Cross-ref: ADR-0023 (ladder parity), docs/reference/V4_AUTHORITY_MODEL.md, handoff pt6.
...
-- Backfill: recompute operational_role for members holding an authoritative chapter_board
-- engagement (the only population whose derivation changed), using the SAME new ladder, and
-- write an audit row per flip. Scoped to chapter_board holders so no unrelated row is touched;
-- A3 was 0 pre-migration (grounded) so this is the complete affected set.
WITH derived AS (
  SELECT
    m.id AS member_id,
    m.operational_role AS old_role,
    COALESCE(
      (SELECT CASE
          WHEN bool_or(ae.kind = 'volunteer' AND ae.role = 'manager')        THEN 'manager'
          WHEN bool_or(ae.kind = 'volunteer' AND ae.role = 'co_gp')          THEN 'manager'
  ...
  WHERE m.member_status = 'active'
    AND EXISTS (
      SELECT 1 FROM public.auth_engagements x
      WHERE x.person_id = m.person_id AND x.is_authoritative = true AND x.kind = 'chapter_board'
    )
),
changed AS (
  SELECT member_id, old_role, new_role FROM derived WHERE old_role IS DISTINCT FROM new_role
),
upd AS (
  UPDATE public.members m
  SET operational_role = c.new_role, updated_at = now()
  FROM changed c
  WHERE m.id = c.member_id
  RETURNING m.id AS member_id, c.old_role, c.new_role
)
INSERT INTO public.admin_audit_log (actor_id, action, target_type, target_id, changes, metadata)
SELECT
  NULL,
  'member.operational_role_ws1_chapter_board_precedence',
  'member',
  upd.member_id,
  jsonb_build_object(
    'field', 'operational_role',
    'old_value', upd.old_role,
    'new_value', upd.new_role,
    'effective_date', CURRENT_DATE
  ),
  jsonb_build_object(
    'source', 'onda2_ws1_governance_outranks_research',
    'reason', 'Wave 2 WS-1: chapter_board moved above researcher/observer in the V4 cache ladder (PM ''governança vence'')',
    'migration', '20260805000285'
  )
FROM upd;
