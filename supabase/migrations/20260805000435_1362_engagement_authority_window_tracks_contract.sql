-- #1362: an approved-application engagement's authority window must track its Núcleo contract,
-- not the researcher-era cycle close.
--
-- Regression (2026-07-13): the VEP JSON import re-fired sync_operational_role_cache on Paulo Jr
-- (pmi 1158211), a researcher promoted to tribe leader, and he dropped to `guest`. Root cause:
-- operational_role derives from auth_engagements.is_authoritative, which requires
-- `end_date IS NULL OR end_date >= CURRENT_DATE`. His leader engagement pointed at the leader
-- application (contract 2026-07-01 -> 2027-06-30) but still carried the OLD researcher-contract
-- dates (2026-01-20 -> 2026-06-30). Once 2026-06-30 passed and any trigger re-fired, authority was
-- lost. The cache is lazy (recomputes only on trigger fire) so the bug stayed latent until the
-- import touched his row.
--
-- Two writers left the window stale: manage_initiative_engagement(update_role) changes only `role`;
-- approve_selection_application re-pointed selection_application_id but stamped end_date from
-- selection_cycles.close_date, NOT the #1316 SSOT selection_applications.nucleo_contract_end.
--
-- Fix: approve_selection_application now sources the engagement end_date from the application's
-- nucleo_contract_end on INSERT, and on the promotion/renewal path EXTENDS the existing engagement's
-- end_date to the new contract end when it lasts longer (never shrinks a perpetual/longer window).
-- Plus an idempotent backfill of any active engagement whose window expires before its active
-- contract, and of invalid end<start windows.
--
-- Guard: tests/contracts/1362-engagement-authority-window.test.mjs (writer-agnostic invariant).
-- Base: live body of approve_selection_application (pg_get_functiondef) with ONLY the two date
-- expressions changed — preserves the #1163 pmi_id-first matching, #615 error surfacing, chapter
-- derivation, onboarding seeding and anomaly-log capture.
...
BEGIN
  ...
  IF v_existing_engagement_id IS NULL THEN
    ...
    VALUES (
      ...
      CURRENT_DATE,
      -- #1362: prefer the Núcleo contract end (#1316 SSOT) so the engagement authority window covers
      -- the full contract, not just the cycle close (a 1-year contract crosses two semesters).
      COALESCE(
        v_app.nucleo_contract_end,
        v_cycle_end_date,
  ...
  ELSE
    -- #1362: a promotion/renewal re-points the existing active engagement at the newly-approved
    -- application. The authority window (auth_engagements.is_authoritative requires
    -- end_date >= CURRENT_DATE) must track the new contract, otherwise a leader promoted off a
    -- researcher contract keeps the OLD end_date and silently drops to guest once it passes (the
    -- 2026-07-13 Paulo regression). Extend end_date to the new contract end when it lasts longer;
    -- never shrink a perpetual (NULL) or already-longer window. The application pointer keeps its
    -- prior set-if-null semantics.
    UPDATE public.engagements
    SET selection_application_id = COALESCE(selection_application_id, p_application_id),
        end_date = CASE
          WHEN end_date IS NOT NULL
           AND v_app.nucleo_contract_end IS NOT NULL
           AND end_date < v_app.nucleo_contract_end
          THEN v_app.nucleo_contract_end
          ELSE end_date
        END,
        updated_at = now()
    WHERE id = v_existing_engagement_id;
...
-- Backfill (idempotent): any active engagement whose window expires before its still-active linked
-- contract silently loses authority mid-contract. Align end_date to the contract end.
UPDATE public.engagements e
SET end_date = sa.nucleo_contract_end, updated_at = now()
FROM public.selection_applications sa
WHERE e.selection_application_id = sa.id
  AND e.status = 'active'
  AND e.end_date IS NOT NULL
  AND sa.nucleo_contract_end IS NOT NULL
  AND e.end_date < sa.nucleo_contract_end;

-- Backfill (idempotent): invalid windows (end before start) on active engagements -> perpetual.
UPDATE public.engagements
