-- p150 hotfix (2026-05-12) — get_selection_dashboard 100-args jsonb_build_object limit.
--
-- Bug: previous version had 51 key/value pairs = 102 args, exceeding the
-- PG 100-args function limit. Postgres returned "cannot pass more than 100
-- arguments to a function" on every call → frontend showed empty table.
-- Surfaced in postgres logs immediately after p150 fix (peer_eval_distinct_evaluators)
-- + p150 B-light (3 video_screening_* fields) deploy.
--
-- Fix: collapse 4 new fields into 2 sub-objects (peer_extra + video_agg) —
-- saves 4 args (-4 to 98). Existing peer_eval_count + video_screening_done
-- are preserved unchanged (frontend backwards compat).
  ...
  SELECT jsonb_build_object(
    ...
    'applications', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        -- p150 hotfix: peer_extra subgroup (was: peer_eval_distinct_evaluators inline)
        ...
        -- p150 hotfix: video_agg subgroup (was: 3 inline fields exceeding 100-args limit)
