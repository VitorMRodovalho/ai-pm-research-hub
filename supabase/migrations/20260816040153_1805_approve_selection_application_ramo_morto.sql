-- #1805 (parte 2) — ramo morto em approve_selection_application.
--
-- `role_applied IN (..., 'coordinator', ...)`: o literal nao existe no dominio do CHECK
-- (`researcher`, `leader`, `both`, `manager`), entao o ramo nunca casava. Efeito de
-- comportamento: NENHUM — os dois ramos anteriores ja cobriam leader e researcher, e manager
-- segue coberto. Separado da parte 1 de proposito: e a unica funcao do caminho critico de
-- aprovacao, e a correcao e cosmetica, entao ela nao viaja junto com mudanca de predicado.
--
-- `both` NAO ganha ramo aqui: cai no ELSE e vira researcher, como sempre caiu. Mudar isso e
-- decisao de produto, nao limpeza de literal morto. Hoje: 137 researcher, 33 leader, 0 both.
...
BEGIN
  ...
  v_engagement_role := CASE
    -- #1805: 'coordinator' saiu da lista -- nao existe no dominio do CHECK de
    -- role_applied ('researcher','leader','both','manager'), entao era ramo
    -- morto. 'both' NAO entra aqui de proposito: cai no ELSE e vira researcher,
    -- comportamento preexistente e inalterado por esta correcao.
    WHEN v_app.role_applied IN ('leader', 'researcher', 'manager') THEN v_app.role_applied
    ELSE 'researcher'
  END;
