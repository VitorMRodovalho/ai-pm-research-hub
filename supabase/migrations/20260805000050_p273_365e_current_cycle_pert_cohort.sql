-- p273 #365e — CR-042 current-cycle PERT cohort correction
--
-- WHAT:
--   Replaces the historical approved+active-volunteer cohort in
--   _compute_pert_cutoff_core with the current cycle candidate cohort, scoped by
--   role_applied and score dimension. Re-runs recompute_all_active_pert_cutoffs()
--   at the end so stored pert_* and final_score_pert_* fields are recalibrated
--   retroactively.
--
-- WHY:
--   PM clarified that CR-042 was authored from the Excel/imported cycle premise:
--   the cutoff is the PERT of the current cycle's distribution, by track and
--   dimension. The committee approval process remains human-in-the-loop; this
--   migration only recalibrates displayed/supporting cutoffs and bands. It does
--   not write decisions or statuses.
--
-- COMPAT:
--   Function signatures are preserved. p_filter_active_only remains accepted for
--   legacy callers but no longer changes cohort membership; the current-cycle
--   cohort is the canonical CR-042 behavior from p273 onward.
...
COMMENT ON FUNCTION public._compute_pert_cutoff_core(uuid, text, boolean, text, uuid) IS
  'p273 #365e: CR-042 current-cycle PERT cutoff core. Cohort = applications in the same cycle and role with the requested score dimension populated. The legacy p_filter_active_only argument is accepted for signature compatibility but no longer filters to historical active volunteers. Formula remains (2*min + 4*avg + 2*max)/8. No status/decision writes.';
