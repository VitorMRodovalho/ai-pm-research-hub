-- p197 (2026-05-19): enrich get_evaluation_form payload with VEP semantic context
--
-- Context: Fabricio submitted Luíse Quintana with anomalous scores via MCP Claude Chat
-- (weighted_subtotal 96 vs first evaluator 148, diff -52, mediana ciclo 135) on 2026-05-19 01:00.
-- Root cause: evaluator received minimal context (no cv_extracted_text, no LinkedIn profile, no
-- service history, no PMI memberships, no PERT cutoff reference, no computed criterion weights),
-- so Claude had no anchor to validate the score scale before submitting.
--
-- This migration enriches the payload returned by get_evaluation_form (called by both UI
-- evaluators and MCP evaluators BEFORE submitting). The submit_evaluation MCP wrapper is
-- patched separately in supabase/functions/nucleo-mcp/index.ts to consume the new fields.
--
-- New fields:
-- - cv_extracted_text (PDF parseado, post-p195 VEP storage sync)
-- - resume_storage_path + resume_synced_at (private bucket reference)
-- - profile_* (PMI VEP LinkedIn enrichment: about_me, specialties, company, designation, industry,
--   certifications array, volunteer_interest, location, state, country, profile_linkedin_url)
-- - linkedin_relevant_posts + ai_pm_focus_tags
-- - service_history_* (PMI volunteer history: count, chapters, first/latest dates)
-- - pmi_memberships (jsonb)
-- - credly_url, is_open_to_volunteer
-- - is_returning_member + previous_cycles + application_count + returning_member_match
--   (was previously restricted to manage_member via get_application_returning_context)
-- - criteria_with_weights (computed: key, label, guide, weight, max, max_weighted_contribution,
--   your_score, your_weighted_contribution, your_criterion_note — pulled from caller's draft)
-- - max_weighted_subtotal (sum of weight*max across criteria — ceiling reference)
-- - pert_cutoff block (target_score, band_lower, band_upper, cohort_n, method, calc_at)
--
-- Auth: unchanged (committee membership of the cycle OR manage_platform).
-- PII access log: expanded array to include the new fields exposed.
-- Structure: application object built in 6 chunks merged via || to dodge PG's 100-arg
-- jsonb_build_object cap (initial single-call attempt failed with code 54023).
--
-- Rollback: this is a non-destructive payload expansion — drop and re-apply migration
-- 20260516170000_phase_b_batch20_get_evaluation_form_v4 (with p110 hotfix on top) to revert.
