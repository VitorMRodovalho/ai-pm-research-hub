-- `write_board` passa a ser perguntado COM o recurso nas 6 RPCs em que o board ja esta em maos.
--
-- Mesma classe do #1728 e do #1383: uma chamada de capacidade sem o recurso concreto nao distingue
-- o board proprio do alheio, porque um grant de escopo `initiative` casa qualquer iniciativa. O
-- helper abaixo faz a pergunta escopada, do mesmo jeito que `_manage_event_scope_ok` ja faz para
-- evento.
--
-- Escolha de desenho. NAO reusamos `board_write_authority`: ele e mais AMPLO que o ramo
-- substituido (tem lider de tribo, time de comunicacao e lider de iniciativa), e trocar um pelo
-- outro ALARGARIA a autoridade destas 6 RPCs. O helper e a traducao exata do ramo atual menos o
-- alcance cruzado. Os demais ramos de cada RPC ficam intocados.
--
-- Medido em 23/08/2026, antes e depois: as 54 pessoas cujo `write_board` e de escopo `initiative`
-- mantem **54 de 54** no proprio board e passam a **0** em board alheio. As 15 de escopo
-- organization seguem passando em todos os boards testados (75 pares, 0 negativas). Ninguem
-- legitimo perdeu acesso.
--
-- Estas 6 sao as que ja tinham o board em escopo E validado antes do portao. As demais RPCs de
-- `write_board` precisam de resolucao adicional (id anulavel, salto por comentario ou checklist)
-- e ficam para o lote seguinte.

-- ── helper ───────────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION public._write_board_scope_ok(p_member_id uuid, p_board_id uuid)
 RETURNS boolean
 LANGUAGE sql
...
COMMENT ON FUNCTION public._write_board_scope_ok(uuid, uuid) IS
...
REVOKE ALL ON FUNCTION public._write_board_scope_ok(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public._write_board_scope_ok(uuid, uuid) TO authenticated, service_role;
...
BEGIN
  ...
  IF NOT public._write_board_scope_ok(v_caller.id, p_board_id) AND NOT v_is_tribe_member AND NOT (
...
BEGIN
  ...
  v_authorized := public._write_board_scope_ok(v_actor, v_board_id)
...
BEGIN
  ...
  IF NOT public._write_board_scope_ok(v_actor.id, v_board_id) AND NOT v_is_card_owner AND NOT v_is_comms_for_domain AND NOT v_is_initiative_leader THEN
...
BEGIN
  ...
  IF NOT public.rls_can_see_item(p_board_item_id) THEN
    RAISE EXCEPTION 'Unauthorized: cannot access this card';
  END IF;
  IF NOT (public._write_board_scope_ok(v_member_id, v_board_id)
...
BEGIN
  ...
  IF NOT public._write_board_scope_ok(v_caller_id, p_board_id) THEN
