-- #301 / ADR-0108: add invariant AM_drive_curation_grant_terminal_consistency.
-- Full CREATE OR REPLACE rebuild of check_schema_invariants (body-hash drift gate requires the
-- latest migration to carry the complete current body). Captured verbatim from the prior latest
-- (AL, migration 20260805000263) + the new AM block appended before END. Total 39 -> 40.
