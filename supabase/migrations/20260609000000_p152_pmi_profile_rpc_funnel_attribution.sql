-- p152 W4 GAP-152.1 (2026-05-12) — funnel attribution na PMI Profile tab.
--
-- Strategic: PM benefit from seeing "where did candidate come from?" pre-interview.
-- Columns existem desde p86+: referral_source (text), referrer_member_id (uuid),
-- utm_data (jsonb), imported_at, vep_application_id, vep_opportunity_id.
-- Mas nunca foram surfaced no modal. Lazy-load via get_application_pmi_profile
-- (em vez de dashboard, evita 100-arg ceiling).
--
-- Adds 'funnel' section to RPC return jsonb:
-- - referral_source (text, e.g. 'linkedin' / 'pmigo_email')
-- - referrer_member_id + referrer_member_name (resolved JOIN)
-- - utm_data (jsonb — campaign analytics)
-- - imported_at (timestamp of /ingest)
-- - vep_application_id + vep_opportunity_id (cross-ref VEP)
