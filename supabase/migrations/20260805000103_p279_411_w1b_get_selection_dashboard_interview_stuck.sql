-- p279 #411 Wave 1b — surface meta.interview_stuck in get_selection_dashboard
--
-- WHAT: Extend public.get_selection_dashboard payload's applications[].meta chunk
--       with `interview_stuck` (boolean). True iff the application is parked in
--       status 'interview_scheduled' AND it has at least one selection_interviews
--       row that is still status='scheduled', already past (scheduled_at < now()),
--       and never conducted (conducted_at IS NULL).
--
-- WHY: Issue #411 Wave 1b adds a toolbar filter chip "Stuck scheduled" (F2) and
--      Wave 1d a modal rescue button (F4). Both need a per-row boolean that the
--      table/modal can read directly. The existing payload exposes
--      meta.interview_scheduled (EXISTS scheduled|completed|rescheduled — too
--      broad) and meta.interview_next_at (MIN scheduled_at within a 12h future
--      grace — NULL for a 12-day-old stuck interview), but NEITHER exposes the
--      precise stuck predicate, which requires the latest interview's conducted_at
--      and past scheduled_at. selection_interviews / selection_applications are
--      both rpc_only_deny_all (qual=false) so the frontend cannot SELECT them
--      directly; get_selection_dashboard is the existing SECDEF read surface and
--      the cleanest insertion point. Adding one boolean key is a no-op for current
--      consumers (additive jsonb key).
--
-- PREDICATE (canonical — shared by the F2 chip and the F4 rescue button):
--      app.status = 'interview_scheduled'
--      AND EXISTS (selection_interviews si WHERE si.application_id = a.id
--                  AND si.status = 'scheduled'
--                  AND si.conducted_at IS NULL
--                  AND si.scheduled_at IS NOT NULL
--                  AND si.scheduled_at < now())
--      The app.status='interview_scheduled' clause makes the flag robust against
--      false positives on applications that already advanced (interview_done /
--      final_eval / approved) but still carry an un-updated stale interview row.
--      It is a strict superset of the SPEC §F4 button text (interview-anchored
--      only); including app.status never over-shows — it only suppresses already-
--      advanced apps, which is the safer UX. The Wave 2b rescue cron applies an
--      additional 48h grace window on top of this same interview-anchored core.
--
-- SCOPE: Read-side only — no writes, no signature change, no migrations to other
--      RPCs. Pairs with src/pages/admin/selection.astro Wave-1b chip wiring in
--      the same PR.
--
-- SEDIMENT-238.C: CREATE OR REPLACE FUNCTION preserves the existing DEFAULT
--      clause `p_cycle_code text DEFAULT NULL` byte-for-byte. No signature change.
--      SECDEF + search_path pinned (public, pg_temp).
--
-- PHASE C DRIFT NOTE (SEDIMENT-269.A): the body below is the literal extension of
--      the migration-52 body (the last shipped get_selection_dashboard), with ONE
--      new line inserted inside the per-app `meta` jsonb_build_object, between
--      `'video_screening_done', ...` and the closing `)`. Diff vs migration 52 =
--      1 added line. Phase C md5 will change; new hash recorded post-apply.
--
-- ROLLBACK: re-issue CREATE OR REPLACE FUNCTION public.get_selection_dashboard(text)
--      with the pre-p279 body (last shipped via 20260805000052 / p271 #411 W1a).
--      The interview_stuck field then drops from payload; the frontend chip + F4
--      button fall back to "never stuck" since `r.meta?.interview_stuck === true`
--      evaluates falsy on undefined.
