-- p197b (2026-05-19): extend get_selection_dashboard cycle object with pert_cutoff block
--
-- Context: p197 enriched get_evaluation_form so individual evaluators see the cutoff before
-- submitting. p197b complements that for the committee dashboard at /admin/selection — the
-- ranking table needs the cycle-level cutoff visible (chip in header) and per-row color-code
-- (research/leader_score vs band). Powers the UX the PM flagged: "nota de corte que é a mediana
-- dos atuais membros ativos... nem as notas finais estarem color code por esta metrificação".
--
-- Changes:
-- - cycle object now includes pert_cutoff: { target_score, band_lower, band_upper, cohort_n,
--   method, calc_at, apps_with_pert, apps_total }
-- - Aggregated via MAX since pert_* are identical across applications in a cycle (computed
--   by compute_pert_cutoff RPC, persisted per-row for query convenience).
--
-- All other behavior preserved exactly (applications + stats untouched). Auth unchanged
-- (view_internal_analytics gate).
--
-- Rollback: drop and re-apply prior get_selection_dashboard version.

DROP FUNCTION IF EXISTS public.get_selection_dashboard(text);
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'cycle', (SELECT jsonb_build_object(
      ...
      -- p197b: PERT cutoff aggregated from selection_applications (same per cycle by design)
...
COMMENT ON FUNCTION public.get_selection_dashboard(text) IS
  'p197b (2026-05-19): cycle object now includes pert_cutoff block (target_score, band_lower, '
  'band_upper, cohort_n, method, calc_at, apps_with_pert, apps_total). Powers /admin/selection UI '
  'cutoff chip + ranking row color-code (research/leader_score vs band). Aggregated via MAX since '
  'pert_* values are identical across applications in the same cycle (computed by compute_pert_cutoff '
  'RPC, persisted per-row). Other behavior unchanged from p195.';
