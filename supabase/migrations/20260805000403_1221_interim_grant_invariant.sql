-- #1221 fatia 1 (Part B) / ADR-0121: append invariant AP_interim_grant_reverted_when_cert_issued to
-- check_schema_invariants(). Deferred from #1117 (the ADR-0121 interim leader-grant lifecycle guard):
-- an engagement must not carry BOTH metadata->>'interim_grant'=true AND agreement_certificate_id (the
-- interim flag must be reverted once the real term is signed). Baseline 0 (zero interim flags live).
--
-- Full function re-emitted from its LIVE definition (mig 20260805000398, the latest CREATE OR REPLACE;
-- normalized md5 verified against live before authoring) + the appended AP block. Body up to AO is
-- byte-identical to live so the Phase-C body-drift gate stays green on the unchanged invariants.
