-- #1276 fix(selection): get_selection_health quebrado — onboarding_tokens.created_at inexistente
-- A coluna de tempo real da tabela é `issued_at` (não `created_at`). O bloco de stale-tokens
-- referenciava `t.created_at`, causando "column t.created_at does not exist" em runtime e cegando
-- o dashboard de saúde do funil de seleção. Fix de 1 linha; base no corpo VIVO (pg_get_functiondef).
