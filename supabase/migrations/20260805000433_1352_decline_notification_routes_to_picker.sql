-- #1352 — requester-side journey: a declined tribe request must route the requester to the
-- picker (where they choose another tribe), not to the tribe that just rejected them.
--
-- Before: review_tribe_request set the outcome notification link to '/tribe/<legacy_tribe_id>' for
-- BOTH approve and decline. On approve that is correct (it is now the requester's tribe). On decline
-- it dead-ends the requester at the tribe that rejected them, with no path to re-pick. The picker
-- (TribeRequestBlock) lives at /workspace.
--
-- After: decline routes to /workspace and the body names the next step ("escolha outra tribo no seu
-- painel"); approve is unchanged. This closes the tribe-journey triad (#1350 request gate, #1351 GP
-- alert, #1352 requester journey).
--
-- Re-captured from the live body (drift-safe CREATE OR REPLACE); the only changes vs live are the
-- decline branch of the notification body + the link CASE. No behavioural change to authority,
-- capacity, the atomic switch (#1263), or idempotency (#1280).
