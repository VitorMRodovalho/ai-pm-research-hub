-- ════════════════════════════════════════════════════════════════
-- p277 triage: invariant R honors alternate emails (#420 sibling / Issue #180 class)
-- ════════════════════════════════════════════════════════════════
...
-- recorded emails satisfies "approved application has a member". The pejota81 alternate
-- was backfilled onto Paulo via the canonical member_add_alternate_email() RPC (one-time
-- data correction, not hardcoded here per the no-generated-ids-in-data-migration rule).
-- All other 22 invariants are byte-identical to the prior body (sourced from migration
-- 20260805000046; Phase-C md5 parity verified). R's description + status='approved' scope +
-- lower(m.email)=lower(a.email) clause are preserved (existing contract tests stay green).
-- ROLLBACK: re-apply 20260805000046's check_schema_invariants body (drops the member_emails clause).
-- ════════════════════════════════════════════════════════════════
...
-- Sanity: after extension (+ the Paulo alternate already committed), R must be 0.
DO $sanity$
DECLARE v_r integer;
BEGIN
  SELECT violation_count INTO v_r FROM public.check_schema_invariants()
  WHERE invariant_name = 'R_approved_application_has_member';
  IF v_r IS DISTINCT FROM 0 THEN
    RAISE EXCEPTION 'p277 invariant-R fix: R still reports % violation(s) post-apply', v_r;
  END IF;
END
$sanity$;
