-- #1446: get_selection_dashboard(NULL) must default to the current OPEN cycle,
-- not the most-recently-created row. A historical cycle re-imported later
-- (cycle2-2025 imported 2026-07-13) gets a newer created_at and was hijacking the
-- default, shadowing the only open cycle (cycle4-2026, created 2026-05-09). This made
-- /admin/selection, GpActionTodayWidget and the MCP surface open on the wrong cycle.
-- Only the ELSE branch of the default-cycle resolution changes; the rest of the body
-- is reproduced byte-for-byte from the live definition (GC-097 / body-drift discipline).
