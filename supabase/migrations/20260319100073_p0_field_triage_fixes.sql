...
-- P0-4: Fix get_admin_dashboard — .full_name → .name + gamification_points.cycle → created_at filter
  ...
  SELECT jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'cpmai_current', (
        ...
        WHERE category = 'cert_cpmai' AND created_at >= v_cycle_start
      ),
      'cpmai_target', (
        SELECT target_value FROM annual_kpi_targets
        WHERE kpi_key = 'cpmai_certifications' AND cycle = (SELECT sort_order FROM cycles WHERE is_current = true LIMIT 1)
        LIMIT 1
      ),
      'chapters_current', (SELECT count(DISTINCT chapter) FROM members WHERE is_active = true AND chapter IS NOT NULL),
      'chapters_target', (
        SELECT target_value FROM annual_kpi_targets
        WHERE kpi_key = 'chapters_participating' AND cycle = (SELECT sort_order FROM cycles WHERE is_current = true LIMIT 1)
        LIMIT 1
      )
    ),
    'alerts', (
      SELECT COALESCE(jsonb_agg(alert), '[]'::jsonb) FROM (
        SELECT jsonb_build_object('severity', 'high', 'message', count(*) || ' membros sem tribo', 'action_label', 'Ir para Tribos', 'action_href', '/admin/tribes') as alert
        FROM members WHERE is_active = true AND tribe_id IS NULL HAVING count(*) > 0
        UNION ALL
        SELECT jsonb_build_object('severity', 'medium', 'message', count(*) || ' stakeholders sem conta', 'action_label', 'Ver Membros', 'action_href', '/admin/members')
        FROM members WHERE is_active = true AND auth_id IS NULL AND operational_role IN ('sponsor', 'chapter_liaison') HAVING count(*) > 0
        UNION ALL
        SELECT jsonb_build_object('severity', 'medium', 'message', count(*) || ' membros em risco de dropout', 'action_label', 'Ver lista', 'action_href', '/admin/members')
        FROM members m WHERE m.is_active = true AND m.tribe_id IS NOT NULL
        AND m.id NOT IN (SELECT a.member_id FROM attendance a JOIN events e ON e.id = a.event_id WHERE e.date > now() - interval '60 days')
        HAVING count(*) > 0
      ) sub
    ),
    'recent_activity', (
      ...
      FROM (
        SELECT * FROM (
          SELECT jsonb_build_object('type', 'audit', 'message', actor.name || ' ' || al.action || ' em ' || COALESCE(target.name, '?'), 'details', al.changes, 'timestamp', al.created_at) as activity, al.created_at as ts
          FROM admin_audit_log al LEFT JOIN members actor ON actor.id = al.actor_id LEFT JOIN members target ON target.id = al.target_id
          WHERE al.created_at > now() - interval '7 days' ORDER BY al.created_at DESC LIMIT 10
        ) a1
        UNION ALL
        SELECT * FROM (
          SELECT jsonb_build_object('type', 'campaign', 'message', 'Campanha "' || ct.name || '" enviada', 'timestamp', cs.created_at), cs.created_at
          FROM campaign_sends cs JOIN campaign_templates ct ON ct.id = cs.template_id
          WHERE cs.created_at > now() - interval '7 days' ORDER BY cs.created_at DESC LIMIT 5
        ) a2
        UNION ALL
        SELECT * FROM (
