-- p152 W4 OPP-152.7-lite (2026-05-12) — sibling applications na PMI Profile tab.
--
-- Strategic: PM observation 12/05 — William Junio aplicou pra researcher (id
-- 6187b0b2) + leader (97a6df7d). Booking flow ties interviews to ONE
-- application (leader). Researcher fica sem evidência de entrevista. PM ask:
-- "uma oportunidade para entrevistas posteriores entao, pq facilita vc manter
-- historico de reagendamento, a entrevista em si, dentre outros dados
-- agregados que pode vir".
--
-- MVP scope: surface SIBLING applications (same email + SAME cycle, different
-- application_id) na PMI Profile tab. PM sees "este candidato tem outra
-- aplicação (researcher) — score também lá" hint. Manual coverage por enquanto.
--
-- Bug fix paralelo: previous_cycles antes incluía siblings (same cycle). Now
-- correctly filters cycle_id <> v_app.cycle_id (siblings vão pra dedicada).
