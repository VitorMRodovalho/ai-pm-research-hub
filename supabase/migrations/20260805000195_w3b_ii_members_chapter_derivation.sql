-- Wave 3b-ii (#740 / ADR-0104) — members.chapter becomes a derived/compat value.
--
-- members.chapter is now MAINTAINED by triggers from the two canonical sources:
--   COALESCE('PMI-' || entry_chapter_code, 'PMI-' || primary affiliation code, legacy chapter)
-- so every existing read-site (get_chapter_dashboard/needs/kpis, admin_list_members(p_chapter),
-- exec_chapter_webinar_metrics, /admin/chapter*.astro, leaderboards) keeps reading members.chapter
-- but now sees the member's REAL entry/primary chapter — no per-site repoint needed.
--
-- DESIGN — event-specific triggers (NOT a blanket BEFORE override), so that the admin free-text
-- edit path (admin_update_member / admin_update_member_audited: chapter = COALESCE(p_chapter, chapter))
-- is NOT silently overridden: those UPDATEs set `chapter` but never `entry_chapter_code`, so T1 does
-- not fire; and they do not touch member_chapter_affiliations, so T2 does not fire. The SSOT paths
-- (set_my_entry_chapter, the pmi-vep-sync worker) DO drive the value:
--   T1: BEFORE UPDATE OF entry_chapter_code ON members          → entry choice drives chapter
--   T2: AFTER INS/DEL/UPD OF is_primary ON member_chapter_affiliations → worker/primary drives chapter
--
-- Backfill is a NO-OP for current data: the 3a backfill seeded affiliations FROM members.chapter, so
-- 'PMI-' || primary_code == the current members.chapter for all 72 active registry members; alumni/
-- inactive (no affiliations) and Outro/Externo (no registry chapter) fall through to the legacy value.
--
-- Alumni/inactive decision (ADR-0104 contract): the legacy members.chapter stays authoritative for
-- non-active members (no affiliation backfill for them) — the COALESCE fall-through preserves it.
--
-- Cross-ref: ADR-0104, ADR-0006 (person_id), #740 Wave 3.
--
-- ROLLBACK: DROP TRIGGER IF EXISTS trg_z_derive_member_chapter ON public.members;
--           DROP TRIGGER IF EXISTS trg_recompute_member_chapter_on_affiliation ON public.member_chapter_affiliations;
--           DROP FUNCTION IF EXISTS public.derive_member_chapter_before();
--           DROP FUNCTION IF EXISTS public.recompute_member_chapter_from_affiliation();
--           then re-apply the prior check_schema_invariants() body (without invariant U). members.chapter
--           data is unaffected (backfill was a no-op); the column simply stops being trigger-maintained.

-- ── T1: entry-chapter choice drives members.chapter (BEFORE, sets NEW.chapter in place) ──────────
CREATE OR REPLACE FUNCTION public.derive_member_chapter_before()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
BEGIN
  NEW.chapter := COALESCE(
    CASE WHEN NEW.entry_chapter_code IS NOT NULL AND NEW.entry_chapter_code <> ''
         THEN 'PMI-' || NEW.entry_chapter_code END,
    CASE WHEN NEW.person_id IS NOT NULL THEN (
           SELECT 'PMI-' || a.chapter_code
             FROM public.member_chapter_affiliations a
            WHERE a.person_id = NEW.person_id AND a.is_primary
...
DROP TRIGGER IF EXISTS trg_z_derive_member_chapter ON public.members;
CREATE TRIGGER trg_z_derive_member_chapter
  BEFORE UPDATE OF entry_chapter_code ON public.members
  FOR EACH ROW
  EXECUTE FUNCTION public.derive_member_chapter_before();

-- ── T2: primary affiliation change drives members.chapter (AFTER, updates the affected member) ───
CREATE OR REPLACE FUNCTION public.recompute_member_chapter_from_affiliation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
AS $function$
DECLARE
  v_pid uuid := COALESCE(NEW.person_id, OLD.person_id);
BEGIN
  -- Guarded by IS DISTINCT FROM so it only writes (and only cascades the other members
  -- triggers) when the derived value actually changes. This UPDATE sets chapter+updated_at
  -- only, never entry_chapter_code, so T1 does not fire → no recursion / no double-compute.
  UPDATE public.members m
     SET chapter = COALESCE(
           CASE WHEN m.entry_chapter_code IS NOT NULL AND m.entry_chapter_code <> ''
                THEN 'PMI-' || m.entry_chapter_code END,
           (SELECT 'PMI-' || a.chapter_code
              FROM public.member_chapter_affiliations a
             WHERE a.person_id = m.person_id AND a.is_primary
             ORDER BY a.updated_at DESC
             LIMIT 1),
           m.chapter),
         updated_at = now()
   WHERE m.person_id = v_pid
     AND m.chapter IS DISTINCT FROM COALESCE(
           CASE WHEN m.entry_chapter_code IS NOT NULL AND m.entry_chapter_code <> ''
                THEN 'PMI-' || m.entry_chapter_code END,
           (SELECT 'PMI-' || a.chapter_code
              FROM public.member_chapter_affiliations a
...
DROP TRIGGER IF EXISTS trg_recompute_member_chapter_on_affiliation ON public.member_chapter_affiliations;
CREATE TRIGGER trg_recompute_member_chapter_on_affiliation
  AFTER INSERT OR DELETE OR UPDATE OF is_primary ON public.member_chapter_affiliations
  FOR EACH ROW
  EXECUTE FUNCTION public.recompute_member_chapter_from_affiliation();

-- ── One-time backfill recompute (expected: 0 rows changed — affiliations were seeded FROM chapter) ─
UPDATE public.members m
   SET chapter = COALESCE(
         CASE WHEN m.entry_chapter_code IS NOT NULL AND m.entry_chapter_code <> ''
              THEN 'PMI-' || m.entry_chapter_code END,
...
-- ── Invariant U: every active registry-chaptered member has exactly one primary affiliation ──────
-- Added to check_schema_invariants() (CREATE OR REPLACE, signature unchanged). Reproduces the live
-- body verbatim and appends the U block before END. Non-registry chapters (Outro/Externo) are
-- excluded — they are legitimately unaffiliated; the COALESCE derivation falls through to legacy.
...
BEGIN
  ...
  SELECT 'U_active_person_has_primary_chapter_affiliation'::text,
         'every active registry-chaptered member''s person_id must have exactly one is_primary=true member_chapter_affiliations row, else the members.chapter COALESCE(entry, primary, legacy) derivation breaks silently (ADR-0104 Wave 3b-ii). Non-registry chapters (Outro/Externo) excluded — legitimately unaffiliated.'::text,
         'high'::text, COUNT(*)::integer,
         (SELECT array_agg(member_id ORDER BY member_id) FROM (SELECT member_id FROM drift LIMIT 10) s)
  FROM drift;
