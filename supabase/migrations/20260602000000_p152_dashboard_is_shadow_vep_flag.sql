-- p152 Wave 1.0 (2026-05-12) — is_shadow_vep flag on get_selection_dashboard.
--
-- Strategic context: handoff_p152 backlog item 0 (UX filter stop-gap before
-- frontend reconciliation route). PM discovered cycle 3-b2 shows 5 "shadow VEP"
-- entries — existing active members for whom the PMI recruiter created a new
-- VEP application in Batch 2 (likely bulk-process post-aprovação Núcleo). They
-- are NOT genuine new candidates and inflate the funnel cognitive load.
--
-- Heuristic for "shadow VEP":
--   application.status is terminal (approved/converted/cancelled/rejected/withdrawn)
--   AND there exists an active member (is_active=true) with same email whose
--   created_at < application.created_at (member existed BEFORE this app —
--   so this app is NOT the original conversion).
--
-- This conservatively excludes the ORIGINAL conversion application (where
-- member.created_at > application.created_at, because onboarding happens AFTER
-- applying). It also leaves pending applications visible even for active
-- members (legitimate promotion-track re-applications stay reviewable).
--
-- Added to existing extra_flags subgroup → 0 outer args added (sub-object
-- grows from 12 → 14 args internally, well under 100-arg ceiling).
--
-- Frontend (separate commit): new quick filter button "Ocultar shadow VEP"
-- (default ON) consumes r.extra_flags.is_shadow_vep.
    ...
    'applications', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'extra_flags', jsonb_build_object(
          ...
          -- p152 Wave 1.0: shadow VEP detection (active member with same email
          -- created before this app + this app already in terminal state).
          'is_shadow_vep', (
