-- PR 2 of #766 item 2/4 (server-side milestones framework).
-- See docs/specs/SPEC_766_SERVER_SIDE_MILESTONES.md §7 (PR2).
--
-- Adds the `term_signed` milestone: recorded when a member's volunteer_agreement
-- certificate is issued. Mirrors the sibling _trg_complete_volunteer_term_on_cert
-- (mig 20260805000018) trigger surface (AFTER INSERT ON certificates, gated by a WHEN
-- clause). Reuses record_milestone() from PR1 (mig 20260805000201); the table, RLS and
-- the 3 RPCs are unchanged.
--
-- Backfill is SILENT (acknowledged_at = now()) for the 40 members who already hold an
-- issued volunteer_agreement cert, so they are NOT re-celebrated. Race-safe: the backfill
-- runs BEFORE CREATE TRIGGER (SPEC §6.3). occurred_at = the cert's issued_at (the real
-- signing moment); source_id = the earliest such cert per member.
--
-- FE: NO term_signed celebration card in this PR (ux-leader DEFER-FE — the OnboardingChecklist
-- HBLOCK "first days" card already owns the just-signed beat; the generic multi-milestone
-- surface lands after PR3 adds first_attendance/first_deliverable). The only FE change in
-- this PR is the HBLOCK copy fix (future- vs past-oriented body keyed on volunteer_term).
--
-- Invariant AB added to check_schema_invariants() (28 -> 29): a term_signed milestone must
-- have a volunteer_agreement cert of ANY status for the member. Directional & Wave-3c-safe.
--
-- ROLLBACK:
--   DROP TRIGGER IF EXISTS trg_record_term_signed_milestone ON public.certificates;
--   DROP FUNCTION IF EXISTS public._trg_record_term_signed_milestone();
--   DELETE FROM public.member_milestones WHERE milestone_key = 'term_signed';
--   -- re-apply 20260805000200's check_schema_invariants() body (without the AB block).
--   NOTIFY pgrst, 'reload schema';

-- 1. Trigger function — record term_signed when a volunteer_agreement cert is issued.
CREATE OR REPLACE FUNCTION public._trg_record_term_signed_milestone()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
BEGIN
  IF NEW.member_id IS NULL THEN RETURN NEW; END IF;
  PERFORM public.record_milestone(
    NEW.member_id, 'term_signed', 'certificate', NEW.id,
    jsonb_build_object('via', 'certificate_trigger', 'verification_code', NEW.verification_code)
  );
  RETURN NEW;
END; $fn$;
REVOKE ALL ON FUNCTION public._trg_record_term_signed_milestone() FROM PUBLIC;

-- 2. Silent backfill of the existing issued volunteer_agreement signers. MUST run BEFORE
--    CREATE TRIGGER (race-safe, SPEC §6.3). DISTINCT ON earliest cert per member.
...
SELECT DISTINCT ON (c.member_id)
  c.member_id, 'term_signed', c.issued_at, 'certificate', c.id, now(),
  jsonb_build_object('backfill', true, 'migration', '20260805000202')
FROM public.certificates c
WHERE c.type = 'volunteer_agreement' AND c.status = 'issued'
ORDER BY c.member_id, c.issued_at ASC
ON CONFLICT (member_id, milestone_key) DO NOTHING;

-- 3. Sanity: every member with an issued volunteer_agreement cert now has a term_signed
--    milestone (mirrors the …018 sanity guard). Fails the migration if backfill missed any.
DO $sanity$
DECLARE v_missing int;
BEGIN
  SELECT count(*) INTO v_missing
  FROM (SELECT DISTINCT member_id FROM public.certificates
        WHERE type='volunteer_agreement' AND status='issued' AND member_id IS NOT NULL) c
  WHERE NOT EXISTS (
    SELECT 1 FROM public.member_milestones mm
    WHERE mm.member_id = c.member_id AND mm.milestone_key = 'term_signed'
  );
  IF v_missing > 0 THEN
    RAISE EXCEPTION 'PR2 #766 backfill sanity FAIL: % issued-cert members lack a term_signed milestone', v_missing;
  END IF;
  RAISE NOTICE 'PR2 #766 backfill sanity OK: all issued-cert members have term_signed.';
END$sanity$;

-- 4. Trigger — AFTER INSERT only (mirrors the sibling; no draft->issued UPDATE path exists
--    today, so an UPDATE arm would be speculative. If such a path is ever added, add the
--    UPDATE arm to BOTH this trigger and _trg_complete_volunteer_term_on_cert atomically).
--    status guard is explicit in the WHEN clause (more conservative than the sibling).
DROP TRIGGER IF EXISTS trg_record_term_signed_milestone ON public.certificates;
CREATE TRIGGER trg_record_term_signed_milestone
  AFTER INSERT ON public.certificates
  FOR EACH ROW
  WHEN (NEW.type = 'volunteer_agreement' AND NEW.status = 'issued')
  EXECUTE FUNCTION public._trg_record_term_signed_milestone();

COMMENT ON FUNCTION public._trg_record_term_signed_milestone() IS
  '#766 PR2: records the term_signed member_milestone when a volunteer_agreement certificate is issued (AFTER INSERT, gated by WHEN type+status). Idempotent / first-occurrence via record_milestone ON CONFLICT DO NOTHING. Sibling of _trg_complete_volunteer_term_on_cert.';

-- 5. check_schema_invariants() with the AB invariant appended. Body reproduced verbatim
--    from 20260805000200 (28 invariants, byte-equal) with the AB RETURN QUERY block added
--    before END. Live is patched via an equivalent DO-block transform (pg_get_functiondef +
--    anchored insert) to avoid re-transcribing the 22k-char body; this file is the canonical
--    capture for the rpc-migration-coverage Phase C gate (file body == live prosrc, normalized).
