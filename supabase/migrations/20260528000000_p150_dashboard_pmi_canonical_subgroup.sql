-- p150 (2026-05-12) — add pmi_canonical subgroup to get_selection_dashboard.
--
-- PM feedback: "coluna Capitulo, parece que nao esta trazendo a informacao
-- Reliable, do proprio PMI, acho que esta trazendo do form de aplicacao".
-- CONFIRMED: today's `chapter` field is form-declared (free text by candidate).
-- The PMI-canonical source is `service_history_chapters` (Member History agg
-- text, e.g. "Ceará, Brazil Chapter;PMI Global"). pmi_memberships jsonb is
-- the planned canonical but currently NULL on 32/32 cycle 4 — worker via
-- /ingest sets pmi_data_fetched_at but the array stays NULL (path drift;
-- script-mapper.ts:72 expects array, JSON delivers double-encoded string).
--
-- Fix: derive pmi_canonical subgroup on-read from existing fields so the UI
-- can show reliable info even without re-ingest. Backfill of pmi_memberships
-- jsonb is separate work (worker patch + JSON re-parse).
--
-- Sub-object reduces args (one key/value pair instead of 7 flat fields),
-- staying within the 100-args jsonb_build_object limit hit by p150 hotfix.
  ...
  SELECT jsonb_build_object(
    ...
    'applications', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        -- p150 PM feedback: pmi_canonical subgroup. Derived on-read from existing
        -- fields so UI can show reliable chapter + member status even without
        -- pmi_memberships jsonb backfill. 7 sub-fields = 1 outer arg.
        'pmi_canonical', jsonb_build_object(
          'chapter_canonical', (
            -- First chapter from service_history_chapters (PMI Member History agg),
            -- excluding "PMI Global". Returns NULL if no specific chapter found.
            -- Example input: "Ceará, Brazil Chapter;PMI Global" → "Ceará, Brazil Chapter"
            SELECT trim(c)
            FROM unnest(string_to_array(COALESCE(a.service_history_chapters, ''), ';')) AS c
            WHERE trim(c) <> '' AND trim(c) <> 'PMI Global'
            LIMIT 1
          ),
