-- ═══════════════════════════════════════════════════════════════
-- Exclude initiative-scoped events from main attendance grids
--
-- Bug reported by Fabrício (T6 leader): after changing geral rito from
-- weekly to biweekly, 15/Abr still appeared with "expectation of geral"
-- for all 51 members. Root cause: CPMAI Kickoff on 2026-04-15 was tagged
-- type='geral' AND initiative_id=CPMAI. The grid RPCs enrolled every
-- member via "WHEN type IN ('geral','kickoff')" without checking
-- initiative_id, so non-CPMAI members got marked absent and rates dropped.
--
-- Structural fix: initiative-scoped events (geral/kickoff/lideranca/comms
-- with initiative_id set) belong on their initiative page, not the main
-- attendance grid. Tribe events (type='tribo' with initiative_id) stay
-- because they already scope correctly via tribe_id.
--
-- Rollback: remove the initiative_id filter from grid_events CTE in both.
-- ═══════════════════════════════════════════════════════════════
...
DECLARE
  v_caller record; v_cycle_start date; v_result jsonb;
BEGIN
  ...
  IF v_caller.is_superadmin IS NOT TRUE
    ...
    IF v_caller.tribe_id IS NOT NULL THEN p_tribe_id := v_caller.tribe_id;
    ELSE RETURN jsonb_build_object('error', 'No tribe assigned'); END IF;
  END IF;
...
DECLARE v_caller record; v_cycle_start date; v_result jsonb;
BEGIN
  SELECT * INTO v_caller FROM members WHERE auth_id = auth.uid();
  IF NOT FOUND THEN RETURN jsonb_build_object('error', 'Unauthorized'); END IF;
  IF v_caller.is_superadmin IS NOT TRUE
  ...
  grid_events AS (
    ...
    FROM events e LEFT JOIN tribes t ON t.id = e.tribe_id
    WHERE e.date >= v_cycle_start
