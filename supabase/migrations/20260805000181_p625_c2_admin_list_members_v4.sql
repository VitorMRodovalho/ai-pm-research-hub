-- #625 C2: admin_list_members vira V4-native.
-- Adiciona filtros por iniciativa/capítulo/ciclo + campos derivados de engagements:
--   engagements[] (kind/role/initiative + labels do catálogo), cycles[] (member_cycle_history),
--   term_status (🟢 green / 🟡 amber). 🔴 vencido fica para #571 (sem âncora de validade hoje).
-- DROP+CREATE porque a assinatura muda (3 params novos) — GC-097.
DROP FUNCTION IF EXISTS public.admin_list_members(text, text, integer, text);
...
COMMENT ON FUNCTION public.admin_list_members(text, text, integer, text, uuid, text, text) IS
  '#625 C2: lista membros V4-native; filtros iniciativa/capitulo/ciclo + engagements[]/cycles[]/term_status; pre-onboarding via member_is_pre_onboarding.';

REVOKE ALL ON FUNCTION public.admin_list_members(text, text, integer, text, uuid, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_list_members(text, text, integer, text, uuid, text, text) TO authenticated;
