-- ═══════════════════════════════════════════════════════════════
-- Fix articles_published: count only curated articles, not template cards
-- Old filter: status IN ('published', 'approved', 'done') — caught board
--   instruction cards with status='done' that aren't real articles
-- New filter: curation_status = 'approved' — only articles that passed
--   the curation committee review count as published
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    CASE v_target.metric_key
      -- a) ARTICLES: only count items approved by curation committee
