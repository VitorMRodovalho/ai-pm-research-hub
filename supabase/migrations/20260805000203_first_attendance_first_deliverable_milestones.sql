-- PR 3 of #766 item 2/4 (server-side milestones framework).
-- See docs/specs/SPEC_766_SERVER_SIDE_MILESTONES.md §7 (PR3).
--
-- Adds two milestones: `first_attendance` and `first_deliverable`. Each mirrors an
-- existing sibling trigger surface so the milestone fires on the SAME domain event:
--   * first_attendance  -> mirrors auto_complete_first_meeting() (AFTER INSERT ON
--     attendance, WHEN present=true). INSERT-only: the sibling is INSERT-only, and an
--     absence-corrected-to-present (false->true UPDATE) is a pre-existing equivalence
--     gap in the sibling, deferred. "First" is guaranteed by member_milestones'
--     UNIQUE(member_id,milestone_key) + record_milestone ON CONFLICT DO NOTHING.
--   * first_deliverable -> mirrors trg_tribe_deliverable_completed_xp() (AFTER INSERT OR
--     UPDATE ON tribe_deliverables). Keyed on status='completed' (the canonical "done"
--     signal and the same event as the XP grant), NOT on completed_at — completed_at is a
--     derived audit column the XP trigger itself sets in the same transaction, so keying a
--     second trigger on it would be a fragile ordering dependency.
--
-- Backfill is SILENT (acknowledged_at = now()) so existing members are NOT re-celebrated;
-- only post-deploy events fire a pending milestone. Race-safe: backfills run BEFORE the
-- CREATE TRIGGER statements (SPEC §6.3). occurred_at = the real event moment
-- (attendance.checked_in_at / tribe_deliverables.completed_at), COALESCEd to a non-null
-- timestamp because both source columns are nullable and member_milestones.occurred_at is
-- NOT NULL. Cohorts at apply time (live, 2026-06-17): 60 present-attendance members; 1
-- completed-deliverable member (status='completed', completed_at IS NULL on that row — the
-- reason AD and the backfill filter key on status, not completed_at).
--
-- Invariants AC + AD appended to check_schema_invariants() (29 -> 31), mirroring AA/AB:
-- source_id is informational-only without FK, so these directional milestone->source
-- checks are the only integrity guard against a fabricated/bad-backfilled milestone.
--
-- FE: new global island MilestoneCelebration.tsx (mounted in BaseLayout) celebrates
-- first_attendance + first_deliverable from get_my_milestones().pending; onboarding_complete
-- stays owned by OnboardingChecklist and term_signed stays deferred (ux-leader).
--
-- ROLLBACK:
--   DROP TRIGGER IF EXISTS trg_record_first_attendance_milestone ON public.attendance;
--   DROP TRIGGER IF EXISTS trg_record_first_deliverable_milestone ON public.tribe_deliverables;
--   DROP FUNCTION IF EXISTS public._trg_record_first_attendance_milestone();
--   DROP FUNCTION IF EXISTS public._trg_record_first_deliverable_milestone();
--   DELETE FROM public.member_milestones WHERE milestone_key IN ('first_attendance','first_deliverable');
--   -- re-apply 20260805000202's check_schema_invariants() body (without the AC/AD blocks).
--   NOTIFY pgrst, 'reload schema';

-- 1. Trigger functions -------------------------------------------------------------------

-- first_attendance: mirrors auto_complete_first_meeting (AFTER INSERT WHEN present=true).
CREATE OR REPLACE FUNCTION public._trg_record_first_attendance_milestone()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
BEGIN
  IF NEW.member_id IS NULL THEN RETURN NEW; END IF;
  PERFORM public.record_milestone(
    NEW.member_id, 'first_attendance', 'attendance', NEW.id,
    jsonb_build_object('via', 'attendance_trigger', 'event_id', NEW.event_id)
  );
  RETURN NEW;
END; $fn$;
REVOKE ALL ON FUNCTION public._trg_record_first_attendance_milestone() FROM PUBLIC;

-- first_deliverable: mirrors trg_tribe_deliverable_completed_xp firing condition exactly.
-- No WHEN clause (it needs TG_OP/OLD); the guard lives in the body like the XP sibling.
CREATE OR REPLACE FUNCTION public._trg_record_first_deliverable_milestone()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
BEGIN
  IF NEW.status = 'completed'
    ...
    PERFORM public.record_milestone(
      NEW.assigned_member_id, 'first_deliverable', 'tribe_deliverable', NEW.id,
      jsonb_build_object('via', 'tribe_deliverable_trigger', 'cycle_code', NEW.cycle_code)
    );
  END IF;
  RETURN NEW;
END; $fn$;
REVOKE ALL ON FUNCTION public._trg_record_first_deliverable_milestone() FROM PUBLIC;

-- 2. Silent backfills (acknowledged_at=now()). MUST run BEFORE CREATE TRIGGER (SPEC §6.3).

-- first_attendance: earliest present attendance per member (60 members live).
...
SELECT DISTINCT ON (a.member_id)
  a.member_id, 'first_attendance',
  COALESCE(a.checked_in_at, a.created_at, now()), 'attendance', a.id, now(),
  jsonb_build_object('backfill', true, 'migration', '20260805000203')
FROM public.attendance a
WHERE a.present = true AND a.member_id IS NOT NULL
ORDER BY a.member_id, COALESCE(a.checked_in_at, a.created_at, now()) ASC
ON CONFLICT (member_id, milestone_key) DO NOTHING;

-- first_deliverable: earliest completed deliverable per assigned member. Keyed on
-- status='completed' (NOT completed_at IS NOT NULL) so the row whose completed_at is NULL
-- but status='completed' is still backfilled (1 such member live).
INSERT INTO public.member_milestones
  (member_id, milestone_key, occurred_at, source_type, source_id, acknowledged_at, metadata)
SELECT DISTINCT ON (td.assigned_member_id)
  td.assigned_member_id, 'first_deliverable',
  COALESCE(td.completed_at, td.created_at, now()), 'tribe_deliverable', td.id, now(),
  jsonb_build_object('backfill', true, 'migration', '20260805000203')
FROM public.tribe_deliverables td
WHERE td.status = 'completed' AND td.assigned_member_id IS NOT NULL
ORDER BY td.assigned_member_id, COALESCE(td.completed_at, td.created_at, now()) ASC
ON CONFLICT (member_id, milestone_key) DO NOTHING;

-- 3. Sanity guards — every eligible member must now hold the milestone (mirror PR2).
DO $sanity$
DECLARE v_missing int;
BEGIN
  SELECT count(*) INTO v_missing
  FROM (SELECT DISTINCT member_id FROM public.attendance
        WHERE present = true AND member_id IS NOT NULL) a
  WHERE NOT EXISTS (
    SELECT 1 FROM public.member_milestones mm
    WHERE mm.member_id = a.member_id AND mm.milestone_key = 'first_attendance'
  );
  IF v_missing > 0 THEN
    RAISE EXCEPTION 'PR3 #766 first_attendance backfill sanity FAIL: % present-attendance members lack a milestone', v_missing;
  END IF;

  SELECT count(*) INTO v_missing
  FROM (SELECT DISTINCT assigned_member_id AS member_id FROM public.tribe_deliverables
        WHERE status = 'completed' AND assigned_member_id IS NOT NULL) d
  WHERE NOT EXISTS (
    SELECT 1 FROM public.member_milestones mm
    WHERE mm.member_id = d.member_id AND mm.milestone_key = 'first_deliverable'
  );
  IF v_missing > 0 THEN
    RAISE EXCEPTION 'PR3 #766 first_deliverable backfill sanity FAIL: % completed-deliverable members lack a milestone', v_missing;
  END IF;
  RAISE NOTICE 'PR3 #766 backfill sanity OK.';
END$sanity$;

-- 4. Triggers (created AFTER the backfill, race-safe).
DROP TRIGGER IF EXISTS trg_record_first_attendance_milestone ON public.attendance;
CREATE TRIGGER trg_record_first_attendance_milestone
  AFTER INSERT ON public.attendance
  FOR EACH ROW
  WHEN (NEW.present = true)
  EXECUTE FUNCTION public._trg_record_first_attendance_milestone();

DROP TRIGGER IF EXISTS trg_record_first_deliverable_milestone ON public.tribe_deliverables;
CREATE TRIGGER trg_record_first_deliverable_milestone
  AFTER INSERT OR UPDATE ON public.tribe_deliverables
  FOR EACH ROW
  EXECUTE FUNCTION public._trg_record_first_deliverable_milestone();

COMMENT ON FUNCTION public._trg_record_first_attendance_milestone() IS
  '#766 PR3: records the first_attendance member_milestone on a member''s first present attendance (AFTER INSERT ON attendance, WHEN present=true; mirrors auto_complete_first_meeting). First-occurrence via record_milestone ON CONFLICT DO NOTHING.';
COMMENT ON FUNCTION public._trg_record_first_deliverable_milestone() IS
  '#766 PR3: records the first_deliverable member_milestone when a tribe_deliverable assigned to the member reaches status=completed (AFTER INSERT OR UPDATE; mirrors trg_tribe_deliverable_completed_xp firing condition). First-occurrence via record_milestone ON CONFLICT DO NOTHING.';

-- 5. check_schema_invariants() with AC + AD appended (29 -> 31). The body below is reproduced
--    verbatim from 20260805000202 (29 invariants, byte-equal) with the AC and AD RETURN QUERY
--    blocks added before END. This file is the canonical capture for the rpc-migration-coverage
--    Phase C gate (file body == live prosrc, normalized) — the whole CREATE OR REPLACE is applied
--    to live so file and live are identical by construction.
