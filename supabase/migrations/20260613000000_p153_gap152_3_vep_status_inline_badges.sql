-- p153 GAP-152.3 — Inline VEP status badges on /admin/selection + /admin/members
--
-- 1) Extends get_selection_dashboard with `vep_recon` sub-object inside each application row
--    (status_raw, last_seen_at, reconciled_at). Grouped to stay below 100-arg jsonb_build_object
--    ceiling (sediment feedback_jsonb_build_object_100args_limit.md — dashboard sits at ~96 args).
--
-- 2) Extends admin_list_members with `vep_status_raw` + `vep_last_seen_at` via LATERAL JOIN on
--    selection_applications.email (case-insensitive), taking the most recent application with a
--    non-NULL vep_status_raw. Lets /admin/members surface VEP divergence inline (e.g. member
--    `Active` but VEP says `Withdrawn` = mismatch already flagged in /admin/vep-reconciliation
--    bucket C).
--
-- Source: selection_applications.vep_status_raw populated by worker pmi-vep-sync from PMI
-- Community API (Submitted | Active | Withdrawn | Declined | OfferNotExtended).
--
-- DDL via apply_migration (per project rule .claude/rules/database.md).
