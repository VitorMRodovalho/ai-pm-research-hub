-- p173 — Invariant J chain-aware refinement
--
-- Context: J_current_version_published was flagging governance_documents
-- with current_version_id pointing to non-locked document_versions —
-- even when an approval_chain is actively in review/approved/activated
-- (closed_at NULL). The chain itself, on close, will lock the version
-- (locked_at is set in the close trigger / activation step).
--
-- Drift example caught: d7447a94 (TAP CPMAI Ciclo 3) in legitimate
-- mid-review awaiting president_go gate. Without this refinement,
-- every new TAP/Charter creates a false positive while chain runs.
--
-- Refinement: ignore docs with an open chain (status IN review/approved/
-- activated AND closed_at IS NULL). If chain is abandoned (no progress
-- for too long), that's a different drift class out of scope here.
--
-- All other invariants (A1-Q except J) unchanged — full body re-emitted
-- per CREATE OR REPLACE swap semantics.
