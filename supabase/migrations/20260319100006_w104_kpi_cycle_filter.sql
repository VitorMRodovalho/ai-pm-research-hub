-- ═══════════════════════════════════════════════════════════════
-- Fix exec_portfolio_health: filter all metrics by cycle date range
-- Previously returning all-time data instead of current cycle only
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    CASE v_target.metric_key
      ...
      -- e) CERTIFICATION: denominator = active contributors only (operational roles + designated)
      WHEN 'certification_rate' THEN
        SELECT ROUND(
          COALESCE(
            COUNT(*) FILTER (WHERE m.cpmai_certified = true)::numeric * 100
            / NULLIF(COUNT(*), 0),
            0
          )
        ) INTO v_current
