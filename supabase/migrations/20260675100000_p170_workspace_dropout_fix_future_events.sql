-- p170 Workspace dropout fix follow-up — exclude future events from mandatory denominator
--
-- PM ask 2026-05-16: Vitor mostra 8/12 (67%) mas matematicamente correto seria 8/9 (89%).
-- Diagnóstico: 3 events futuros (Jun 18, Jun 4, May 21) estão no denominador como
-- "mandatory missed" mas ainda não aconteceram → membro nunca atendeu (impossível) → penalizado.
--
-- Escala: 3 future general + 14 future tribe events = 17 events futuros inflando
-- denominator de 50 active members.
--
-- Fix: adicionar `AND e.date <= CURRENT_DATE` nos CTEs general_events e tribe_events
-- do get_attendance_panel. Events futuros saem do conjunto de mandatórios.
--
-- Side-effect: combined_pct sobe para realidade. PM Vitor: 8/12=67% → 8/9=89%.
-- Membros que aparenciam at_risk podem voltar a healthy se faltavam só por contagem
-- futura.
--
-- Rollback: restore filter as `e.date::date BETWEEN p_cycle_start AND p_cycle_end`.
...
BEGIN
  ...
  WITH general_events AS (
    ...
    WHERE e.date::date BETWEEN p_cycle_start AND LEAST(p_cycle_end, CURRENT_DATE)  -- p170: exclude future
...
COMMENT ON FUNCTION public.get_attendance_panel(date, date) IS
  'p170 — exclude future events from mandatory denominator. LEAST(cycle_end, CURRENT_DATE) garante que events ainda não acontecidos não penalizem members. Também filtra status != cancelled (events cancelados não contam).';
