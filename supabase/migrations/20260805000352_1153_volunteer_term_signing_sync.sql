...
--
-- NOTA (corpo das funções): os corpos abaixo são byte-equivalentes ao que está VIVO em PROD
-- (pg_get_functiondef); comentários explicativos ficam FORA dos blocos $function$ para não
-- introduzir drift de corpo vs. captura (tests/contracts/rpc-migration-coverage.test.mjs Phase C).
...
-- activate_volunteer_term_version — RPC admin explícita (decisão PM 2026-07-06): flip atômico
-- da ativação, gated por manage_platform (can_by_member). Governança roda 1x após o lock da
-- versão jurídica (Onda 1). Só ativa uma versão TRAVADA (locked_at). Não toca o RPC de lock da
-- cadeia (menor blast radius). Supersede qualquer outra linha 'active' antes de ativar (INV-1).
...
-- 'clauses' por rollback + para o invariante de dados do #648. CREATE OR REPLACE (mesma
-- assinatura text,text,text). Erra 'approved_body_unavailable' se a versão ativa não tiver corpo.
