  ...
  try {
    ...
    await t.test('sync trigger inserts primary email row on member insert', async () => {
      ...
      const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/members`, {
        ...
        body: JSON.stringify({
          name: 'Test Sync Member __205_synthetic__',
          email: testEmail,
          member_status: 'active',
          operational_role: 'guest',
        }),
      });
    ...
    await t.test('invariant T detects member with 0 primary emails', async () => {
      ...
      assert.equal(queryEmailsRes.status, 200);
      const emails = await queryEmailsRes.json();
      const primaryEmails = emails.filter(e => e.is_primary === true);
      assert.equal(primaryEmails.length, 0, 'Test member should have exactly 0 primary emails representing a breach of invariant T');
    });
    ...
    await t.test('cleanup deletes member and cascades to member_emails', async () => {
      ...
      const queryRes = await fetch(
        `${SUPABASE_URL}/rest/v1/member_emails?member_id=eq.${memberToDelete}`,
        {
          method: 'GET',
          headers: {
            apikey: SERVICE_ROLE_KEY,
