...
test('#191 + #188 + #415 + #459 + #209 + #301: /health declares the corrected /mcp tool count (314)', () => {
  assert.match(EF, /"\/mcp":\s*\{\s*server:\s*"nucleo-ia-hub"\s*,\s*version:\s*"2\.79\.0"\s*,\s*tools:\s*314\s*\}/,
    '/health must report 314 (311 after #209, +3 via #301 curation drive grant tools)');
});
