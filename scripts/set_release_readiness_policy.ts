...
const MODE = process.argv.find((arg) => arg.startsWith('--mode='))?.split('=')[1] ?? 'strict';
...
async function main() {
  const { data, error } = await sb.rpc('admin_set_release_readiness_policy', {
    p_policy_key: 'default',
    p_mode: MODE,
  ...
  console.log('[release-readiness-policy] updated');
  console.log(JSON.stringify(data, null, 2));
}

main().catch((error) => {
  console.error('[release-readiness-policy] failed', error);
  process.exit(1);
});
