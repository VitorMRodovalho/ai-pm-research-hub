...
- **Version:** v3.2.1 (Structural Quality) | **236 MCP tools (157R+79W) + 4 prompts + 3 resources v2.53.0** | 29 Edge Functions + **1 Cloudflare worker (pmi-vep-sync) LIVE** | **1418** unit (**1383** pass DB-aware) + 40 e2e tests | **66 ADRs + 7 amendments** (p82 CBGPL launch readiness 2026-04-29 18h: Ricardo Vargas exibe QR PMI VEP no fim do CBGPL com 1200 pessoas + 15 chapters PMI Brasil anuncia união Núcleo. Phase B portal completa: 6 UX fixes R1-R6 (capture mobile, persistence copy, vertical layout, progressive disclosure, replace flow, inline error), interview opt-out all-or-nothing com Google Calendar booking page nativa (`gh9WjefjcmisVLoh7`) + Apps Script auto-add guests (Vitor + Fabricio), profile self-service (LinkedIn/Credly/WhatsApp `update_application_profile_via_token` token-auth + credly_url column). Phase C parcial: EF `pmi-ai-analyze` (gemini-2.5-flash + responseSchema PT-BR + retry 1s/4s/9s + anti-bias guidelines, smoke Marcos: senior+manager+fit5+6 tags), dispatch fire-and-forget via `give_consent_via_token`. DEFERRED V2: CV PDF extraction + LinkedIn post scraping. ADR-0066 PMI Journey v4 Phase 1 LIVE com Amendment 2026-04-29 pivot to /ingest pattern: 4 new tables (selection_evaluation_ai_suggestions, pmi_video_screenings, onboarding_tokens, cron_run_log) + 18 RPCs (12 token-auth + 6 service helpers — incluindo opt_out_all_pillars + revert_interview_optout + update_application_profile_via_token + give_consent_via_token+ai_dispatch + campaign_send_one_off) + 2 views + 3 triggers. Worker pmi-vep-sync deployed `dfaee1d6` + browser script `extract_pmi_volunteer.js`. | p80 marathon: Phase B'' batches 15-20 (18 V3→V4 conversions across attendance/tribe/privacy/interview/showcase/admin/onboarding/selection-committee/eval-readers/dashboard tracks) — Phase B'' 131/246 (~53.3%) | p79: Mayanna feedback rounds 2+3 + #113 frontend 5/5 closed + Phase B'' batches 11-14 | p78: ADR-0065 Drive Phase 4 auto-discovery atas via cron + filename date heuristic + auto-promote + Pattern 43 4th reuse health RPC; ADR-0064 Drive Phase 3 LIVE com Path F OAuth refresh; 5 EFs Drive, 8 MCP tools Drive total | p77 ULTRA-EXTREME-marathon: Mayanna 6/7 items closed, ADR-0063 WhatsApp, drift v4 catched 3 LGPD broken | Phase B'' V3→V4: **131/246 (~53.3%)**)
