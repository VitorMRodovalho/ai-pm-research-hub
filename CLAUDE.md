...
- **Version:** v3.2.1 (Structural Quality) | **236 MCP tools (157R+79W) + 4 prompts + 3 resources v2.53.0** | 28 Edge Functions | **1418** unit (**1383** pass DB-aware) + 40 e2e tests | **65 ADRs + 6 amendments** (p80: Phase B'' batches 15-17 (register_own_presence + select_tribe + validate_privacy_policy + mark_interview_status + can_manage_knowledge + trigger_backup + register/remove_event_showcase + finalize_decisions V3→V4) + supabase-js 2.101→2.105 patch bump | p79: Mayanna feedback rounds 2+3 + #113 frontend 5/5 closed + Phase B'' batches 11-14 (mark_member_present/excused, notify_privacy_policy_change, admin_list_tribes/lineage, can_manage_comms_metrics, sync_attendance_points) | p78: ADR-0065 Drive Phase 4 auto-discovery atas via cron + filename date heuristic + auto-promote + Pattern 43 4th reuse health RPC; ADR-0064 Drive Phase 3 LIVE com Path F OAuth refresh (PM not Workspace Admin → DwD blocked, user lacks Shared Drive perm → Path B blocked); 5 EFs Drive (list/upload/create-subfolder/discover-atas + Vault helper RPC), 8 MCP tools Drive total | p77 ULTRA-EXTREME-marathon: Mayanna 6/7 items closed, ADR-0063 WhatsApp, drift v4 catched 3 LGPD broken, board_item_comments, 7MB upload, comms domain auth, MCP tools/list crit fix z.record→JSON string | Phase B'' V3→V4: **121/246 (~49.2%)**)
