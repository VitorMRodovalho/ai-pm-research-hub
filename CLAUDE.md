...
- **Version:** v3.2.1 (Structural Quality) | **236 MCP tools (157R+79W) + 4 prompts + 3 resources v2.53.0** | 28 Edge Functions + **1 Cloudflare worker (pmi-vep-sync)** | **1418** unit (**1383** pass DB-aware) + 40 e2e tests | **66 ADRs + 6 amendments** (p81: ADR-0066 PMI Journey v4 Phase 1 substrate LIVE — 4 new tables (selection_evaluation_ai_suggestions, pmi_video_screenings, onboarding_tokens, cron_run_log) + 12 RPCs (8 token-auth + 4 service helpers) + 2 views (v_cron_last_success, v_ai_human_concordance) + 3 triggers (consent gate + supersede on revoke) + B1 role_applied 'manager' + B2 PARTIAL COMPOUND UNIQUE on (vep_application_id, vep_opportunity_id) preserving 5 dual-track triaged_to_leader pairs + B3 campaign_send_one_off slug wrapper. Worker pmi-vep-sync com refresh_token KV pattern, 7d token TTL R2, sha256 token hash R2 metadata. campaign_templates seeded slug=pmi_welcome_with_token + cron_failure_alert. PM-pending: KV namespace create + refresh_token seed + 8 secrets put + frontend portal /pmi-onboarding/[token] | p80 marathon: Phase B'' batches 15-20 (18 V3→V4 conversions across attendance/tribe/privacy/interview/showcase/admin/onboarding/selection-committee/eval-readers/dashboard tracks) + supabase-js 2.101→2.105 patch bump — Phase B'' 131/246 (~53.3%) | p79: Mayanna feedback rounds 2+3 + #113 frontend 5/5 closed + Phase B'' batches 11-14 | p78: ADR-0065 Drive Phase 4 auto-discovery atas via cron + filename date heuristic + auto-promote + Pattern 43 4th reuse health RPC; ADR-0064 Drive Phase 3 LIVE com Path F OAuth refresh; 5 EFs Drive, 8 MCP tools Drive total | p77 ULTRA-EXTREME-marathon: Mayanna 6/7 items closed, ADR-0063 WhatsApp, drift v4 catched 3 LGPD broken, board_item_comments, 7MB upload, comms domain auth, MCP tools/list crit fix z.record→JSON string | Phase B'' V3→V4: **131/246 (~53.3%)**)
