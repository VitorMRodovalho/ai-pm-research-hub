...
- **Version:** v3.2.1 (Structural Quality) | **236 MCP tools (157R+79W) + 4 prompts + 3 resources v2.53.0** | 28 Edge Functions | **1418** unit (**1383** pass DB-aware) + 40 e2e tests | **65 ADRs + 6 amendments** (p79: Mayanna feedback rounds 2+3 — Items 03 RLS card-scoped fix (4 policies aceitam write_board), Item 07 Drive section card+iniciativa (initiative_drive_links surfaced), reply auto-notify type=card_comment_reply, reply textarea pre-popula @mention; #113 5/5 frontend items closed; Phase B'' batch 11 mark_member_present/excused V3→V4 | p78: ADR-0065 Drive Phase 4 auto-discovery atas via cron + filename date heuristic + auto-promote + Pattern 43 4th reuse health RPC; ADR-0064 Drive Phase 3 LIVE com Path F OAuth refresh (PM not Workspace Admin → DwD blocked, user lacks Shared Drive perm → Path B blocked); 5 EFs Drive (list/upload/create-subfolder/discover-atas + Vault helper RPC), 8 MCP tools Drive total | p77 ULTRA-EXTREME-marathon: Mayanna 6/7 items closed, ADR-0063 WhatsApp, drift v4 catched 3 LGPD broken, board_item_comments, 7MB upload, comms domain auth, MCP tools/list crit fix z.record→JSON string | Phase B'' V3→V4: **107/246 (~43.5%)**)
