...
- **Version:** v3.2.1 (Structural Quality) | **234 MCP tools (155R+79W) + 4 prompts + 3 resources v2.52.0** | 27 Edge Functions | **1418** unit (**1383** pass DB-aware) + 40 e2e tests | **64 ADRs + 6 amendments** (p78: ADR-0064 Drive Phase 3 — drive-upload-to-folder + drive-create-subfolder EFs deployed, 3 MCP tools (register_card_drive_file, upload_text_to_drive_folder, create_drive_subfolder), DwD discovery + sub claim impersonate `nucleoia@pmigo.org.br`, blocked on PM Passo 9 Workspace Admin DwD enable | p77 ULTRA-EXTREME-marathon: Drive Phase 1+1b+2 LIVE, Mayanna 6/7 items closed, ADR-0063 WhatsApp, drift v4 catched 3 LGPD broken, board_item_comments, 7MB upload, comms domain auth, MCP tools/list crit fix z.record→JSON string | Phase B'' V3→V4: **105/246 (~42.7%)**)
